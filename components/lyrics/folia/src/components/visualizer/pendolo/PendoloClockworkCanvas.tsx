  ...
  useEffect(() => {
    ...
    const render = (timestamp: number) => {
      ...
      smoothedBassRef.current +=
        (clampedBass - smoothedBassRef.current) *
      ...
      if (!p.paused) {
        ...
        secondGearVelocityRef.current +=
          displacement * 92 * p.motionProfile.escapementSpringMultiplier * dt;
        secondGearVelocityRef.current *= Math.exp(
          -13 * p.motionProfile.escapementDampingMultiplier * dt,
        );
        secondGearAngleRef.current += secondGearVelocityRef.current * dt;
      }
      const bassOscillation =
        Math.sin(phaseRef.current) *
        (0.15 + bass * 0.7 * p.motionProfile.bassResponseMultiplier) *
      ...
      if (p.showCenterGradient) {
        ...
        grad.addColorStop(0.7, colorWithAlpha(bgCol, 0.2));
      ...
      if (p.showCover && currentCoverImg) {
        ...
        ctx.drawImage(
          currentCoverImg,
          p.centerX - coverRadius,
          p.centerY - coverRadius,
          size,
          size,
        );
        ctx.restore();
      ...
      const primaryAlpha10 = colorWithAlpha(p.primaryTextColor, 0.1 * decorOpacityMultiplier);
      ...
      const gearPrimarySubtleAlpha = colorWithAlpha(
        p.primaryTextColor,
        0.32 * decorOpacityMultiplier,
      );
      const gearAccentAlpha = colorWithAlpha(p.accentTextColor, 0.58 * decorOpacityMultiplier);
      const gearAccentStrongAlpha = colorWithAlpha(
        p.accentTextColor,
        0.72 * decorOpacityMultiplier,
      );
      ...
      for (let i = 0; i < guillocheRayCount; i++) {
        ...
        ctx.strokeStyle =
          i % 2 === 0
            ? primaryAlpha10
            : colorWithAlpha(p.primaryTextColor, 0.07 * decorOpacityMultiplier);
        ctx.lineWidth = 0.7;
        ctx.beginPath();
      ...
      drawGearTeeth(
        ...
        gearAccentStrongAlpha,
        2.1,
        colorWithAlpha(p.accentTextColor, 0.1 * decorOpacityMultiplier),
      );
      ...
      drawHairspring(
        ...
        colorWithAlpha(p.accentTextColor, 0.3 * decorOpacityMultiplier),
        0.8,
      );
      ...
      ctx.beginPath();
      ctx.arc(0, 0, transR * 0.8, 0, Math.PI * 2);
      ctx.clip();
      ...
      for (let i = 1; i <= genevaStripeCount; i++) {
        ...
        ctx.strokeStyle =
          i % 2 === 0
            ? primaryAlpha10
            : colorWithAlpha(p.primaryTextColor, 0.06 * decorOpacityMultiplier);
        ctx.lineWidth = 0.7;
        ctx.beginPath();
      ...
      const focalAxisEndRadius = Math.min(p.lyricRingRadius - Math.max(14, p.baseRadius * 0.025));
