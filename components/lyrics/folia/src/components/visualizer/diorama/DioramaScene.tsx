...
export const resolveTextLife = (distanceToCamera: number): number => {
  const farT = clamp01(
    (TEXT_FADE_IN_END - distanceToCamera) / (TEXT_FADE_IN_END - TEXT_FADE_IN_START),
  );
  const nearT = clamp01(
    (distanceToCamera - TEXT_DISSOLVE_END) / (TEXT_DISSOLVE_START - TEXT_DISSOLVE_END),
  );
...
const DioramaScene: React.FC<DioramaSceneProps> = ({
  ...
  useEffect(() => {
    ...
    if (typeof document !== "undefined" && document.fonts?.ready) {
      document.fonts.ready.then(() => {
        if (mounted) setFontsEpoch((e) => e + 1);
      });
    }
    return () => {
      mounted = false;
  ...
  const mountedIndices = useMemo(() => {
    ...
    if (transitionOutgoingIndex != null)
      addWindow(transitionOutgoingIndex, OUTGOING_LINES_BEHIND, OUTGOING_LINES_AHEAD);
  ...
  const visibleLines = useMemo(() => {
    ...
    for (const i of mountedIndices) {
      ...
      const placement = getDioramaTextPlacement(
        resolved.localIndex,
        resolved.segment.seed,
        motion.weaveScale,
      );
      const position = {
  ...
  const particleClusters = useMemo(() => {
    ...
    for (const i of Array.from(clusterIndices).sort((a, b) => a - b)) {
      ...
      buildFormation(localIndex, segment.seed, shot, frame, placement, particleScale).forEach(
        (piece, slot) => {
          result.push({
            ...piece,
            key: `${i}-${slot}`,
    ...
    return selectVisibleDioramaClusters(result, geometryVisibility).filter((cluster) =>
      mounted.has(cluster.sourceLine),
    );
  }, [
    geometryMode,
    mountedIndices,
  ...
  useEffect(() => {
    moteWrittenRef.current = [];
  }, [moteCircumference, moteRadial, activeSegKey]);
  ...
  if (transitionOutgoingIndex != null && !lineRasterCacheRef.current.has(transitionOutgoingIndex)) {
    ...
    if (leaving?.line?.fullText) {
      lineRasterCacheRef.current.set(
        transitionOutgoingIndex,
        rasterDioramaLine(leaving.line.fullText, fontStack),
      );
    }
  }
  ...
  useFrame((frameState, delta) => {
    ...
    if (showParticles) {
      ...
      let dirty = false;
      for (
        let line = globalIndex - DIORAMA_MOTE_LINES_BEHIND;
        line <= globalIndex + DIORAMA_MOTE_LINES_AHEAD;
        line += 1
      ) {
    ...
    visibleLines.forEach(({ index, placement, isOutgoing }) => {
      ...
      const fit =
        resolveFrameFitScale(worldWidth, DIORAMA_HERO_DISTANCE, fov, aspect) *
        placement.scale *
        lyricsFontScale;
    ...
    if (
      shouldResetDioramaUnitState(
        prevActiveGlobalRef.current,
        globalIndex,
        unitLightValsRef.current?.length,
        activeLineUnits.length,
      )
    ) {
    ...
    if (unitsGroup && activeUnitsRaster && activeEntry && activeLine) {
      const fit =
        resolveFrameFitScale(activeUnitsRaster.lineWidth, DIORAMA_HERO_DISTANCE, fov, aspect) *
        activeEntry.placement.scale *
        lyricsFontScale;
      ...
      const tintSeparation =
        Math.abs(damped.accent.r - damped.primary.r) +
      ...
      if (tintSeparation < 0.4) {
        ...
        const targetLum =
          primaryLum > 0.5
            ? accentLum * (1 - 0.5 * deficit)
            : accentLum + (1 - accentLum) * 0.55 * deficit;
      ...
      activeLineUnits.forEach((unit, i) => {
        ...
        baseMat.opacity = Math.min(
          1,
          (UNSUNG_UNIT_OPACITY +
            (ACTIVE_LINE_OPACITY - UNSUNG_UNIT_OPACITY) * sungMix +
            0.08 * Math.min(1, gradientEnergy)) *
            life,
        );
        ...
        const unitTarget =
          keywordUnitColors.get(i) ?? (gradientIntensity > 0 ? _gradDeep : _sungTint);
        ...
        if (soulMat && soulMesh) {
          ...
          const flightMix = sung
            ? smoothstep01(clamp01((now - unit.endTime) / SOUL_HANDOFF_SECONDS))
            : 0;
          ...
          soulMesh.position.y =
            LINE_FONT_SIZE * (SOUL_ACTIVE_LIFT_EM * onGlyph + SOUL_DETACH_LIFT_EM * flown);
  ...
  return (
    <group>
      ...
      {showLyrics &&
        visibleLines.map(({ index, line, position, quaternion, isOutgoing }) => {
          ...
          return (
            ...
            <mesh
              key={index}
              ref={(el) => {
                if (el) lineMeshRefs.current.set(index, el);
                else lineMeshRefs.current.delete(index);
              }}
              position={position}
            ...
            >
              <planeGeometry
                args={[raster.canvasWidthPx * worldPerPx, raster.canvasHeightPx * worldPerPx]}
              />
              <meshBasicMaterial
                ref={(el) => {
                  if (el) lineMatRefs.current.set(index, el);
                  else lineMatRefs.current.delete(index);
                }}
                map={raster.texture}
                transparent
      ...
      {showLyrics && activeLine?.fullText && activeEntry && activeUnitsRaster && (
        ...
        <group
          ...
          quaternion={activeEntry.quaternion}
        >
          {activeUnitsRaster.units.map((placed, unitIndex) => (
            <React.Fragment key={unitIndex}>
              <mesh
                ref={(el) => {
                  unitGlowMeshRefs.current[unitIndex] = el;
                }}
                visible={false}
              ...
              >
                ...
                <meshBasicMaterial
                  ref={(el) => {
                    unitGlowMatRefs.current[unitIndex] = el;
                  }}
              ...
              <mesh
                ref={(el) => {
                  unitSoulMeshRefs.current[unitIndex] = el;
                }}
                visible={false}
              ...
              >
                ...
                <meshBasicMaterial
                  ref={(el) => {
                    unitSoulMatRefs.current[unitIndex] = el;
                  }}
              ...
              <mesh position={[placed.centerX, 0, 0]} renderOrder={19}>
                ...
                <meshBasicMaterial
                  ref={(el) => {
                    unitBaseMatRefs.current[unitIndex] = el;
                  }}
                  map={placed.raster.baseTexture}
                  transparent
