...
const StrengthControl: React.FC<StrengthControlProps> = ({
  ...
  const matchedPreset = STRENGTH_PRESETS.find(
    (preset) => Math.abs(preset.value - intensity) < 0.001,
  )?.key;
  ...
  const choices: Array<{ key: StrengthChoice; label: string }> = [
    { key: "low", label: t("folia.options.dioramaStrengthLow") },
    { key: "mid", label: t("folia.options.dioramaStrengthMid") },
    { key: "high", label: t("folia.options.dioramaStrengthHigh") },
    { key: "custom", label: t("folia.options.dioramaStrengthCustom") },
  ];
  ...
  return (
    <>
      ...
      >
        {t("folia.options.dioramaEffectStrength")}
      </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            <span style={{ color: "var(--text-primary)" }}>
              {t("folia.options.dioramaCustomStrength")}
            </span>
          ...
          <input
            ...
            step="0.05"
            value={intensity}
            aria-label={`${label} ${t("folia.options.dioramaCustomStrength")}`}
