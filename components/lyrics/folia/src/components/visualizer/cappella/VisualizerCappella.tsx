...
import {
  ...
  resolveCappellaAvatarUrl,
} from "./avatarImages";
import {
  createCappellaAgentSenderResolver,
  type CappellaMessageSender,
} from "./cappellaMessageSenders";
...
const pickStableEmoImage = (
  imagePool: CappellaEmojiImage[],
...
const getCappellaIntensityConfig = (
  animationIntensity: Theme["animationIntensity"],
): CappellaIntensityConfig => {
...
const buildCappellaMessages = (
  ...
  if (lines.length === 0) {
    const fallbackEmo = showEmoMessages
      ? pickStableEmoImage(
          ...
          "no-lyrics",
          titleText,
          config.sequencing.forceRightEveryLines,
        )
      : null;
  ...
  lines.forEach((line, lineIndex) => {
    ...
    const shouldForceRight =
      !agentSender && (lineIndex + 1) % config.sequencing.forceRightEveryLines === 0;
    const shouldCarrySender = Boolean(
      !agentSender &&
      isShortLine &&
      lastLyricSender &&
    ...
    const baseSide =
      config.sequencing.sideSequence[sideSequenceCursor % config.sequencing.sideSequence.length];
    const shouldFlipSide =
      !shouldForceRight &&
      seededUnit("flip", line.startTime, lineIndex) < config.sequencing.sideFlipChance;
    const resolvedSide = shouldFlipSide ? (baseSide === "left" ? "right" : "left") : baseSide;
    ...
    if (canAppendRandomEmo) {
      const score = seededUnit(
        "random-emo",
        line.startTime,
        line.endTime,
        lineIndex,
        config.sequencing.randomEmoChance,
      );
      if (score < config.sequencing.randomEmoChance) {
        const reactionImage = pickStableEmoImage(
          emoImagePool,
          "reaction",
          line.startTime,
          line.endTime,
...
const buildCharacterRevealTimes = (line: Line, characters: string[]) => {
  ...
  line.words.forEach((word, index) => {
    ...
    for (
      let characterIndex = previousWordEndCharacterIndex;
  ...
  for (
    let characterIndex = previousWordEndCharacterIndex;
    characterIndex < revealTimes.length;
    characterIndex += 1
  ) {
...
const buildCharacterFadeDurationsMs = (line: Line, characters: string[]) => {
  ...
  if (lineTimeline.length === characters.length) {
    lineTimeline.forEach((timing, index) => {
      fadeDurationsMs[index] = Math.max(
...
const getTimedMessageState = (
  ...
  if (message.kind === "emo") {
    return {
      isActive:
        currentTime >= message.activationStartTime && currentTime < message.activationEndTime,
...
const getBubbleColors = (message: CappellaMessage, theme: Theme) => {
  ...
  const avatarTone =
    (message.avatarIndex % (AVATAR_GRID_SIZE * AVATAR_GRID_SIZE)) /
    (AVATAR_GRID_SIZE * AVATAR_GRID_SIZE - 1);
  const accentMix = 0.18 + avatarTone * 0.62;

  return {
    backgroundColor: mixColors(theme.secondaryColor, theme.accentColor, accentMix, 1),
    borderColor: mixColors(
      theme.secondaryColor,
      theme.accentColor,
      Math.min(accentMix + 0.18, 1),
      0.26,
    ),
...
const CappellaAvatar: React.FC<{
  ...
  const resolvedIndex = shouldUseAvatarGridCrop
    ? side === "right"
      ? RIGHT_AVATAR_INDEX
      : LEFT_AVATAR_INDICES[avatarIndex % LEFT_AVATAR_INDICES.length]
    : avatarIndex;
...
const ActiveCappellaText: React.FC<{
  ...
  const visibleFadeDurations = revealPlan.fadeDurationsMs.slice(
    0,
...
const CappellaMessageRow = React.forwardRef<HTMLDivElement, CappellaMessageRowProps>(
  (
    ...
    const timedState = timedData
      ? getTimedMessageState(timedData, currentTime.get(), currentLineIndex)
      : null;
    ...
    const bubblePaddingX = isActiveMessage
      ? motionConfig.activePaddingX
      : motionConfig.inactivePaddingX;
    const bubblePaddingY = isActiveMessage
      ? motionConfig.activePaddingY
      : motionConfig.inactivePaddingY;
    ...
    const [isTimestampVisible, setIsTimestampVisible] = useState(
      () =>
        ...
        (timedData.kind === "emo"
          ...
          : isPassedMessage ||
            currentTime.get() >= getTimestampReadyTime(preparedMetrics, timedData.line)),
    );
    // 表情图片：active 时更大
    const emoImageSize = isActiveMessage
      ? motionConfig.emoActiveSize
      : motionConfig.emoInactiveSize;
    const targetSize = useMemo(() => {
      ...
      if (isActiveMessage) {
        ...
        const clampedTargetCount = Math.max(
          0,
          Math.min(targetCharacterCount, prepared.sizes.length - 1),
        );
    ...
    const scaleOverflow =
      isActiveMessage && motionConfig.activeScale > 1
        ? Math.ceil(
            Math.max(
              isEmoMessage
                ? emoImageSize
                : message.kind === "lyric" && preparedMetrics
                  ? (preparedMetrics.sizes[preparedMetrics.sizes.length - 1]?.height ??
    ...
    useEffect(() => {
      ...
      setTargetCharacterCount((current) =>
        current === nextTargetCount ? current : nextTargetCount,
      );
      const nextTimestampVisible =
        message.kind === "emo"
      ...
      if (message.kind === "lyric" || message.kind === "emo") {
        const nextTimestampVisible =
          message.kind === "emo"
            ? latest >= message.activationEndTime
            : isPassedMessage || latest >= getTimestampReadyTime(preparedMetrics, message.line);
        setIsTimestampVisible((current) =>
          current === nextTimestampVisible ? current : nextTimestampVisible,
        );
      }
      ...
      >
        <motion.div
          animate={{
            ...
            scale: isActiveMessage
              ? motionConfig.activeScale
              : isPassedMessage
                ? motionConfig.passedScale
                : 1,
        ...
        >
          ...
          {isEmoMessage ? (
            <motion.div
              ...
              transition={{
                width: {
                  duration: motionConfig.emoSizeTransitionDuration,
...
const VisualizerCappella: React.FC<VisualizerCappellaProps> = (props) => {
  ...
  const [visibleLineIndex, setVisibleLineIndex] = useState(() =>
    getVisibleLineIndexAtTime(lines, currentTime.get()),
  );
  const visibleLineIndexRef = useRef(visibleLineIndex);
  const titleText = songTitle?.trim() || t("folia.ui.noTrack");
  const avatarSeed = seed ?? titleText;
  const intensityConfig = useMemo(
    () => getCappellaIntensityConfig(theme.animationIntensity),
  ...
  const resolvedCappellaTuning = useMemo<CappellaTuning>(
    () => ({
      ...
      avatarSource:
        cappellaTuning.avatarSource === "builtin" ||
        cappellaTuning.avatarSource === "color" ||
        cappellaTuning.avatarSource === "cover" ||
        (cappellaTuning.avatarSource === "custom" && cappellaCustomAvatarImages.length > 0)
          ? cappellaTuning.avatarSource
          : DEFAULT_CAPPELLA_TUNING.avatarSource,
    }),
    [
      cappellaCustomEmojiImages.length,
      cappellaCustomAvatarImages.length,
      cappellaTuning.avatarSource,
      cappellaTuning.emojiPackSource,
      cappellaTuning.showEmoMessages,
    ],
  );
  ...
  const messages = useMemo(
    () =>
      buildCappellaMessages(
        lines,
        titleText,
        intensityConfig,
        resolvedCappellaTuning,
        activeEmoImages,
        isPreviewMode,
      ),
