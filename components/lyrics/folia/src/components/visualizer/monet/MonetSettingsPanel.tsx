...
const SectionLabel: React.FC<{ children: React.ReactNode; theme: MonetSettingsTheme }> = ({
  children,
  theme,
}) => (
  <div
...
export const MonetSettingsPanel: React.FC<VisualizerSettingsPanelProps> = ({
  ...
  const resolvedTuning = {
    keywordColoringEnabled:
      monetTuning.keywordColoringEnabled ?? DEFAULT_MONET_TUNING.keywordColoringEnabled,
    ...
    showPortraitDragHanger:
      monetTuning.showPortraitDragHanger ?? DEFAULT_MONET_TUNING.showPortraitDragHanger,
  };

  const keywordColoringOptions = useMemo<PresetOption<boolean>[]>(
    () => [
      { value: true, label: t("folia.options.monetKeywordColoringOn") },
      { value: false, label: t("folia.options.monetKeywordColoringOff") },
    ],
    [t],
  );
  ...
  const portraitSourceOptions = useMemo<PresetOption<MonetPortraitSource>[]>(
    () => [
      { value: "cover", label: t("folia.options.monetPortraitSourceCover") },
      {
        value: "custom",
        label: t("folia.options.monetPortraitSourceCustom"),
  ...
  const audioStyleOptions = useMemo<PresetOption<MonetAudioStyle>[]>(
    () => [
      { value: "bar", label: t("folia.options.monetAudioStyleBar") },
      { value: "line", label: t("folia.options.monetAudioStyleLine") },
    ],
    [t],
  );
  const portraitStyleOptions = useMemo<PresetOption<"rectangular" | "square">[]>(
    () => [
      { value: "rectangular", label: t("folia.options.monetPortraitStyleRectangular") },
      { value: "square", label: t("folia.options.monetPortraitStyleSquare") },
    ],
    [t],
  );
  const showPortraitDragHangerOptions = useMemo<PresetOption<boolean>[]>(
    () => [
      { value: true, label: t("folia.options.monetPortraitDragHangerShow") },
      { value: false, label: t("folia.options.monetPortraitDragHangerHide") },
    ],
    [t],
  );
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (result.ok) {
      ...
      setFeedback(result.error || t("folia.options.monetUploadPortrait"));
    }
  };
    ...
    >
      <div className="space-y-1">
        <div className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>
          {t("folia.options.monetSettings")}
        </div>
        <div className="text-xs opacity-50" style={{ color: "var(--text-secondary)" }}>
          {t("folia.options.monetSettingsDesc")}
        </div>
      </div>
      ...
      <div className="space-y-3.5 rounded-[18px] border border-white/5 bg-black/5 p-3.5 dark:bg-white/5">
        ...
        <SliderControl
          label={t("folia.options.monetFontScale")}
        ...
        <PresetGroup
          label={t("folia.options.monetKeywordColoring")}
        ...
        <PresetGroup
          label={t("folia.options.monetShowDescription")}
      ...
      <div className="space-y-3.5 rounded-[18px] border border-white/5 bg-black/5 p-3.5 dark:bg-white/5">
        ...
        <PresetGroup
          label={t("folia.options.monetPortraitSource")}
        ...
        <PresetGroup
          label={t("folia.options.monetPortraitStyle")}
        ...
        <PresetGroup
          label={t("folia.options.monetPortraitDragHanger")}
        ...
        <div className="space-y-2.5">
          <SectionLabel theme={theme}>{t("folia.options.monetUploadPortrait")}</SectionLabel>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <ImagePlus size={15} />
              <span>{t("folia.options.monetUploadPortrait")}</span>
            </button>
            <button
              type="button"
            ...
            >
              <Trash2 size={15} />
              <span>{t("folia.options.monetClearPortrait")}</span>
            </button>
          </div>
      ...
      <div className="space-y-3.5 rounded-[18px] border border-white/5 bg-black/5 p-3.5 dark:bg-white/5">
        <div
          ...
          style={{ color: theme.accentColor }}
        >
          {t("folia.options.monetAudioStyle")}
        </div>

        <PresetGroup
          label={t("folia.options.monetAudioStyle")}
