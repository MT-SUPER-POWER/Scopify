...
const MonetFloatingDecor: React.FC<MonetFloatingDecorProps> = ({ theme, staticMode = false }) => {
  ...
  const renderParticleContent = (p: FloatingParticle) => {
    if (p.iconName) {
      ...
      if (Icon) {
        return (
          <Icon size={p.size} strokeWidth={1} absoluteStrokeWidth color={theme.secondaryColor} />
        );
      }
    }
