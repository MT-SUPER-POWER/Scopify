...
export function PlayerCommandHandler() {
  useEffect(() => {
    ...
    cmdChannel.onmessage = (event) => {
      ...
      switch (type) {
        case 'PLAY_NEXT': player.playNext(); break;
        case 'PLAY_PREV': player.playPrev(); break;
        case 'TOGGLE_PLAY': player.togglePlaying(); break;
        case 'SET_VOLUME': player.setVolume(payload); break;
        case 'SYNC_USER_STORE':
          const userStr = localStorage.getItem("user-storage");
          if (userStr) {
            try { useUserStore.setState(JSON.parse(userStr).state); } catch (e) { }
          }
          break;
        case 'REQUEST_STATE':
