...
export default function AlbumPage() {
  ...
  const ALBUM_INFO = useMemo(() => {
    ...
    return {
      type: album.type || t("album.meta.type"), // 例如: "专辑", "EP"
  ...
  useEffect(() => {
    if (ALBUM_INFO?.cover) {
      const cachedColor = colorCache.get(ALBUM_INFO.cover);
      if (cachedColor) {
  ...
  if (!albumId)
    return (
      <div className="w-full min-h-screen flex flex-col items-center justify-center bg-[#121212] text-zinc-400 gap-4">
        ...
        <span className="text-lg font-medium tracking-wider">{t("album.empty.invalidId")}</span>
      </div>
    );

  if (isLoading && !albumDetail)
    return (
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-start gap-6 px-6 pt-24 pb-6">
        ...
        >
          <Image
            ...
            src={
              ALBUM_INFO?.cover ||
              "https://pixabay.com/images/download/clker-free-vector-images-turntable-309662_1920.png"
            }
            alt={ALBUM_INFO?.title || t("album.coverAlt")}
        ...
        <div className="flex flex-col flex-1 min-w-0 text-white pt-1 md:pt-2">
          ...
          <div className="flex flex-wrap items-center gap-2.5 text-sm text-white/80 drop-shadow-md">
            <div className="flex items-center gap-2 group cursor-pointer mr-1 text-white">
              {ALBUM_INFO?.artistAvatar ? (
                <Image
                  width={49}
                  height={49}
                  src={ALBUM_INFO.artistAvatar}
                  alt={ALBUM_INFO.artistName}
            ...
            <span className="font-medium text-white">
              {t("album.totalSongs", { count: ALBUM_INFO?.totalSongs ?? 0 })}
            </span>
          </div>
        </div>
      </div>
