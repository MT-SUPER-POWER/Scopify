...
- **补充桌面工程结构优化计划**：参考 SPlayer 的配置与目录职责，规划 Scopify 独立打包配置、统一构建路径及修正文档目录；本次仅新增计划，尚未实施结构调整。
- **约束 Agent 的额外验证流程**：明确禁止未经用户要求编写或运行测试、冒烟、哈希、类型检查及其他附加校验；取消 GitNexus 强制前置分析与提交前验证，要求围绕实际开发任务完成修改后直接交付。
- **引入 electron-positioner 规范托盘窗口定位**：使用 `electron-positioner` 替代原先在 `tray.ts` 中针对屏幕边界和任务栏的硬编码魔法偏移与手动坐标计算，补充对应 TypeScript 类型声明，使系统托盘弹出气泡具备标准的屏幕边界自适应避让与居中能力。

### Fixed
- **托盘显示改用普通窗口绘制**：参考主窗口改为不透明宿主；实际卡片挂载与字体首帧绘制，移除托盘透明背景切换及整卡缩放淡入，避免显示过程中空帧和背景切换造成闪烁。
- **托盘右键防闪与点击位置定位**：右键改为幂等打开，合并短时间重复输入并延迟确认失焦，避免托盘焦点切换引发反复显隐；首次显示等待窗口就绪，以右键时的鼠标坐标为锚点，按当前显示器工作区翻转及约束位置，支持副屏负坐标与隐藏托盘抽屉。
