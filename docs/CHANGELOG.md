...
- 迁移 Folia 和声歌词渲染与显示控制，并为未来带歌手分轨的歌词源保留完整数据契约。
- 迁移随机歌词动画、动画与背景步进切换、设置预览暂停、相邻曲目标题预览和播放防休眠。
- 迁移全局歌词偏移校准模态，支持草稿预览、拖拽刻度尺、滚轮和键盘微调。
- 均衡器加入三个自定义槽位，以及高低切、驱动、颗粒、磁带摆动、黑胶噪声、立体声宽度、混响和低频冲击效果链。
- 新增 Folia 视频导出，可从歌曲开头或当前位置录制，支持取消、自动结束和 Electron MP4/WebM 保存。
- 自定义音频预设支持动态新建、改名、复制、删除和参数自动保存，并兼容迁移旧的固定槽位。

## Visual

- **莫奈布局改用裸图标控制**：快速设置中的半屏渐变与全屏叠色不再显示文字胶囊或按钮底色，分别使用侧栏布局和全屏扩展图标表达，完整名称保留在悬停说明与无障碍标签中。

- **拆分 Folia 与 Playbar 均衡器界面**：两处入口不再共享带视觉 token 的均衡器组件；Playbar 使用站点主题，Folia 使用当前歌词主题，避免强调色、边框和弹层背景串色。

- **精简 Folia 快速选项操作**：移除歌词动画与背景模式两侧的逐项切换箭头，统一通过当前选项按钮打开完整菜单，避免两套选择方式并存。

- Visual Setting 的歌词动画与背景模式改用 Folia 下拉选择按钮，保留播放器原有设置入口。
- 将和声歌词控制移入“字幕”Tab 的内容区，与字幕背景、字体和缩放设置统一滚动，不再占用独立头部。
- 统一和声歌词与 Folia 设置面板的主题 token，修复白昼主题下卡片、文字和开关颜色割裂的问题。
- 修复 Monet、Nomand 与 Latent 背景设置沿用旧翻译命名空间、界面直接显示翻译键的问题。
- 补齐 Nomand 五种背景滤镜及其参数的中、繁、英翻译，并限制设置项最小宽度，避免长翻译键撑破右侧面板。
- 补齐 Sonnet v0.6.19 的新增动态图形、场景装饰和主题化镜头素材。
- 补齐 Monet 背景漂移和光带效果，以及 Nomand 的百叶玻璃、纸张、半色调、镜头畸变等滤镜。
- 四种漫游背景改为使用 Folia 注册表提供各自的快捷控制。
- 重做 Folia 均衡器为主题化声音工作台，统一顶部层级、预设管理、EQ 和音色效果区域的视觉语言。

## Quality

- 扩展 desktop-contract 视频导出能力，统一由主进程处理窗口捕获、保存路径、文件写入和窗口恢复。

## Fixed

- **修复 Folia 视频导出的运行时边界**：视频导出通过 Web Runtime 调用桌面能力，不再由界面组件直接访问 Electron preload bridge。

- **修复 Playbar 均衡器预设下拉不可见**：将预设菜单 portal 提升到音频设置 Popover 之上，点击当前预设后可正常显示完整选项。

- 修正 Folia 设置页弹层级别过高的问题，使字体/歌词上传、性能警告、歌词匹配、主题库、均衡器及其他二级弹窗能够显示在设置页上方并正常交互。

- 将 @paper-design/shaders-react 对齐至 Folia v0.6.19 使用的 0.0.80，修复 Nomand 镜头畸变滤镜缺少 LensDistortion 导出导致的构建失败。
- 重排 Folia 均衡器顶部工具条，并提升预设下拉菜单层级，修复菜单被模态遮罩挡住而无法操作的问题。
