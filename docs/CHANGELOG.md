...
- **统一列表拖拽风格**：侧栏、歌单歌曲与播放队列移除把手，支持整项长按拖动；使用封面与文字的小型跟手缩略图和插入线，原行保留可读内容，松手直接落位淡入。保留普通点击与排序 API，播放队列仍只修改本地队列。

- **移除队列拖拽说明文字**：删除底部状态说明、拖动卡片上方的位置提示和手柄悬浮说明，保留跟手卡片、让位动画及落位反馈。

- **精简队列提示**：移除空闲时的常识性排序说明及其占位，仅在拖动与落位时显示反馈。

- **恢复完整队列拖动反馈**：修复拖动浮层被播放队列弹层遮挡的层级问题，恢复拖动期间的邻项让位动画，显示跟手歌曲卡片、可读占位、实时目标序号与落位确认，补充键盘操作提示；松手后仍直接在目标位置淡入，不执行跨行回放动画。

- **改善播放队列拖拽与落位反馈**：拖动占位保留六成透明度的歌曲封面与文字；松手立即清除排序位移，在目标位置短暂淡入，取消拖后跨行过渡与自动焦点滚动，并让歌曲标识在移动前后保持稳定。

- **完善播放队列拖拽交互**：使用 dnd-kit 替换原生 HTML 拖放，序号处提供可聚焦手柄，支持指针与键盘排序、浮动歌曲预览、列表让位动画、原位占位及边缘自动滚动；保留虚拟列表，拖动期间队列发生外部变化时取消提交，只调用播放队列移动操作。

- **收紧播放队列拖拽布局**：移除队列项独立的拖拽占位列，悬停时在序号位置显示拖拽手柄，缩小行内边距与封面前间距；拖拽只调整当前播放队列，不影响实际歌单顺序。
