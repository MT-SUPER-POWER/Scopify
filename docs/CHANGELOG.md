...
- **乐签卷撕跟手性**：取消固定 90px 放大映射和按住时自动撕离，改用实际票高与有方向的拖动位移驱动卷曲，支持反拖展开及回卷中重新抓取；仅松手后完成撕离或回卷。卷曲绘制合并到每帧一次，分段明暗使用遮罩替代逐段滤镜。

- **乐签双向卷撕**：票根由整体旋转扯离改为 48 段纸面弯曲，随拖动卷起并露出纸背，撕裂前沿逐段推进；根据起手位置与首段拖动方向选择自上而下或自下而上，横向拖动沿靠近起手的一端卷起。卷完后再脱落关闭，保留未完成时的回卷和独立播放入口。

- **乐签撕票与寄语调整**：撕票响应区域扩大至票根的日期及空白区域，保留播放、二维码的独立点击；分离拖拽与离场动画，缩短拉动距离，达到阈值即撕离，取消或短距离拖动仍回弹。评论区取消底部挤压，改为舒展的寄语排版，增强文字对比、引号层次和署名间距。
...
- **莱茵档案阅读体验**：以《明日方舟》莱茵生命档案终端的视觉语言重新组织背景，调整阵列透视、右侧构图、象牙白材质与冷暖光照，为歌词留出清晰阅读区；切歌抽取的档案轻微转向读者，玻璃外壳内展示当前单曲封面与歌名，移除通用编号标签。封面独立异步载入，归位中的旧档案保留自己的歌曲内容；继续复用常驻场景，暂停冻结当前呼吸与展示进度。
...
- **调整莱茵背景切歌展示与清晰度**：普通播放时所有档案片留在阵列中呼吸，仅切歌触发 0.9 秒升起、1.2 秒展示、1.1 秒归位；连续切歌让旧展示从当前位置归位，暂停保留动画进度。移除景深模糊，默认画质的 DPR 上限提高至 2 并启用 SMAA，保留玻璃材质和环境遮蔽。
