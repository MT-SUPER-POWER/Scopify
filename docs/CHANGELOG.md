...
- **统一命令工作区悬浮胶囊选框与内边距节奏**：一级命令根页面、二级设置页、播放队列、专辑曲目与搜索建议统一采用四周留有 10px 呼吸感（`px-2.5 py-2`）的悬浮圆角（`rounded-lg`）高亮选框，内部配合紧凑轻盈的内边距（`px-3.5 py-2.5`）与统一的图标徽标容器（`size-7 bg-white/8`），使整个工作区兼具 Raycast 风格的现代悬浮质感与精炼行高。
