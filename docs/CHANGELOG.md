...
- **音频输出设备选择与快捷键**：将 PlayBar 原蓝牙占位按钮升级为音频输出设备选择控件，点击即可在 Popover 中直接切换音频输出设备并支持列表刷新；配置全局快捷键 `Ctrl/Cmd+Alt+O`（`toggle-audio-output-device`）快速打开/关闭输出设备面板，同步补齐简中、繁中与英文文案。

- **首页丰富度扩充（新歌速递 · 官方排行榜 · 新碟上架）**：首页引入三大核心音乐发现板块。「新歌速递」提供双列紧凑单曲列表，支持单曲试听与一键播放全部；「官方排行榜」横向精选飙升、新歌、热歌、原创四大核心榜单，呈现 Top 3 歌曲预览并支持一键整榜播放与进入详情；「新碟上架」采用标准卡片网格呈现最新数字专辑与 EP，支持单行折叠收纳与分页翻页。全局各分区动线编排顺畅，骨架屏同步更新。

- **自定义歌词配色与桌面应用**：配色区新增独立命名方案，支持颜色编辑、快捷应用及单项/批量删除，应用时保留字体、布局和动画。桌面小歌词复用胶囊渲染，接入播放投影的真实时间、逐词/整句填色、翻译与歌词偏移；设置通过同源存储同步，窗口按内容尺寸调整并保留拖动、置顶、穿透和关闭入口。

- **Playbar 桌面小歌词入口**：新增控制弹层，连接现有独立桌面歌词窗口的显示、置顶和鼠标穿透，并提供外观及快捷键设置入口；默认 Ctrl/Cmd+Alt+L 切换窗口，Ctrl/Cmd+Shift+L 切换控制面板，沿用应用内快捷键编辑与冲突检查。本次不替换现有窗口渲染。

- **歌词样式方案库与播放填色**：支持命名保存完整歌词样式、快捷应用、重命名、更新、单项及批量删除，方案独立持久化。新增未唱底色与已唱渐变覆盖、边缘柔和度、整句时长、暂停及进度拖动；多行文本按字符顺序推进，预览进度暂为模拟时钟，后续接入真实歌词时间轴。

- **字幕歌词预览工作台**：借鉴 subtitle-app 的胶囊排版、主副行、中文折叠与流式吐字，将字体、字重、颜色与渐变、宽度、对齐、间距、圆角、模糊和阴影接入外观设置；新增可调时长的淡入、浮入、展开及逐字效果。预览支持场景切换、自定义文案、显隐、重播、循环和原尺寸查看，默认单行；参数持久化并保留旧主题库与轮换设置。本阶段仅实现预览，独立桌面窗口、置顶与歌词数据同步留待后续。

- **主题批量管理**：我的主题新增管理模式，支持多选、全选及批量删除；删除确认列出主题并说明当前背景、每日轮换和时段引用的影响。单个与批量删除共用原子更新逻辑，移除每日轮换引用、将受影响时段替换为雾银，保留自动轮换模式与时间安排。

- **命名主题与时段轮换**：外观设置分为精选主题与我的主题；支持创建、编辑命名、另存和删除主题，将颜色、强度、渐变范围一同保存到本机。旧版自定义槽位自动迁移；每日轮换可选择主题，新增带 24 小时时间带的时段配置弹窗，自定义分界时间和对应主题，保存前校验连续时序，重新激活窗口后按本地时间更新。

### Fixed

- **内置背景主题只读详情**：查看当前暖砂等内置主题时保留原名与完整参数，不再进入未命名新主题；主题卡片通过悬浮遮罩提供查看和编辑，新建收敛到我的主题，主动创建副本后才允许修改。保存后保留详情页并更新地址与成功状态，全局 Header 负责返回。

- **主题编辑页顶部避让**：背景与字幕主题编辑页共用容器增加全局 Header 下方的顶部留白，避免标题、返回和保存操作被导航栏遮挡。

- **桌面字幕状态即时同步**：窗口开关与置顶、穿透偏好通过事件同步，关闭小窗口后 Playbar 立即更新状态；悬浮工具条提供高亮的置顶与双语开关，主窗口和字幕窗口共享双语设置，替代控制面板的定时轮询。

- **桌面字幕位置保持**：修改置顶、鼠标穿透或其他窗口偏好时保留用户拖动后的位置，不再跳回屏幕底部。

- **桌面字幕日语译文漏行**：中文自动折叠结合整首歌词的假名识别，不再把含汉字的日语句子误判为中文；纯汉字且具有不同中文译文的句子也保留双语显示。

- **最近播放歌单封面与歌曲数修复**：修复 `/record/recent/playlist` 接口下发 `coverImgUrl` 时因未兼容该字段导致封面路径拼接为无效 URL 的破损问题；全面兼容 `getPlaylistTrackCount`，并在歌曲总数缺省时优雅展示上次播放歌曲或创建者信息，避免硬编码展示“共 0 首”。

- **推荐歌单播放量显示条件修正**：修正 `GridCard` 对播放量的展示门槛，仅在 `playCount > 0` 时展示播放次数角标，解决部分推荐歌单显示“0次播放”的异常问题。

- **字幕预览场景联动**：修正双语场景仅更换文案、未开启译文导致第二行不显示的问题；场景切换同步设置单行、双语及中文折叠，译文不再等待流式主行结束才出现。场景、文案、重播与查看方式统一移至左侧控制区，右侧仅保留预览，并共享同一份状态。

- **主题名称预览**：修正实时预览中主题名称插值格式，显示内置或用户命名主题的真实名称。

### Visual

- **设置页面标题层级统一**：主题与配色二级页主标题采用设置首页相同的响应式字号与字重。

- **外观二级页标题精简**：移除主题与配色详情标题区下方的分隔线，将“内置 · 只读”收为主标题旁的 Badge。

- **外观控件收紧**：移除重复说明与当前主题操作行，查看入口收进卡片悬浮遮罩；字幕设置 Tabs 按内容宽度排列，精简重复标题，新建配色收进分区标题。外观页不再显示无关的全局保存按钮，副本名称明确标识来源。主题与配色共用卡片交互，内置悬浮查看、自定义悬浮编辑；我的配色使用边框空态，标题移除数量，管理保留在所属区域。

- **字幕配色与显示设置分离**：外层即时调整字体、布局、双语、填色和动画；内置配色与我的配色分区展示，二级页面仅保存完整配色字段。内置配色可只读查看并创建副本，保存后留在页面并反馈状态；旧样式资产迁移为配色，保留当前显示设置。

- **字幕编辑按钮风格统一**：字幕样式与配色的编辑按钮共用网络设置 Ping 按钮的透明底、小圆角、细边框和紧凑间距，并配合小铅笔图标；共用样式定义，保持设置页操作按钮一致。

- **主题编辑操作与入口对齐**：移除编辑页局部返回按钮和退出弹窗，导航由全局 Header 负责；撤销、另存和保存集中到页面底部。字幕样式与配色入口改为左侧标题、右侧编辑按钮的标准设置行，方案管理移到分区标题旁。

- **歌词预览操作归位**：预览场景、主行与译文输入移到歌词预览下方，外观设置与字幕编辑页同步使用集中预览区域；标题栏播放、暂停图标改为实心填充。

- **字幕主题独立编辑页**：新增 `/setting/appearance/subtitle`，完整样式与仅配色共用编辑、保存和预览流程；外观首页仅保留方案选择与编辑入口，详细控件按配色、排版和动画分组，卡片次要操作收进菜单。样句切换仅影响预览，不再修改桌面字幕的双语与折叠设置。

- **背景主题独立编辑页**：新增 `/setting/appearance/background`，把配色、强度与渐变范围移入二级路由，保留原有设置双栏和主题选择、轮换入口；编辑使用独立草稿和真实首页预览，支持撤销、另存、保存应用与返回时放弃修改。

- **字幕预览控件精简**：保留原有双栏布局，主行与译文输入统一设置标签和间距，原文语言自动识别并以文字提示；播放/暂停改为预览标题旁的小图标，重播、循环和显隐收进更多菜单，移除进度条，并让暂停停留在当前动画位置。

- **字幕控制状态与入口尺寸**：放大 Playbar 字幕图标以补偿图形留白，保持开启与弹层展开的高亮；控制面板可直接切换双语，预览场景及播放按钮按实际状态高亮，重播改为普通动作样式，避免误认为已启用功能。

- **桌面字幕悬浮工具条**：移除字幕胶囊向下扩散的阴影；窗口控制贴合实际字幕框右上边缘，仅在悬浮或键盘聚焦时出现，扩大按钮点击区域并保留字幕拖动。

- **艺人热门精选卡片与标题精简**：移除「艺人热门歌曲」分区标题上方冗余的副标题文案；将卡片主标题统一为「{艺人名} 热门精选」；鉴于首页接口无需且无法全量预载每位关注歌手的真实曲目总数，将卡片副标题优化为契合 Spotify 精选风格的「精选最热单曲」，取代死板的歌曲计数。

- **官方排行榜 Hero 轮播大卡片**：将官方排行榜重构为四列 Hero 轮播大卡片，左上角标展示官方榜单图，大卡片在 Top 3 歌曲封面间自动平滑交叉淡入淡出；移除底部冗余试听按钮，突出曲目名、歌手与排行动力，鼠标悬浮时提供左右平滑切歌与整榜一键起播控制。

- **官方排行榜局部设计提案**：基于实际 `/toplist/detail` 返回的四榜摘要制作两列两行概念稿，放大榜单封面并区分歌名与歌手，保留已认可的播客卡片方向；仅更新设计资料。

- **首页排版概念稿**：新增当前首页长截图与 ImageGen 设计提案，统一歌单和新碟网格、收紧活动横幅，并梳理新歌、榜单、播客及歌手的浏览顺序；本次仅产出设计资料，尚未修改首页实现。

- **首页播客推荐取消高度折叠限制**：移除播客推荐列表硬编码的高度截断与底部遮罩，默认完整呈现单页推荐内容；展开时仅呼出分页控件进行翻页切换，与首页其他卡片分区保持清爽一致。

- **字幕预览布局**：移除原尺寸预览及 1440px 缩小画布，直接按可读字号展示并自适应换行；字体、配色与动画设置置于上方，场景文案及播放预览控制统一放在底部，右侧仅显示预览。

- **重置设置行对齐**：背景与歌词样式的恢复默认操作改为标准设置行，左侧显示重置标题，右侧放置恢复默认按钮，与其余控件保持一致。

- **外观与样式设置**：沿用现有设置页等宽双栏，新增雾银、晨雾、浅苔、暖砂、淡紫及自定义渐变，支持强度、范围、固定或每日轮换预设，实时生效并自动保存；首页、播客与收藏页改用统一背景，保留按时段问候语。右侧预览容器通过 children 复用真实首页内容，隔离预览交互。
- **字幕歌词样式预览**：下方提供字体、字号、文字颜色与底色透明度设置，使用 ImageGen 生成的山雾背景展示单行小横条预览并保存样式；独立桌面字幕歌词窗口尚未接入。

- **首页渐变主题选色**：新增独立 HTML 选色预览，提供银灰及低饱和色系的首页场景对照、多选与参数导出，供对照与选择背景主题。
...
- **Web 组件目录架构整理归类**：全面清理 `components/` 根目录下散落的组件文件，按功能职责收敛至独立目录并统一通过 `index.tsx` 总入口装配与导出（包括 `Header`、`MainLayout`、`PlayBar`、`Siderbar`、`player`、`providers`、`shared` 等），直接迁移全站引用路径，消除冗余的根目录代理文件，实现 `components/` 根目录零散落文件。

- **精简桌面小歌词快捷键跳转**：移除桌面小歌词控制面板（`DesktopSubtitleControl`）中冗余的快捷键设置跳转按钮，保留样式设置入口。

- **开启 1.5.3 开发周期**：同步工作区、Web、Docs 和桌面应用版本，后续需求统一记录于此版本。

## v1.5.2

### Visual

- **音乐厅乐签**：使用 ImageGen 生成的独立棉纸材质与 Paper Shaders 纸张纹理，将乐签重构为主票和可撕票根；歌曲、乐评和日期分别采用衬线、宋体与等宽字体。加入鼠标 3D 视差、拖动起翘与松手回弹，拉过阈值后票根落下并关闭弹窗；保留歌曲播放、真实签到数据、会员中心二维码、关闭按钮与 Esc，支持键盘撕票及减少动态效果偏好。

- **头像菜单视觉重构**：按确认稿改为带轻微氛围色的个人菜单，72px 头像与三行用户信息等高对齐，补充明确的个人主页入口；签到区去除嵌套卡片，最近三天使用真实签到状态连接进度线；统一导航图标、行高与右侧箭头，退出登录左对齐并收紧底部留白。保留签到详情、登录退出及桌面/Web 条件入口。

- **侧栏歌单分区**：歌单区域统一组织为「专属音乐」「创建的歌单」「收藏的歌单」三个折叠分区，私人电台与喜欢的音乐归入专属音乐，移除资料库中的重复喜欢入口；保留喜欢歌曲的拖入收藏能力和普通歌单的分组内排序。

- **首页内容布局优化**：推荐卡片改为轻量透明底与更大封面，弱化卡片文字并取消推荐标题中的重复用户名；收紧分区间距、柔化首页氛围背景。快捷入口按两整行、歌单与歌手按一整行折叠，随可用宽度测量高度，移除整行内容上的渐变遮罩，同步加载骨架布局。本阶段保持现有侧栏、Header 和底部播放器结构。

- **歌单多选选中态优化与无感单击**：普通单次点击不再显示任何选中高亮或阴影，仅在使用 Ctrl / Cmd 或 Shift 触发多选时呈现选中态；移除大绿色选中样式，统一沿用行原有 hover 效果并将其固定（`bg-content/10`），保持视觉轻量一致。

- **歌单编辑弹窗视觉体验升级**：弹窗打开时正确呈现歌单既有封面，鼠标悬浮时淡入半透明磨砂“替换封面”遮罩；歌单标签区彻底移除原生粗糙滚动条，改用轻量平滑的 shadcn ScrollArea 组件。

### Added

- **歌单页面编辑入口**：Action 区的 More 按钮支持点击展开菜单，普通自建歌单可直接编辑封面、名称、描述与标签，并提供复制歌单链接；与侧栏共用权限判断和更新逻辑，保存后同步刷新当前歌单与侧栏。我喜欢的音乐、他人歌单和虚拟歌单不提供编辑入口。

- **歌单多选批量拖拽与叠层预览**：歌曲列表支持 `Shift` 区间连选与 `Ctrl` / `Cmd` 自由多选，多选拖拽时呈现多卡片 3D 叠层预览，并在拖拽小窗口左上角常驻圆形数字角标显示已选数量；支持 `Esc` 清空选择。
- **拖拽至「我喜欢的音乐」一键点赞**：侧边栏「我喜欢的音乐」支持作为放置目标（Drop Target），拖拽单首或多选歌曲悬停时红心发光律动，松手一键批量点赞；播放队列弹窗同样支持反向拖出收藏。
- **多选右键菜单扩展与单曲动作限制**：多选状态下支持“播放所选（X 首）”直接将所选歌曲组织为新队列并即刻播放；支持“新建歌单并收纳（X 首）”弹窗快速建单并批量关联，以及批量喜欢/入库/入队；自动限制并隐藏“评论”、“查看歌手”、“复制链接”及单首移出等不可复用操作。
- **Folia 档案歌词模式**：新增独立「档案 / Archive」模式，将歌曲信息、歌词序号、逐词读取和时间码组织为科研档案版式；支持译文或音译、点击歌词跳转、暂停冻结揭示动画、静态与减少动态效果，以及窄屏排版。补齐 Web 与桌面控制器入口、模式持久化及三种语言文案。

- **Folia 莱茵档案背景**：新增独立 Rhine 背景，按需加载 RhineLabUI 档案盒模型与玻璃材质，连接当前歌曲、切歌升降和三种音频律动；暂停保留最后一帧的起伏、升降进度与音频平滑状态，恢复时继续。支持明暗、呼吸、律动强度、遮罩、画质和帧率设置及持久化，不改变现有歌词表现。保留来源与许可说明。

- **歌单与侧栏原位拖拽排序**：自建歌单歌曲表格和侧栏歌单直接使用 dnd-kit 拖拽，松手即时落位并自动调用歌曲/歌单排序 API，失败恢复原顺序；保留完整歌曲 ID、创建/收藏分组与喜欢的音乐固定位置，搜索和按列排序时暂停手动歌曲排序。移除独立排序面板入口，不增加常驻说明。
...
- **侧栏歌单右键权限**：更新与删除仅面向普通自建歌单；喜欢的音乐不显示管理操作，收藏的他人歌单仅提供取消收藏与独立确认文案。执行前重新核对当前账户、所有者和特殊类型，防止菜单打开后状态变化导致误操作；请求处理中阻止重复提交，成功后同步侧栏列表与缓存。

- **私人电台请求与前后顺序**：每次进入电台重新请求歌曲并从新队列首曲开始，隔离换模式、重开电台与后台补歌的请求批次，避免旧响应覆盖新选择；上一首只回溯本轮已播历史，队尾补歌失败时不再循环回首曲，等待补歌期间发生的其他播放操作优先。

- **修复暂停空载时的周期渲染**：播放投影快照字段未变化时复用缓存，不再因每秒心跳生成新对象而通知整组界面订阅者；保留心跳存活检测、真实状态更新与播放位置采样。前台浏览器复测消除了已定位的播放时钟周期长任务，并通过暂停心跳、播放/定位和断连检测 3 项回归测试。

- **减少歌单滚动中的重复挂载**：滚动开始/结束不再切换歌曲行的组件层级，保留右键菜单触发器与拖拽行的实例；稳定歌曲行 DOM ref、排序后的歌曲数组和虚拟列表取 key 函数，避免滚动更新反复解绑拖拽节点及重算测量缓存。右键菜单内容与新建歌单弹窗仅在打开时创建。对 80 首以内的歌单使用原生滚动，较长歌单保留虚拟列表并取消滚动回调中的强制同步提交；隔离歌曲内容与拖拽/右键事件属性，稳定播放回调，菜单查询和状态订阅延迟到打开时初始化。已对每日推荐和 116 首歌单进行前台浏览器采样及交互检查，记录见 [歌单滚动采样](playlist-scroll-performance.md)。同时移除建单弹窗在表格内重复渲染的遮罩，避免非法 DOM 嵌套。

- **补齐歌曲跨歌单拖放**：单曲与多选歌曲可投放到侧栏自建歌单或「我喜欢的音乐」，统一在拖拽结束时提交并刷新缓存；添加前核对目标权限与完整歌曲 ID，跳过重复歌曲，提示全部重复、部分跳过及失败结果，并解析后端嵌套业务状态，避免失败被当作成功。收藏歌单、来源歌单、处理中目标及非接收区域置灰并拒绝投放；允许从只读歌单拖出收藏，同时保留搜索、列排序和权限对原地排序的限制，多选拖回原列表不再误重排单首。取消、失焦与卸载清理拖拽状态，批量喜欢按顺序执行并报告部分失败。

- **固定档案页脚位置**：歌词记录与时间码始终位于底部播放器上方，播放器显隐不再改变页脚高度；矮屏下统一隐藏辅助页脚，避免控制条出现时版式跳动。

- **修复莱茵背景切歌时重新加载**：将莱茵背景挂载到稳定的 Folia 舞台层，歌词壳层只保留背景配置并跳过重复挂载；按歌曲 ID 重建歌词组件或随机切换歌词模式时，继续复用原 Canvas、模型、呼吸相位与切歌展示进度。

- **修复莱茵背景初始化失败**：适配 Three.js 0.185 的 SMAA 纹理字段，避免旧类型声明对应的空字段触发 `reading 'image'` 异常；抗锯齿资源监听缺失时不再阻断背景，初始化中途失败也由渲染链统一释放该通道。

- **隔离公共工具中的浏览器取色依赖**：将图片取色函数移至独立 `lib/web/imageColor.ts`，迁移专辑、歌单、电台和用户资料调用方，按需加载 `colorthief`；根布局与首页使用 `cn` 等公共工具时不再直接引入其 Node `sharp` 分支。

- **修复拖拽落位后滚动闪烁**：落位动画由列表统一记录并仅播放一次，虚拟行滚出后重新进入视口不再重复淡入，也不再重复恢复键盘焦点。

- **修复整项长按拖拽被提前取消**：移除鼠标长按等待期间的 6 像素位移取消条件，避免按住移动后整次拖拽失效；鼠标持续按住 180 毫秒即可启动，触屏保留滑动滚动所需的位移容差。

- **队列落位后恢复序号**：松手后立即显示目标行序号，重新悬停时再显示拖拽手柄；避免鼠标操作后的残留焦点持续触发手柄显示。
