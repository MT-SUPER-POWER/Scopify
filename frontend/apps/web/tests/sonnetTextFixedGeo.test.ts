...
import { resolveSonnetTextFixedGeoPlan } from "@/components/lyrics/folia/src/components/visualizer/sonnet/sonnetTextFixedGeo";
...
describe("Sonnet text fixed geometry", () => {
  it("preserves the normal 50/50 hollow and solid category split", () => {
    const plans = Array.from({ length: 400 }, (_, seed) =>
      resolveSonnetTextFixedGeoPlan(seed, false),
    );
  ...
  it("preserves the chorus 90/10 hollow and solid category split", () => {
    const plans = Array.from({ length: 300 }, (_, seed) =>
      resolveSonnetTextFixedGeoPlan(seed, true),
    );
  ...
  it("distributes hollow geometry across the two frames and two new wireframe designs", () => {
    ...
    expect(variants).toEqual(
      new Set(["straight-frame", "rotated-frame", "orbit-crosshair", "split-arches"]),
    );
  });
});
