      ...
      {hotspots.map((hotspot) => {
        ...
        return (
          <button
            ...
            onMouseLeave={() =>
              setVisibleSection((previous) => (previous === hotspot.section ? null : previous))
            }
            onFocus={() => setVisibleSection(hotspot.section)}
            onBlur={() =>
              setVisibleSection((previous) => (previous === hotspot.section ? null : previous))
            }
            className={`group pointer-events-auto absolute rounded-[24px] border text-left transition-all duration-200 outline-none ${hotspot.className}`}
            style={{
              borderColor,
              backgroundColor:
                isVisible && isActive ? colorWithAlpha(theme.accentColor, 0.08) : "transparent",
              boxShadow:
                isVisible && isActive
