...
class AnimatedGraphics {
  ...
  set rotation(v: number) {
    this.display.rotation = v;
  }

  get mask() {
    return this.display.mask;
  }
  set mask(v: any) {
    this.display.mask = v;
  }
  ...
  bezierCurveTo(c1x: number, c1y: number, c2x: number, c2y: number, tx: number, ty: number) {
    const len =
      Math.hypot(c1x - this.lastX, c1y - this.lastY) +
        ...
        if (!isRectWipe) {
          for (const p of cmd.path) {
            ...
            if (p.type === "moveTo") this.display.moveTo(p.x, p.y);
              ...
              this.display.arc(p.cx, p.cy, p.r, p.start, p.end, p.anticlockwise);
            else if (p.type === "quadraticCurveTo")
              this.display.quadraticCurveTo(p.cx, p.cy, p.tx, p.ty);
            else if (p.type === "bezierCurveTo")
              this.display.bezierCurveTo(p.c1x, p.c1y, p.c2x, p.c2y, p.tx, p.ty);
          }
        }
          ...
          if (p.type === "moveTo") {
            ...
            if (currentLen + p.len <= targetLen) {
              ...
              if (p.type === "lineTo") {
                ...
                const newTx =
                  (1 - ratio) * (1 - ratio) * p.lastX +
                  2 * (1 - ratio) * ratio * p.cx +
                  ratio * ratio * p.tx;
                const newTy =
                  (1 - ratio) * (1 - ratio) * p.lastY +
...
export const buildSonnetShotMg = (
  ...
  const drawHatching = (
    ...
    h: number,
    spacing = 8,
    target: import("pixi.js").Container = bg.display,
  ) => {
    ...
    for (let i = -w; i < w + h; i += spacing) {
      hatch
        .moveTo(x + i, y)
        .lineTo(x + i + h, y + h)
        .stroke({ color: primary, width: 1, alpha: 0.15 });
    }
  ...
  bg.moveTo(-hw + marginX + 20, barY)
    .lineTo(hw - marginX - 20, barY)
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    if (type === 3 && hasIcons && iconTextureIndex !== null) {
      ...
      if (type === 0 || (type === 3 && !hasIcons)) {
        g.rect(-pSize / 2, -pSize / 2, pSize, pSize).fill({
