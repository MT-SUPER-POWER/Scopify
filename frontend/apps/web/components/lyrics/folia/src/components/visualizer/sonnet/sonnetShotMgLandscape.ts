import {
  ...
  strokePolygon,
  type SonnetPoint,
  type SonnetThemedShotMgOptions,
} from "./sonnetThemedShotMgPrimitives";
...
export const drawSonnetTerracesMg = ({
  target,
  radius,
  width,
  height,
  ...
  for (let band = 0; band < 7; band += 1) {
    ...
    target
      ...
      .stroke({
        color: band % 2 ? secondary : primary,
        width: band % 3 === 0 ? 2.5 : 1,
        alpha: 0.34 + band * 0.04,
      });
  }
};

export const drawSonnetMountainLakeMg = ({
  target,
  radius,
  width,
  height,
  ...
  const back: SonnetPoint[] = [
    [-bleed.x, radius * 0.1],
    [-radius * 0.42, -radius * 0.46],
    [-radius * 0.14, -radius * 0.16],
    [radius * 0.22, -radius * 0.62],
    [bleed.x, radius * 0.1],
  ];
  const front: SonnetPoint[] = [
    [-bleed.x, radius * 0.22],
    [-radius * 0.28 + shift, -radius * 0.2],
    [radius * 0.06, radius * 0.05],
    [radius * 0.48 + shift, -radius * 0.28],
    [bleed.x, radius * 0.22],
  ];
  ...
  for (let line = 0; line < 7; line += 1) {
    ...
    target
      .moveTo(-bleed.x + inset, y)
      .lineTo(bleed.x - inset, y)
      .stroke({ color: line % 2 ? secondary : primary, width: 1, alpha: 0.2 + line * 0.035 });
  }
  target
    .circle(-radius * 0.48, -radius * 0.48, radius * 0.1)
    .fill({ color: secondary, alpha: 0.14 });
  target
    .circle(-radius * 0.48, -radius * 0.48, radius * 0.13)
    .stroke({ color: secondary, width: 2, alpha: 0.5 });
};

export const drawSonnetCoastalCliffMg = ({
  target,
  radius,
  width,
  height,
  ...
  const cliff: SonnetPoint[] = [
    [-bleed.x * direction, bleed.y],
    [-radius * 0.78 * direction, -radius * 0.1],
    [-radius * 0.5 * direction, -radius * 0.34],
    [-radius * 0.18 * direction, radius * 0.08],
  ...
  target
    .rect(towerX - radius * 0.09, -radius * 0.42, radius * 0.18, radius * 0.45)
    .fill({ color: secondary, alpha: 0.12 });
  target
    .rect(towerX - radius * 0.09, -radius * 0.42, radius * 0.18, radius * 0.45)
    .stroke({ color: secondary, width: 2, alpha: 0.7 });
  target
    .moveTo(towerX - radius * 0.14, -radius * 0.42)
    .lineTo(towerX, -radius * 0.56)
  ...
  for (let wave = 0; wave < 6; wave += 1) {
    ...
    target
      .moveTo(-radius * 0.05 * direction, y)
      .quadraticCurveTo(radius * 0.35 * direction, y - radius * 0.08, bleed.x * direction, y)
      .stroke({
        color: wave % 2 ? secondary : primary,
        width: wave % 3 === 0 ? 2 : 1,
        alpha: 0.26 + wave * 0.045,
      });
  }
};
