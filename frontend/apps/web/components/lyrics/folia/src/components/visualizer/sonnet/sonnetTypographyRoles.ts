...
export const findSonnetHeroSegmentIndex = (segments: SonnetSemanticSegment[]) => {
...
export const findSonnetSemiHeroSegmentIndices = (
  ...
  const wordLikeCount = segments.filter(
    (segment) => segment.isWordLike && getSonnetVisibleSegmentLength(segment) > 0,
  ).length;
  ...
  const bestOf = (list: typeof candidates) =>
  ...
  if (wordLikeCount >= SEMI_HERO_MULTI_WORD_COUNT && primary) {
    const secondary = bestOf(
      secondarySide.filter(({ index }) => Math.abs(index - primary.index) >= SEMI_HERO_MIN_GAP),
    );
