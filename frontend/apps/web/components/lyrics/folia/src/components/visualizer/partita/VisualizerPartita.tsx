...
const clampPartitaStagger = (value: number, fallback: number) =>
  Number.isFinite(value) ? clamp(value, 0, 180) : fallback;

const resolvePartitaTuning = (tuning?: PartitaTuning): PartitaTuning => {
  const rawMin = clampPartitaStagger(
    tuning?.staggerMin ?? DEFAULT_PARTITA_TUNING.staggerMin,
    DEFAULT_PARTITA_TUNING.staggerMin,
  );
  const rawMax = clampPartitaStagger(
    tuning?.staggerMax ?? DEFAULT_PARTITA_TUNING.staggerMax,
    DEFAULT_PARTITA_TUNING.staggerMax,
  );
...
const resolvePartitaLineRenderProfile = (
  line: Line | null | undefined,
): PartitaLineRenderProfile | null => {
  if (!line) {
    return null;
  }
...
const getPartitaWordDisplayDuration = (word: WordType, renderProfile: PartitaLineRenderProfile) => {
  ...
  const minDuration =
    renderProfile.wordRevealMode === "instant"
      ? 0.08
...
const buildSequentialColumns = (
  ...
  const totalGraphemes = Math.max(
    splitGraphemes(line.fullText.replace(/\s+/g, "")).length,
    line.words.length,
    1,
  );
...
const PartitaWord: React.FC<{
  ...
  return (
    <motion.div
      ...
      initial="waiting"
      animate={status}
      className="relative inline-block origin-center font-bold whitespace-nowrap will-change-transform"
      style={{
        fontSize,
        lineHeight: 1.22,
        marginRight: "0.8rem",
...
const PartitaChunk: React.FC<{
  ...
  const chunkEndTime = getPartitaWordActiveEndTime(
...
const VisualizerPartita: React.FC<VisualizerPartitaProps> = (props) => {
  ...
  const sequentialLayout = useMemo(() => {
    ...
    return getOrBuildPartitaLayout(
      layoutCacheRef.current,
      activeLine,
      theme,
      windowHeight,
      resolvedPartitaTuning,
    );
  ...
  const bodyVariants: Variants = {
    ...
    passed: ({ baseColor, wordRevealMode }: any) => ({
      ...
      transition: {
        ...
        filter: {
          duration: wordRevealMode === "instant" ? 0.12 : wordRevealMode === "fast" ? 0.2 : 0.5,
        },
      },
      transitionEnd: {
        filter: "none",
