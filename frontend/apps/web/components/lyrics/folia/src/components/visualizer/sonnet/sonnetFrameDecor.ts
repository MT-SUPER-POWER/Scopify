...
export const resolveSonnetFrameDecorSpec = (
  segment: SonnetSemanticSegment,
): SonnetFrameDecorSpec => {
  const hash = hashSonnetSeed(
    [segment.text, segment.startOffset, segment.endOffset, "frame-decor"].join(":"),
  );
  return {
...
export const resolveSonnetFrameLocalDimensions = (
  ...
  const isOddQuarterTurn =
    Math.abs(placement.rotation - snappedRotation) < 1e-6 && Math.abs(quarterTurns % 2) === 1;
