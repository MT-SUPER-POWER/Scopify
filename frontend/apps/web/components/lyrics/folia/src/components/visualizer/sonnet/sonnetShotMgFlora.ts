...
export const drawSonnetCamelliaMg = ({
  target,
  radius,
  seed,
  primary,
  ...
  for (let ring = 0; ring < 3; ring += 1) {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      drawPetal(
        ...
        0,
        0,
        radius * (0.28 + ring * 0.15),
        radius * (0.075 + ring * 0.018),
        angle,
...
export const drawSonnetTulipFieldMg = ({
  target,
  radius,
  seed,
  primary,
  ...
  for (let index = 0; index < 7; index += 1) {
    ...
    target
      .moveTo(x, bottom)
      .bezierCurveTo(
        x + radius * 0.04 * direction,
        radius * 0.28,
        x - radius * 0.05 * direction,
        top + radius * 0.12,
        x,
        top,
      )
...
export const drawSonnetWildflowerMg = ({
  target,
  radius,
  seed,
  primary,
  ...
  for (let stem = 0; stem < 9; stem += 1) {
    ...
    target
      .moveTo(x, radius * 0.72)
      .quadraticCurveTo(x - lean, radius * 0.12, x + lean, flowerY)
