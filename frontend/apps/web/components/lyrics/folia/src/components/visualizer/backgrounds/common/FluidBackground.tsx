...
const buildSoftFocusCover = (coverUrl: string): Promise<string | null> =>
  new Promise((resolve) => {
...
const buildDownscaledCover = (coverUrl: string, maxSize: number): Promise<string | null> =>
  new Promise((resolve) => {
    if (typeof document === "undefined") {
      resolve(null);
      return;
...
const FluidBackground: React.FC<FluidBackgroundProps> = memo(
  function FluidBackground({ coverUrl, theme }) {
    ...
    const [coverLayers, setCoverLayers] = useState<
      Array<{ url: string; sourceCover: string; key: number }>
    >([]);
    ...
    useEffect(() => {
      ...
      void buildDownscaledCover(coverUrl, COVER_BLUR_SOURCE_MAX).then((smallUrl) => {
        ...
        setCoverLayers((prev) => {
          ...
          return [
            ...(top ? [top] : []),
            { url: src, sourceCover: coverUrl, key: coverKeyRef.current },
          ];
        });
      });
      return () => {
    ...
    return (
      ...
      >
        ...
        {coverUrl ? (
                  ...
                  <motion.div
                    ...
                    onAnimationComplete={() => {
                      ...
                      if (isTop && ready) {
                        ...
                        setReadyCoverKeys((prev) =>
                          prev.has(layer.key) ? new Set([layer.key]) : prev,
                        );
                      }
                    }}
                  >
                    <img
                      ...
                      onError={() => {
                        ...
                        setCoverLayers((prev) =>
                          prev.length > 1 ? prev.filter((l) => l.key !== layer.key) : prev,
                        );
                      }}
                    />
                  </motion.div>
          ...
          <div
            className="absolute inset-0 size-full transition-colors duration-1000"
            style={{
              backgroundColor: theme.backgroundColor,
              opacity: 0.8,
            }}
