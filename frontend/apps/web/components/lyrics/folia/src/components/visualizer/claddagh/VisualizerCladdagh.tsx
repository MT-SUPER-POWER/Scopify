...
const getFractionalActiveIndex = (
  ...
  if (timeline.length === 1) {
    ...
    const targetEnd =
      typeof renderEnd === "number" && Number.isFinite(renderEnd) ? renderEnd : item.endTime;
...
const measureCladdaghTextWidth = (
  text: string,
  fontSpec: string,
  fontPx: number,
...
const measureCladdaghGraphemeOffsets = (
  graphemes: string[],
  fontSpec: string,
  fontPx: number,
  letterSpacingOffsetPx: number = 0,
): number[] => {
...
const buildMeasuredSpacingInfo = <T extends { char: string }>(
  ...
  const offsets = measureCladdaghGraphemeOffsets(
    graphemes,
    fontSpec,
    fontPx,
    letterSpacingOffsetPx,
  );
  ...
  return items.map((item, index) => {
    const centerPx =
      (((offsets[index] ?? 0) + (offsets[index + 1] ?? offsets[index] ?? 0)) / 2) *
      safeSpacingScale;
...
const RingLine: React.FC<RingLineProps> = ({
  ...
  const spacingInfo = useMemo(() => {
    ...
    return buildMeasuredSpacingInfo(
      data,
      fontSpec,
      baseFontSize,
      Rx,
  ...
  useLayoutEffect(() => {
    const handler = (latestTime: number) => {
      ...
      const upcomingEntryProgress = isUpcomingLine
        ? clamp(
            (latestTime - (line.startTime - CLADDAGH_NEXT_LINE_ENTRY_LEAD_SECONDS)) /
      ...
      const activeRenderEnd = activeLine
        ? Math.max(
            ...
            activeNextLine
              ? activeNextLine.startTime - CLADDAGH_NEXT_LINE_ENTRY_LEAD_SECONDS
              : activeLine.endTime + 10.0,
          )
        : undefined;
      ...
      const activeLineProgress = getLinePlaybackProgress(
        activeSpacingInfo,
        latestTime,
        activeRenderEnd,
      );
      const backOrbitFollow =
        Math.PI * CLADDAGH_BACK_ORBIT_FOLLOW_RATIO * (1 - Math.pow(1 - activeLineProgress, 1.35));
      ...
      if (lineIndex < centerLineIndex) {
        ...
        wordOffset +=
          (activeWordOffset * CLADDAGH_BACK_FOLLOW_RATIO + backOrbitFollow) * backFollowFactor;
      }
      ...
      for (let i = 0; i < mvsLength; i++) {
        ...
        const tangentAngle = normalizeReadableAngle(
          (Math.atan2(rotatedTangentY, rotatedTangentX) * 180) / Math.PI,
        );
        ...
        const scale =
          (0.22 + 0.98 * Math.pow(D, 1.5)) *
          (1.0 + (focusScaleRatio ?? 0.65) * F) *
          lengthScaleFactor *
          ((item as any).scaleFactor ?? 1.0);
        ...
        if (currentGlowRadius > 0.5 && shadowFade > 0.01) {
          ...
          const fadedTargetColor = mixColors(
            targetColor,
            targetColor,
            0,
            currentAlpha * shadowFade,
          );
          ...
          if (line.isChorus) {
            ...
            const innerGlowColor = mixColors(
              targetColor,
...
const VisualizerCladdagh: React.FC<VisualizerSharedProps> = (props) => {
  ...
  const Rx =
    (dimensions.width > 0 ? Math.min(dimensions.width * 0.44, 560) : 360) *
    claddaghTuning.radiusScale;
  const Ry = Rx > 0 ? Rx * 0.707 : 254; // 45-degree angle projection ratio
  const focusSpacingScale =
    (1 + claddaghTuning.focusScaleRatio) / (1 + DEFAULT_CLADDAGH_TUNING.focusScaleRatio);
  ...
  const activeSpacingInfo = useMemo(() => {
    ...
    return buildMeasuredSpacingInfo(
      ...
      baseFontSize,
      Rx,
      activeTextSpacingScale,
      claddaghTuning.letterSpacingOffset,
    );
  }, [
    lines,
    renderBaseIndex,
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative flex size-full flex-col items-center justify-center overflow-hidden select-none"
      >
