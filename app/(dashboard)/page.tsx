...
export default function HomePage() {
  ...
  const {
    playlists, bannerPlaylist, suggestedArtists, isLoading, loadingPlayId, hasError,
    dateInfo, userName, handlePlayPlaylist, handlePlayAlbum, fetchHomeData,
  } = useHomeData();

  return (
    <div className="relative pb-24 font-sans">
      <div className={cn("absolute top-0 left-0 right-0 bg-linear-to-b", getTimeTheme().gradient, "z-0 pointer-events-none")} />
      ...
      <div className="relative z-10 p-6 pt-20 space-y-8">
        ...
        <section>
          <CollapsibleSection
            title={
              <div className="flex items-center gap-4">
                <h1 className="text-3xl font-bold text-white tracking-tight leading-none">{t(getTimeTheme().greetingKey)}</h1>
                {(hasError || !isLoading) && (
                  <button type="button" onClick={() => fetchHomeData()} disabled={isLoading}
          ...
          >
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
              <div onClick={() => smartRouter.push("/playlist/?isDailyRecommend=true")}
                ...
                <div className="h-16 w-16 shadow-[2px_0_8px_rgba(0,0,0,0.4)] z-10 shrink-0 flex flex-col rounded-l-md overflow-hidden bg-white select-none">
                  <div className="h-5.5 bg-linear-to-b from-[#e34242] to-[#c42b2b] flex items-center justify-center border-b border-black/10">
                    <span className="text-white text-[10px] font-medium tracking-[0.15em]">{dateInfo.dayOfWeek}</span>
                  </div>
                  <div className="flex-1 relative flex items-center justify-center bg-linear-to-b from-momo-light from-50% to-[#f0f0f0] to-50%">
                    ...
                    <span className="text-3xl font-black text-[#2a2a2a] leading-none z-10 mt-1 tracking-tighter">{dateInfo.dateNum}</span>
                  </div>
                </div>
                <span className="text-white font-bold text-sm ml-4 truncate">{t("home.dailyRecommendations")}</span>
                <button type="button" onClick={(e) => { e.stopPropagation(); smartRouter.push("/playlist/?isDailyRecommend=true"); }}
              ...
              {bannerPlaylist.map((item) => (
                <div key={item.id} onClick={() => smartRouter.push(`/playlist/?id=${item.id}&isRecommend=true`)}
                  className="flex items-center h-16 bg-white/10 hover:bg-white/20 transition-colors rounded-md overflow-hidden group cursor-pointer relative pr-4">
                  <Image width={64} height={64} src={item.picUrl} alt={t("playlist.form.coverAlt")}
                  ...
                  <button type="button" onClick={(e) => handlePlayPlaylist(item.id, e)}
        ...
        {/* 推荐歌单 */}
        <section>
          <CollapsibleSection title={<h2 className="text-2xl font-bold text-white hover:underline tracking-tight">{t("home.madeFor", { name: userName ?? t("home.you") })}</h2>} collapsedHeight="280px">
            {isLoading ? (
              <div className="text-zinc-400 text-sm">{t("home.loadingRecommendations")}</div>
            ) : (
              <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                {playlists.map((playlist) => (
                  <GridCard key={playlist.id} id={playlist.id} name={playlist.name}
                    coverUrl={`${playlist.picUrl}?param=300y300`}
                    subtitle={t("home.playlistSummary", { plays: formatPlayCount(playlist.playCount), count: playlist.trackCount })}
        ...
        {/* 推荐歌手 */}
        <section>
          <CollapsibleSection title={<h2 className="text-2xl font-bold text-white hover:underline tracking-tight">{t("home.suggestedArtists")}</h2>} collapsedHeight="260px">
            <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
              {suggestedArtists.map((artist) => (
                <GridCard key={artist.id} id={artist.id} name={artist.name}
                  coverUrl={`${artist.picUrl}?param=200y200`} isArtist
        ...
        {collectedAlbum.length > 0 && (
          <section>
            <CollapsibleSection title={<h2 className="text-2xl font-bold text-white hover:underline tracking-tight">{t("home.yourCollectedAlbums")}</h2>} collapsedHeight="280px">
              <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                {collectedAlbum.map((item: NeteaseUserAlbum) => (
                  <GridCard key={item.id} id={item.id} name={item.name} coverUrl={item.picUrl}
