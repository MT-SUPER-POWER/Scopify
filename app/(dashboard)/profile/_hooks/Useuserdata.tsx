...
import { getUserDetailInfo, getUserPlaylists, getRecentSongsByID, getRecentPlaylists } from "@/lib/api/user";
import { pruneUser, NeteaseUser } from "@/types/api/user";
...
export function useUserData(uid: string | null) {
  ...
  useEffect(() => {
    if (!uid) return;
    Promise.resolve().then(() => { setIsLoading(true); }); // 确保在当前事件循环结束后才显示加载状态，避免闪烁
    ...
    Promise.allSettled([...baseRequests, ...selfRequests]).then((results) => {
