...
export default function ProfilePage() {
  ...
  if (!uid)
    return <div className="p-8 text-white h-full bg-[#121212]">{t("profile.page.invalidUserId")}</div>;
  ...
  return (
    <div className="relative w-full min-h-screen flex flex-col bg-[#121212] font-sans text-white pb-24">
      ...
      <div className="flex-1 relative z-10 flex flex-col bg-linear-to-b from-black/20 via-[#121212] to-[#121212] via-20%">
        <UserActionBar isSelf={isSelf} onEdit={() => void requireLoginAction("profile-edit", () => setEditOpen(true))} />
        {isSelf && (
        ...
        {isSelf && (
          <div className="px-6 mt-10">
            <h2 className="text-2xl font-bold mb-6 hover:underline cursor-pointer">{t("profile.page.recentPlaylists")}</h2>
            {recentPlaylists.length > 0
              ? <PublicPlaylistGrid playlists={recentPlaylists} onClickPlaylist={(id) => router.push(`/playlist?id=${id}`)} />
              : <div className="text-gray-400 text-sm">{t("profile.page.noRecentPlaylists")}</div>}
          </div>
        )}
        <div className="px-6 mt-10 mb-20">
          <button type="button" onClick={() => setPlaylistsOpen((v) => !v)} className="flex items-center gap-2 mb-6 group">
            <h2 className="text-2xl font-bold group-hover:underline">{t("profile.page.publicPlaylists")}</h2>
            <ChevronUp className={cn("w-5 h-5 text-zinc-400 transition-transform duration-200", playlistsOpen && "rotate-180")} />
          </button>
          {playlistsOpen && <PublicPlaylistGrid playlists={playlists} onClickPlaylist={(id) => router.push(`/playlist?id=${id}`)} />}
        </div>
      </div>
      <EditUserProfileDialog open={editOpen} user={displayUser} saving={savingProfile} onCancel={() => setEditOpen(false)} onConfirm={handleSaveProfile} />
    </div>
  );
}
