...
} from "@/components/lyrics/folia/src/components/visualizer/sonnet/sonnetSpatialMgGeometry";
import {
  drawThemedSonnetShotMg,
  SONNET_THEMED_GEO_VARIANT_COUNT,
  SONNET_THEMED_GEO_VARIANT_START,
  SONNET_THEMED_GEO_VARIANTS,
} from "@/components/lyrics/folia/src/components/visualizer/sonnet/sonnetThemedShotMg";
import { drawAdditionalSonnetShotMg } from "@/components/lyrics/folia/src/components/visualizer/sonnet/sonnetAdditionalShotMg";
import {
  drawOpenSonnetShotMg,
  SONNET_OPEN_GEO_VARIANT_COUNT,
  SONNET_OPEN_GEO_VARIANT_START,
  SONNET_OPEN_GEO_VARIANTS,
} from "@/components/lyrics/folia/src/components/visualizer/sonnet/sonnetOpenFrameShotMg";
...
describe("Sonnet spatial MG variants", () => {
  ...
  it("distributes variant 3 across all molecule sub-variants", () => {
    expect(
      [0, 1, 2].map((cycle) => resolveSonnetMoleculeVariant(3 + cycle * SONNET_GEO_VARIANT_COUNT)),
    ).toEqual([0, 1, 2]);
  });
  ...
  it("distributes variant 8 across all quarter-turn rotations", () => {
    expect(
      [0, 1, 2, 3].map((cycle) =>
        resolveSonnetHudRotationQuarterTurns(8 + cycle * SONNET_GEO_VARIANT_COUNT),
      ),
    ).toEqual([0, 1, 2, 3]);
  });
  ...
  it("reserves twelve themed variants after the existing collection", () => {
    ...
    expect(SONNET_OPEN_GEO_VARIANT_START).toBe(
      SONNET_THEMED_GEO_VARIANT_START + SONNET_THEMED_GEO_VARIANT_COUNT,
    );
  });
  ...
  it("overscans variants with exposed rectangular or landscape edges", () => {
    ...
    for (const variant of [20, 22, 23, 30, 31, 32, 33, 34, 35]) {
      ...
      const target = {
        ...
        quadraticCurveTo: (cx: number, _cy: number, tx: number) => {
          xs.push(cx, tx);
          return target;
        },
        bezierCurveTo: (c1x: number, _c1y: number, c2x: number, _c2y: number, tx: number) => {
          xs.push(c1x, c2x, tx);
          return target;
        },
        arc: (cx: number) => {
