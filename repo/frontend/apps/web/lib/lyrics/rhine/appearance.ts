...
import { glassRevealGLSL, frostedTransmissionGLSL, FROSTED_ROUGHNESS } from "./glassReveal";
import { internalOpticsFragment } from "./internalOptics";
import { themeMaterial } from "./themeMaterial";

import type { Surface, Palette } from "@/types/rhineRuntime";
...
export class CardAppearance {
  ...
  private sources = new Set<Surface>();
  disposeSources() {
    for (const material of this.sources) material.dispose();
    this.sources.clear();
    this.palettes.clear();
  }

  register(name: string, high: Surface, low?: Surface) {
    this.sources.add(high);
    if (low) this.sources.add(low);
