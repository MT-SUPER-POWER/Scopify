...
const addSeam = (ctx: TemperaCompositionContext, polygon: number[], delay: number) => {
  ctx.add(drawPolygonFill(ctx.pixi, polygon, ctx.palette.ink, 0.85, ctx.gradient), {
    delay,
    span: 0.5,
  });
};
...
const duoSplit: TemperaCompositionDrawer = (ctx) => {
  ...
  addPanels(
    ...
    horizontal
      ? [
          {
            polygon: rectPolygon(-bleed, -bleed, width + bleed * 2, height * 0.52 + bleed),
            tone: palette.tone1,
            enterDX: 0,
            enterDY: -height * 0.55,
          },
          {
            polygon: rectPolygon(-bleed, height * 0.52, width + bleed * 2, height * 0.48 + bleed),
            tone: palette.tone3,
            enterDX: 0,
            enterDY: height * 0.55,
          },
      ...
      : [
          {
            polygon: rectPolygon(-bleed, -bleed, width * 0.5 + bleed, height + bleed * 2),
            tone: palette.tone1,
            enterDX: -width * 0.5,
            enterDY: 0,
          },
          {
            polygon: rectPolygon(width * 0.5, -bleed, width * 0.5 + bleed, height + bleed * 2),
            tone: palette.tone3,
            enterDX: width * 0.5,
            enterDY: 0,
          },
...
const quadSplit: TemperaCompositionDrawer = (ctx) => {
  ...
  addPanels(
    ...
    [
      {
        polygon: rectPolygon(-bleed, -bleed, splitX + bleed, splitY + bleed),
        tone: palette.tone1,
        enterDX: -width * 0.3,
        enterDY: -height * 0.3,
      },
      {
        polygon: rectPolygon(splitX, -bleed, width - splitX + bleed, splitY + bleed),
        tone: palette.tone4,
        enterDX: width * 0.3,
        enterDY: -height * 0.3,
      },
      {
        polygon: rectPolygon(-bleed, splitY, splitX + bleed, height - splitY + bleed),
        tone: palette.tone3,
        enterDX: -width * 0.3,
        enterDY: height * 0.3,
      },
      {
        polygon: rectPolygon(splitX, splitY, width - splitX + bleed, height - splitY + bleed),
        tone: palette.tone2,
        enterDX: width * 0.3,
        enterDY: height * 0.3,
      },
...
const triColumn: TemperaCompositionDrawer = (ctx) => {
  ...
  addPanels(
    ...
    [
      {
        polygon: rectPolygon(-bleed, -bleed, edge + bleed, height + bleed * 2),
        tone: palette.tone1,
        enterDX: -width * 0.25,
        enterDY: 0,
      },
      {
        polygon: rectPolygon(edge, -bleed, width - edge * 2, height + bleed * 2),
        tone: palette.tone4,
        enterDX: 0,
        enterDY: -height * 0.3,
      },
      {
        polygon: rectPolygon(width - edge, -bleed, edge + bleed, height + bleed * 2),
        tone: palette.tone1,
        enterDX: width * 0.25,
        enterDY: 0,
      },
...
const thirdsStack: TemperaCompositionDrawer = (ctx) => {
  ...
  addPanels(
    ...
    [
      {
        polygon: rectPolygon(-bleed, -bleed, width + bleed * 2, band + bleed),
        tone: palette.tone2,
        enterDX: -width * 0.2,
        enterDY: 0,
      },
      {
        polygon: rectPolygon(-bleed, band, width + bleed * 2, band),
        tone: palette.tone4,
        enterDX: width * 0.2,
        enterDY: 0,
      },
      {
        polygon: rectPolygon(-bleed, band * 2, width + bleed * 2, height - band * 2 + bleed),
        tone: palette.tone1,
        enterDX: -width * 0.2,
        enterDY: 0,
      },
...
const cornerWedge: TemperaCompositionDrawer = (ctx) => {
  ...
  addPanels(
    ...
    [
      {
        polygon: rectPolygon(-bleed, -bleed, width + bleed * 2, height + bleed * 2),
        tone: palette.tone1,
        enterDX: 0,
        enterDY: 0,
      },
...
const crossAxis: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon((width - barX) / 2, -bleed, barX, height + bleed * 2),
      palette.tone4,
      0.95,
    ),
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(-bleed, (height - barY) / 2, width + bleed * 2, barY),
      palette.tone3,
      0.95,
    ),
...
const offsetHalves: TemperaCompositionDrawer = (ctx) => {
  ...
  addPanels(
    ...
    [
      {
        polygon: rectPolygon(-bleed, -bleed, seam + bleed, height * 0.5 + step + bleed),
        tone: palette.tone1,
        enterDX: -width * 0.3,
        enterDY: 0,
      },
      {
        polygon: rectPolygon(seam, -bleed, width - seam + bleed, height * 0.5 - step + bleed),
        tone: palette.tone3,
        enterDX: width * 0.3,
        enterDY: 0,
      },
...
const stairBlocks: TemperaCompositionDrawer = (ctx) => {
  ...
  tones.forEach((tone, index) => {
    ...
    ctx.add(
      drawPolygonFill(
        ctx.pixi,
        rectPolygon(left, top, width * 0.3, height * 0.34),
        tone,
        0.94,
        ctx.gradient,
      ),
...
const pillarGap: TemperaCompositionDrawer = (ctx) => {
  ...
  addPanels(
    ...
    [
      {
        polygon: rectPolygon(-bleed, -bleed, side + bleed, height + bleed * 2),
        tone: palette.tone4,
        enterDX: -width * 0.3,
        enterDY: 0,
      },
      {
        polygon: rectPolygon(width - side, -bleed, side + bleed, height + bleed * 2),
        tone: palette.tone4,
        enterDX: width * 0.3,
        enterDY: 0,
      },
...
export const TEMPERA_SPLIT_COMPOSITIONS: Partial<
