      ...
      [0, 1].forEach((index) => {
        ...
        ctx.add(
          drawHatchFill(
            ctx.pixi,
            box,
            index === 0 ? spec : { ...spec, spacing: spec.spacing * 0.55 },
            palette.tone4,
            0.75,
          ),
          { delay: 0.34 + index * 0.05, grow: true },
        );
        ctx.add(drawPolygonOutline(ctx.pixi, box, palette.tone4, 1.2, 0.6), {
    ...
    case "poster-diamond":
      ...
      ctx.add(
        drawPolygonFill(
          ctx.pixi,
          diamondPolygon(
            cornerX < width / 2 ? -width * 0.04 : width * 1.04,
            cornerY,
            width * 0.14,
            height * 0.2,
          ),
