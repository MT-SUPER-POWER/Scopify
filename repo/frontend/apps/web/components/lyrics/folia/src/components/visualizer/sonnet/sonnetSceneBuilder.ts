...
export const shouldDrawSonnetSceneBackdrop = (
...
export const buildSonnetScene = (
    ...
    if (shouldDrawSonnetSceneBackdrop(showBackgroundMg, options.transparentBackground)) {
      sceneBackgroundLayer.addChild(
        new Graphics()
          ...
          .fill({ color: colorNumber(pixi, options.theme.backgroundColor), alpha: 0.1 }),
      );
    }
      ...
      sceneBackgroundLayer.addChild(
        new Graphics()
          ...
          .stroke({
            color: colorNumber(
              pixi,
              index % 2 ? options.theme.secondaryColor : options.theme.accentColor,
            ),
            width: index % 3 === 0 ? 2 : 1,
  ...
  const shots = paragraph.shots.map((shot, shotIndex) => {
    ...
    const fontSize = Math.max(
      24,
      Math.min(112, (width / Math.max(7, wordCount * 2.15)) * heroScale * options.lyricsFontScale),
    );
  ...
  const transitionGlitchEffect =
    options.tuning.enableTransitions && !options.staticMode ? createSonnetGlitchEffect(pixi) : null;
