...
import { drawHatchFill, drawLines, drawPolygonFill, drawRings } from "../temperaShapes";
...
const irisHole: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(-ctx.bleed, cy - unit * 0.035, cx - radius * 0.7 + ctx.bleed, unit * 0.07),
      palette.tone4,
      0.9,
      ctx.gradient,
    ),
  ...
  ctx.add(drawRings(ctx.pixi, [{ x: cx, y: cy, radius: radius * 1.14 }], palette.tone4, 1.4, 0.6), {
    delay: 0.2,
    span: 0.5,
    drift: true,
  });
...
const slotRail: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(width * 0.66, top - height * 0.06, width * 0.12, height * 0.28),
      palette.tone2,
      0.95,
      ctx.gradient,
    ),
  ...
  ctx.add(
    drawLines(
      ctx.pixi,
      [{ x1: -bleed, y1: top + height * 0.22, x2: width + bleed, y2: top + height * 0.22 }],
      palette.tone2,
      1.4,
      0.55,
    ),
...
const punchRow: TemperaCompositionDrawer = (ctx) => {
  ...
  ([0.22, 0.78] as const).forEach((y, index) => {
    ctx.add(
      drawLines(
        ctx.pixi,
        [{ x1: 0, y1: height * y, x2: width, y2: height * y }],
        palette.tone4,
        1.6,
        0.6,
      ),
...
const filmGate: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawHatchFill(
      ctx.pixi,
      rectPolygon(width * 0.2, height * 0.68, width * 0.6, height * 0.1),
      buildHatchSpec(ctx.seed, 131, 1.2),
      palette.tone2,
      0.5,
    ),
...
const crossVent: TemperaCompositionDrawer = (ctx) => {
  ...
  if (!ctx.showDecor) return;
  ctx.add(drawRings(ctx.pixi, [{ x: cx, y: cy, radius: arm * 1.24 }], palette.tone4, 1.2, 0.5), {
    delay: 0.2,
    span: 0.5,
  });
};
...
const louvre: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(width * 0.1, height * 0.72, width * 0.08, height * 0.2),
      palette.tone2,
      0.9,
      ctx.gradient,
    ),
...
const notchStack: TemperaCompositionDrawer = (ctx) => {
  ...
  for (let index = 0; index < 4; index += 1) {
    ...
    holes.push(
      rectPolygon(width * (0.6 + index * 0.06), height * (0.1 + index * 0.19), size, size),
    );
  }
  addCutField(ctx, palette.tone4, holes);
...
const wedgeGap: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawLines(
      ctx.pixi,
      [{ x1: apexX, y1: height * 0.58, x2: apexX, y2: height + bleed }],
      palette.tone4,
      1.6,
      0.6,
    ),
...
const dotSieve: TemperaCompositionDrawer = (ctx) => {
  ...
  for (let row = 0; row < 6; row += 1) {
    ...
    for (let column = 0; column < 5; column += 1) {
      ...
      holes.push(
        circlePolygon(x, height * (0.1 + row * 0.16), unit * 0.045 * density + unit * 0.012, 18),
      );
    }
  }
...
export const TEMPERA_APERTURE_COMPOSITIONS: Partial<
