  ...
  for (let i = 0; i < 14; i += 1) {
    ...
    target
      .circle(
        ...
        1.4 + sonnetHash01(seed, i, 109) * 2.2,
      )
      .fill({
        color: i % 3 === 0 ? secondary : primary,
        alpha: 0.3 + sonnetHash01(seed, i, 113) * 0.35,
      });
  }
};
...
const drawCometTrail = ({
  ...
  for (let tail = 0; tail < 3; tail += 1) {
    ...
    target
      .moveTo(hx - direction * radius * 0.04, hy + spread * 0.3)
      .bezierCurveTo(
        hx - direction * radius * 0.35,
        hy + spread,
        hx - direction * radius * 0.6,
        hy + radius * 0.16 + spread,
        hx - direction * radius * (0.85 + tail * 0.06),
        hy + radius * 0.3 + spread * 1.2,
      )
      .stroke({
        color: tail === 1 ? secondary : primary,
        width: 3 - tail,
        alpha: 0.55 - tail * 0.12,
      });
  }
  ...
  for (let i = 0; i < 5; i += 1) {
    ...
    target
      .moveTo(x - s, y)
      .lineTo(x + s, y)
      .stroke({ color: secondary, width: 1, alpha: 0.45 });
    target
      .moveTo(x, y - s)
      .lineTo(x, y + s)
      .stroke({ color: secondary, width: 1, alpha: 0.45 });
...
const drawOrbitSatellites = ({
  ...
  for (let ring = 0; ring < 3; ring += 1) {
    ...
    for (let s = 0; s < segs; s += 1) {
      ...
      target.arc(0, 0, r, start, start + (TAU / segs) * 0.68).stroke({
        color: ring === 1 ? secondary : primary,
        width: ring === 0 ? 2 : 1,
        alpha: 0.35 + ring * 0.08,
      });
    }
...
const drawAuroraRibbons = ({
  ...
  for (let band = 0; band < 4; band += 1) {
    const x0 =
      -radius * 0.6 + band * radius * 0.38 + (sonnetHash01(seed, band, 181) - 0.5) * radius * 0.1;
    const sway = (band % 2 === 0 ? 1 : -1) * radius * 0.2;
    target
      .moveTo(x0, -radius * 0.75)
      .bezierCurveTo(
        x0 + sway,
        -radius * 0.35,
        x0 - sway,
      ...
      .stroke({
        color: band % 2 === 0 ? primary : secondary,
        width: 7 - band,
        alpha: 0.16 + band * 0.05,
      });
    target
      .moveTo(x0 + radius * 0.06, -radius * 0.7)
      .bezierCurveTo(
        x0 + sway + radius * 0.06,
        -radius * 0.3,
        x0 - sway + radius * 0.06,
        radius * 0.12,
        x0 + sway * 0.6 + radius * 0.06,
        radius * 0.5,
      )
...
const drawNebulaVeil = ({
  ...
  for (let blob = 0; blob < 4; blob += 1) {
    ...
    target.bezierCurveTo(
      bx + br,
      by - br * (0.6 + wobble * 0.3),
      bx + br * 0.5,
      by - br,
      bx,
      by - br * (0.9 - wobble * 0.2),
    );
    target.bezierCurveTo(
      bx - br * 0.6,
      by - br * 0.8,
    ...
    target.bezierCurveTo(
      ...
      by + br,
      bx + br * 0.3,
      by + br * (0.8 + wobble * 0.2),
    );
    target.bezierCurveTo(bx + br * 0.8, by + br * 0.6, bx + br, by + br * 0.4, bx + br, by);
    target.stroke({
      color: blob % 2 === 0 ? primary : secondary,
      width: 1.5,
      alpha: 0.35 - blob * 0.04,
    });
...
const drawLunarTide = ({ target, radius, seed, primary, secondary }: AdditionalSonnetMgOptions) => {
  ...
  target
    .arc(mx, my, radius * 0.24, Math.PI * 0.2, Math.PI * 0.8)
  ...
  for (let row = 0; row < 4; row += 1) {
    ...
    for (let i = 0; i < arcs; i += 1) {
      ...
      target.arc(x, y, radius * 0.09, Math.PI, TAU).stroke({
        color: row % 2 === 0 ? primary : secondary,
        width: row === 0 ? 2 : 1,
        alpha: 0.45 - row * 0.07,
      });
    }
