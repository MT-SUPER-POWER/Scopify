...
export const MICRO_LINE_DURATION_THRESHOLD = 0.1;
...
const buildLineRenderEndTime = (
  ...
  const linePassStart =
    Math.max(getLastWordEndTime(line), line.startTime) + transitionTiming.linePassHold;
  const exitStart =
    lineTransitionMode === "fast"
      ? Math.max(
          line.startTime + transitionTiming.enterDuration + 0.01,
          linePassStart,
          line.endTime - transitionTiming.exitDuration,
        )
...
export function buildLineRenderHints(
  lineOrStart: RenderHintLineLike | number,
  endTime?: number,
): LineRenderHints {
  const line =
    typeof lineOrStart === "number"
...
export const getLineRenderHints = <T extends RenderHintLineLike>(
  line: T | null | undefined,
): LineRenderHints | null => {
  if (!line) {
    return null;
  }
...
export const getLineRenderEndTime = <T extends RenderHintLineLike>(
  line: T | null | undefined,
): number => {
  if (!line) {
...
export const migrateLyricLinesRenderHints = <T extends RenderHintLineLike>(
  lines: T[],
): MigrationResult<T[]> => {
  let changed = false;
...
export const ensureLyricDataRenderHints = <T extends RenderHintLyricDataLike>(
  lyrics: T | null | undefined,
): T | null => {
