...
export type VideoExportStatus =
  "idle" | "preparing" | "countdown" | "recording" | "finalizing" | "done" | "error";
...
export type VideoExportPresetValues = [
  VideoExportPresetPair,
  VideoExportPresetPair,
  VideoExportPresetPair,
];
...
const clampVideoExportPresetValue = (value: number, fallback = 720) => {
  ...
  const clampedValue = Math.min(
    ...
    Math.max(VIDEO_EXPORT_PRESET_MIN, safeValue),
  );
...
export const sanitizeVideoExportPresetValues = (
  values: readonly any[],
): VideoExportPresetValues => {
...
export const createVideoExportPresets = (values: readonly any[]): VideoExportPreset[] => {
  ...
  return safeValues.map((pair, index) =>
    buildVideoExportPreset(VIDEO_EXPORT_PRESET_IDS[index], pair.width, pair.height),
  );
};

export const VIDEO_EXPORT_PRESETS: VideoExportPreset[] = createVideoExportPresets(
