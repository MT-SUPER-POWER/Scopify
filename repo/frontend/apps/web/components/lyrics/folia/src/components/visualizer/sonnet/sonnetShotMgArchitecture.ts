...
export const drawSonnetGreenhouseMg = ({
  target,
  radius,
  width,
  height,
  ...
  for (let pane = -3; pane <= 3; pane += 1) {
    ...
    target
      .moveTo(x, radius * 0.58)
      .lineTo(x * 0.38, -radius * (0.58 - Math.abs(pane) * 0.04))
      .stroke({ color: pane % 2 ? secondary : primary, width: 1, alpha: 0.32 });
  }
  const doorX = radius * 0.2 * direction;
  target
    .rect(doorX - radius * 0.13, radius * 0.08, radius * 0.26, radius * 0.5)
    .fill({ color: secondary, alpha: 0.1 });
  target
    .rect(doorX - radius * 0.13, radius * 0.08, radius * 0.26, radius * 0.5)
    .stroke({ color: secondary, width: 2, alpha: 0.62 });
  target
    .moveTo(-bleed.x, radius * 0.58)
    .lineTo(-radius * 0.7, radius * 0.58)
    .stroke({ color: primary, width: 1, alpha: 0.3 });
  target
    .moveTo(radius * 0.7, radius * 0.58)
    .lineTo(bleed.x, radius * 0.58)
    .stroke({ color: primary, width: 1, alpha: 0.3 });
};

export const drawSonnetPagodaMg = ({
  target,
  radius,
  width,
  height,
  ...
  for (let floor = 0; floor < 4; floor += 1) {
    ...
    target
      .rect(-halfWidth * 0.68, y, halfWidth * 1.36, radius * 0.17)
  ...
  target
    .moveTo(0, -radius * 0.62)
    .lineTo(radius * 0.035 * lean, -radius * 0.78)
    .stroke({ color: secondary, width: 3, alpha: 0.65 });
  target
    .moveTo(-bleed.x, radius * 0.64)
    .lineTo(-radius * 0.52, radius * 0.64)
    .stroke({ color: secondary, width: 1, alpha: 0.22 });
  target
    .moveTo(radius * 0.52, radius * 0.64)
    .lineTo(bleed.x, radius * 0.64)
    .stroke({ color: secondary, width: 1, alpha: 0.22 });
};

export const drawSonnetCityFacadeMg = ({
  target,
  radius,
  width,
  height,
  ...
  heights.forEach((heightRatio, index) => {
    ...
    target
      .rect(x, radius * 0.62 - height, buildingWidth, height)
    ...
    for (let row = 0; row < Math.floor(heightRatio * 6); row += 1) {
      for (let column = 0; column < 2; column += 1) {
        if ((row + column + index + seed) % 3 === 0) {
          target
            .rect(
              x + radius * (0.035 + column * 0.085),
              radius * 0.53 - height + row * radius * 0.13,
              radius * 0.045,
              radius * 0.055,
            )
  ...
  target
    .moveTo(-bleed.x, radius * 0.63)
    .lineTo(bleed.x, radius * 0.63)
