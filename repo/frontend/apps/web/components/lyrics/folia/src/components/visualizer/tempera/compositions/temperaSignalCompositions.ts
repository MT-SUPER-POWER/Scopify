...
const sightMark: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    ...
    { delay: 0.04, span: 0.55, enterDX: width * 0.1 },
  );
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(width * 0.24, height * 0.36, width * 0.52, height * 0.28),
      palette.tone3,
      0.94,
      ctx.gradient,
    ),
  ...
  ctx.add(
    drawPolygonOutline(
      ctx.pixi,
      rectPolygon(width * 0.2, height * 0.32, width * 0.6, height * 0.36),
      palette.ink,
      2.4,
      0.85,
    ),
...
const tallyColumn: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(drawLines(ctx.pixi, marks, palette.tone4, 2.4, 0.75), {
    delay: 0.06,
    span: 0.6,
    enterDX: width * 0.1,
  });
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(x - width * 0.02, height * 0.4, width * 0.16, height * 0.12),
      palette.tone4,
      0.95,
      ctx.gradient,
    ),
  ...
  ctx.add(
    drawLines(
      ctx.pixi,
      [{ x1: x, y1: height * 0.06, x2: x, y2: height * 0.92 }],
      palette.ink,
      1.6,
      0.6,
    ),
...
const gridFocus: TemperaCompositionDrawer = (ctx) => {
  ...
  const cell = (column: number, row: number) =>
    rectPolygon(left + cellWidth * column, top + cellHeight * row, cellWidth, cellHeight);
  addCutField(ctx, palette.tone2, [cell(0, 0), cell(3, 2)]);
  addHoleLip(ctx, cell(0, 0), 2, 0.7, 0.1);
  addHoleLip(ctx, cell(3, 2), 2, 0.7, 0.14);
  ctx.add(drawPolygonFill(ctx.pixi, cell(3, 0), palette.tone4, 0.95, ctx.gradient), {
    delay: 0.08,
    span: 0.55,
    enterDX: width * 0.08,
  });
  ...
  for (let column = 0; column <= 4; column += 1) {
    lines.push({
      x1: left + cellWidth * column,
      y1: top,
      x2: left + cellWidth * column,
      y2: top + cellHeight * 3,
    });
  }
  for (let row = 0; row <= 3; row += 1) {
    lines.push({
      x1: left,
      y1: top + cellHeight * row,
      x2: left + cellWidth * 4,
      y2: top + cellHeight * row,
    });
  }
...
const axisCaps: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(drawRings(ctx.pixi, [{ x: cx, y: cy, radius: unit * 0.08 }], palette.ink, 2.4, 0.85), {
    delay: 0.12,
    span: 0.5,
  });
  ([1, -1] as const).forEach((side, index) => {
    ...
    ctx.add(
      drawPolygonFill(
        ctx.pixi,
        axisRect(capX, capY, unit * 0.1, unit * 0.34, ctx.flowAngle),
        palette.ink,
        0.85,
      ),
...
const offsetPlate: TemperaCompositionDrawer = (ctx) => {
  ...
  ([palette.tone2, palette.tone3, palette.tone4] as const).forEach((tone, index) => {
    const plate = rectPolygon(
      width * 0.2 + step * index,
      height * 0.2 + step * index,
      width * 0.5,
      height * 0.5,
    );
    ctx.add(
  ...
  ctx.add(drawRings(ctx.pixi, [{ x: holeX, y: holeY, radius: unit * 0.05 }], palette.ink, 2, 0.8), {
    delay: 0.22,
    span: 0.5,
  });
};
...
const radialComb: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawPolygonFill(
      ...
      [
        hubX,
        hubY,
        hubX + Math.cos(start) * reach,
        hubY + Math.sin(start) * reach,
        hubX + Math.cos(end) * reach,
        hubY + Math.sin(end) * reach,
      ],
      palette.tone4,
      0.95,
      ctx.gradient,
  ...
  ctx.add(
    drawHatchFill(
      ctx.pixi,
      rectPolygon(width * 0.06, height * 0.08, width * 0.2, height * 0.14),
      buildHatchSpec(ctx.seed, 151),
      palette.tone4,
      0.4,
    ),
...
const bracketTarget: TemperaCompositionDrawer = (ctx) => {
  ...
  (
    ...
    ctx.add(
      ...
      { delay: 0.12 + index * 0.04, span: 0.5, enterDX: sx * width * 0.05 },
    );
  });
  ctx.add(
    drawLines(
      ctx.pixi,
      [{ x1: width * 0.1, y1: height * 0.44, x2: width * 0.9, y2: height * 0.44 }],
      palette.tone4,
      1.6,
      0.6,
    ),
...
export const TEMPERA_SIGNAL_COMPOSITIONS: Partial<
