...
const frameWindow: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawHatchFill(
      ctx.pixi,
      diamondPolygon(cx, cy, rx * 0.78, ry * 0.78),
      buildHatchSpec(ctx.seed, 23, 1.4),
      palette.tone2,
      0.45,
    ),
    { grow: true, span: 0.6 },
  ...
  ctx.add(
    drawCrossMarks(
      ctx.pixi,
      buildCrossRow(ctx.seed, 29, width * 0.08, height * 0.14, 3, width * 0.045, 8),
      palette.tone4,
      1.8,
      0.85,
    ),
  ...
  ctx.add(
    drawSquareMarks(
      ctx.pixi,
      buildDotRow(ctx.seed, 37, width * 0.92, height * 0.62, 4, height * 0.06, 7),
      palette.tone4,
      0.8,
    ),
    { delay: 0.28 },
...
const doubleFrame: TemperaCompositionDrawer = (ctx) => {
  ...
  const shifted = rectPolygon(
    width * 0.16 + offset,
    height * 0.2 + offset * 0.6,
    width * 0.68,
    height * 0.6,
  );
  ctx.add(
  ...
  ctx.add(drawPolygonFill(ctx.pixi, shifted, palette.tone3, 0.8, ctx.gradient), {
    delay: 0.06,
    span: 0.55,
    enterDX: offset * 3,
  });
  ctx.add(drawHatchFill(ctx.pixi, shifted, buildHatchSpec(ctx.seed, 43), palette.tone4, 0.4), {
    delay: 0.1,
    span: 0.55,
    grow: true,
  });
  ctx.add(drawPolygonOutline(ctx.pixi, box, palette.ink, 3.5, 0.9), {
    delay: 0.14,
    span: 0.5,
    enterDY: -height * 0.08,
  });
...
const circleWindow: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawHatchFill(
      ctx.pixi,
      rectPolygon(-bleed, -bleed, width + bleed * 2, height + bleed * 2),
      buildHatchSpec(ctx.seed, 47),
      palette.tone4,
      0.4,
    ),
    { delay: 0.04, span: 0.6, grow: true },
  );
  ctx.add(drawPolygonFill(ctx.pixi, circle, palette.paper, 0.95, ctx.gradient), {
    delay: 0.08,
    span: 0.55,
  });
  ...
  ctx.add(
    drawPolygonOutline(
      ctx.pixi,
      circlePolygon(width / 2, height / 2, radius * 1.12),
      palette.ink,
      1.2,
      0.55,
    ),
    { delay: 0.2, drift: true },
  );
};
...
const ladderFrame: TemperaCompositionDrawer = (ctx) => {
  ...
  for (let index = 0; index < steps; index += 1) {
    ...
    ctx.add(
      ...
      { delay: index * 0.05, span: 0.5, enterDX: -width * 0.1 },
    );
  }
  ctx.add(
    drawLines(
      ctx.pixi,
      [{ x1: width * 0.9, y1: -bleed, x2: width * 0.9, y2: height + bleed }],
      palette.tone4,
      1.4,
      0.6,
    ),
...
const cornerBrackets: TemperaCompositionDrawer = (ctx) => {
  ...
  corners.forEach(([x, y, sx, sy], index) => {
    ctx.add(
      ...
      { delay: index * 0.05, span: 0.5, enterDX: sx * width * 0.06, enterDY: sy * height * 0.06 },
    );
  });
  if (!ctx.showDecor) return;
  ...
  ctx.add(
    dotted
      ? drawSquareMarks(
          ctx.pixi,
          buildDotRow(ctx.seed, 59, width * 0.5, height * 0.16, 3, width * 0.03, 6, 0),
          palette.tone4,
          0.8,
        )
      : drawCrossMarks(
          ctx.pixi,
          buildCrossRow(ctx.seed, 61, width * 0.44, height * 0.84, 3, width * 0.04, 7),
          palette.tone4,
          1.8,
          0.8,
        ),
...
const insetBox: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(drawPolygonFill(ctx.pixi, box, palette.tone3, 0.55, ctx.gradient), {
    delay: 0.05,
    span: 0.55,
    enterDY: height * 0.06,
  });
...
const bracketPair: TemperaCompositionDrawer = (ctx) => {
  ...
  ([1, -1] as const).forEach((side, index) => {
    ...
    ctx.add(
      ...
      { delay: index * 0.07, span: 0.5, enterDX: side * width * 0.08 },
    );
  });
};
...
const archWindow: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(drawPolygonFill(ctx.pixi, arch, palette.paper, 0.95, ctx.gradient), {
    delay: 0.05,
    span: 0.55,
    enterDY: -height * 0.08,
  });
  ctx.add(drawPolygonFill(ctx.pixi, body, palette.paper, 0.95, ctx.gradient), {
    delay: 0.05,
    span: 0.55,
    enterDY: height * 0.08,
  });
...
const gridCells: TemperaCompositionDrawer = (ctx) => {
  ...
  for (let row = 0; row < 3; row += 1) {
    for (let column = 0; column < 3; column += 1) {
      ...
      if (row === 1 && column === 1) {
        ctx.add(drawPolygonFill(ctx.pixi, cell, palette.tone4, 0.75, ctx.gradient), {
          delay: index * 0.03,
          span: 0.5,
        });
      }
      ctx.add(drawPolygonOutline(ctx.pixi, cell, palette.tone4, 1.2, 0.6), {
        delay: index * 0.03,
        span: 0.5,
      });
    }
...
const keyhole: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(drawPolygonFill(ctx.pixi, head, palette.paper, 0.96, ctx.gradient), {
    delay: 0.05,
    span: 0.55,
  });
  ctx.add(drawPolygonFill(ctx.pixi, shaft, palette.paper, 0.96, ctx.gradient), {
    delay: 0.08,
    span: 0.55,
    enterDY: height * 0.1,
  });
...
export const TEMPERA_FRAME_COMPOSITIONS: Partial<
