...
const apexMass: TemperaCompositionDrawer = (ctx) => {
  ...
  addFaceRuling(
    ctx,
    [apexX, apexY, width + bleed, height + bleed, apexX + width * 0.24, height + bleed],
    229,
  );
...
const ziggurat: TemperaCompositionDrawer = (ctx) => {
  ...
  for (let course = 0; course < 4; course += 1) {
    ...
    addMass(
      ctx,
      rectPolygon(width / 2 - half, top, half * 2, height * 0.13 + bleed),
      tones[course],
      {
...
const slabWall: TemperaCompositionDrawer = (ctx) => {
  ...
  addGround(ctx, palette.tone1);
  addMass(ctx, rectPolygon(edge, -bleed, width - edge + bleed, height + bleed * 2), palette.tone3, {
    edge: false,
  });
  ctx.add(
    drawLines(
      ctx.pixi,
      [{ x1: edge, y1: -bleed, x2: edge, y2: height + bleed }],
      palette.ink,
      3.5,
      0.9,
    ),
...
const cantilever: TemperaCompositionDrawer = (ctx) => {
  ...
  addMass(
    ctx,
    rectPolygon(0.16 * width, height * 0.5, width * 0.1, height * 0.5 + bleed),
    palette.tone4,
    {
      delay: 0.02,
      enterDY: height * 0.1,
  ...
  addMass(
    ctx,
    rectPolygon(-bleed, height * 0.34, width * 0.78 + bleed, height * 0.16),
    palette.tone3,
    {
      delay: 0.08,
      enterDX: -width * 0.14,
  ...
  addFaceRuling(
    ctx,
    rectPolygon(width * 0.3, height * 0.36, width * 0.44, height * 0.12),
    239,
    palette.paper,
    0.35,
  );
...
const pylonPair: TemperaCompositionDrawer = (ctx) => {
  ...
  ([0.1, 0.74] as const).forEach((x, index) => {
    addMass(
      ctx,
      rectPolygon(width * x, height * 0.28, width * 0.16, height * 0.72 + bleed),
      palette.tone4,
      {
  ...
  addMass(
    ctx,
    rectPolygon(width * 0.04, height * 0.14, width * 0.92, height * 0.16),
    palette.tone3,
    {
      delay: 0.14,
      enterDY: -height * 0.1,
...
const bunkerSlit: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(-bleed, height * 0.34, width + bleed * 2, height * 0.28),
      palette.tone4,
      0.95,
      ctx.gradient,
    ),
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(-bleed, height * 0.68, width + bleed * 2, height * 0.32 + bleed),
      palette.tone4,
      0.95,
      ctx.gradient,
    ),
  ...
  addFaceRuling(
    ctx,
    rectPolygon(width * 0.08, height * 0.72, width * 0.36, height * 0.2),
    241,
    palette.paper,
    0.3,
  );
...
const plinthStack: TemperaCompositionDrawer = (ctx) => {
  ...
  addMass(
    ctx,
    rectPolygon(width * 0.06, height * 0.72, width * 0.88, height * 0.28 + bleed),
    palette.tone4,
    { edgeWidth: 2 },
  );
  addMass(
    ctx,
    rectPolygon(width * 0.16, height * 0.52, width * 0.68, height * 0.2),
    palette.tone3,
    { delay: 0.08, edgeWidth: 2 },
  );
  addMass(
    ctx,
    rectPolygon(width * 0.28, height * 0.34, width * 0.44, height * 0.18),
    palette.tone2,
    { delay: 0.14, enterDY: height * 0.06 },
  );
...
const buttressRun: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawLines(
      ctx.pixi,
      [{ x1: -bleed, y1: height * 0.42, x2: width + bleed, y2: height * 0.42 }],
      palette.ink,
      2.4,
      0.8,
    ),
...
const voidCore: TemperaCompositionDrawer = (ctx) => {
  ...
  addFaceRuling(
    ctx,
    rectPolygon(width * 0.04, height * 0.36, width * 0.14, height * 0.42),
    247,
    palette.tone4,
    0.45,
  );
...
const shearBlock: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawPolygonOutline(
      ctx.pixi,
      rectPolygon(width * 0.1, height * 0.16, width * 0.8, height * 0.68),
      palette.paper,
      1.2,
      0.35,
    ),
...
export const TEMPERA_MONOLITH_COMPOSITIONS: Partial<
