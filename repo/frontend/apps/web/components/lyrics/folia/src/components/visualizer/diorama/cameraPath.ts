...
const vsub = (a: DioramaVec, b: DioramaVec): DioramaVec => ({
  x: a.x - b.x,
  y: a.y - b.y,
  z: a.z - b.z,
});
...
const vadd = (a: DioramaVec, b: DioramaVec): DioramaVec => ({
  x: a.x + b.x,
  y: a.y + b.y,
  z: a.z + b.z,
});
...
export const resolveDioramaMotionParams = (
  ...
  const mode: DioramaSubMode =
    themeIntensity === "calm" ? "calm" : themeIntensity === "chaotic" ? "chaotic" : "normal";
...
export const buildDioramaPath = (count: number, seed?: string | number): DioramaFrame[] => {
  ...
  for (let i = 0; i <= n; i += 1) {
    ...
    const yaw =
      DIORAMA_MAX_YAW *
      (Math.sin(i * 0.23 + base * 0.017) * 0.62 + Math.sin(i * 0.11 + base * 0.041 + 1.3) * 0.38);
...
export const translateFrames = (frames: DioramaFrame[], offset: DioramaVec): DioramaFrame[] =>
  frames.map((f) => ({
    position: vadd(f.position, offset),
    forward: f.forward,
    right: f.right,
    up: f.up,
  }));
...
const getLineNoteProfile = (
  line: Line | undefined,
): { maxNote: number; avgNote: number; noteCount: number } => {
  let maxNote = 0;
  let sum = 0;
  let noteCount = 0;
...
const computeShotWeights = (
  ...
  const sectionStart =
    !!line &&
    !!prev &&
    (line.blockIndex !== prev.blockIndex || (line.songPart ?? "") !== (prev.songPart ?? ""));
...
export const resolveShotOffset = (
  kind: DioramaShotKind,
  ctx: DioramaShotContext,
): DioramaShotOffset => {
 ...
 */
export const resolveCameraDrift = (
  time: number,
  seed: string | number | undefined,
  scale: number,
): DioramaCameraDrift => {
  ...
  return {
    swayX:
      (Math.sin(time * 0.11 + ph) * 0.6 + Math.sin(time * 0.047 + ph * 1.7) * 0.4) *
      DIORAMA_SWAY_AMP *
      scale,
    swayY: Math.sin(time * 0.09 + ph * 0.7) * DIORAMA_SWAY_AMP * 0.5 * scale,
    lift:
      (Math.sin(time * 0.13 + ph * 1.3) * 0.6 + Math.sin(time * 0.061 + ph * 2.1) * 0.4) *
      DIORAMA_LIFT_AMP *
      scale,
...
const getSpecClearanceRadius = (spec: FormationSpec): number => {
  const familyRadius =
    spec.kind === "box" ? 0.5 : spec.kind === "sphere" ? 0.74 : spec.kind === "cone" ? 0.68 : 0.9;
...
export const buildFormation = (
  ...
  const add = (
    ...
    specs.push({
      ...
      u,
      d,
      scale: scale * safeVolumeScale,
      stretchY,
      upright: stretchY > 1.2,
      colorSlot,
    });
  };
  const addPair = (
  ...
  switch (shot) {
    ...
    case "track": {
      ...
      for (let si = 0; si < 2; si += 1) {
        ...
        for (let j = 0; j < 3; j += 1) {
          add(
            "box",
            side * (4.2 + rnd(si * 5 + j) * 0.4),
            -0.4 + (rnd(si * 7 + j) - 0.5) * 0.6,
            0.5 + j * 2.4,
            0.5 + rnd(si * 3 + j) * 0.15,
            2.6 + rnd(si + j) * 0.6,
          );
    ...
    case "crane": {
      ...
      for (let j = 0; j < count; j += 1) {
        ...
        add(
          "box",
          Math.cos(a) * radius,
          Math.abs(Math.sin(a)) * radius + 1.2,
          0.6 + rnd(30 + j) * 0.8,
          0.45 + rnd(40 + j) * 0.25,
          1.4,
        );
    ...
    case "arc": {
      ...
      for (let j = 0; j < count; j += 1) {
        ...
        add(
          j === Math.floor(count / 2) ? "torus" : "sphere",
          Math.sin(a) * radius,
          0.3 + Math.cos(a) * 0.7,
          1.0 + rnd(20 + j) * 1.6,
          0.45 + rnd(30 + j) * 0.32,
...
export const resolveShapeLifeOpacity = (distanceToCamera: number): number => {
  const farT = clamp01(
    (DIORAMA_SHAPE_FADE_IN_END - distanceToCamera) /
      (DIORAMA_SHAPE_FADE_IN_END - DIORAMA_SHAPE_FADE_IN_START),
  );
  const nearT = clamp01(
    (distanceToCamera - DIORAMA_SHAPE_DISSOLVE_END) /
