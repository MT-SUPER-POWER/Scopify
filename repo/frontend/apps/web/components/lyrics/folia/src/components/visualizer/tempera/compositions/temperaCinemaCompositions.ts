...
const addMatte = (ctx: TemperaCompositionContext, hole: Window, tone: string) => {
  ...
  const bars: Array<{ polygon: number[]; enterDX: number; enterDY: number }> = [
    {
      polygon: rectPolygon(-bleed, -bleed, width + bleed * 2, hole.y + bleed),
      enterDX: 0,
      enterDY: -height * 0.2,
    },
    {
      polygon: rectPolygon(
        ...
        hole.y + hole.height,
        width + bleed * 2,
        height - hole.y - hole.height + bleed,
      ),
      enterDX: 0,
      enterDY: height * 0.2,
    },
    {
      polygon: rectPolygon(-bleed, hole.y, hole.x + bleed, hole.height),
      enterDX: -width * 0.2,
      enterDY: 0,
    },
    {
...
const addWindowWash = (ctx: TemperaCompositionContext, hole: Window, alpha: number) => {
  ...
  ctx.add(drawPolygonFill(ctx.pixi, polygon, ctx.palette.tone1, alpha, ctx.gradient), {
    span: 0.55,
  });
  ctx.add(
    drawHatchFill(ctx.pixi, polygon, buildHatchSpec(ctx.seed, 149, 1.5), ctx.palette.tone4, 0.2),
    { delay: 0.1, span: 0.6, grow: true },
  );
};

const matte =
  (aspect: number, fill: number, washAlpha = 0.35): TemperaCompositionDrawer =>
  (ctx) => {
    ...
    ctx.add(
      drawLines(
        ...
        [
          {
            x1: hole.x,
            y1: hole.y - ctx.height * 0.05,
            x2: hole.x,
            y2: hole.y - ctx.height * 0.02,
          },
          {
            x1: hole.x + hole.width,
            y1: hole.y + hole.height + ctx.height * 0.02,
            x2: hole.x + hole.width,
            y2: hole.y + hole.height + ctx.height * 0.05,
          },
        ],
        ctx.palette.paper,
        2,
...
const cinemaTwin: TemperaCompositionDrawer = (ctx) => {
  ...
  ([left, right] as const).forEach((hole, index) => {
    const polygon = rectPolygon(hole.x, hole.y, hole.width, hole.height);
    ctx.add(
      drawPolygonFill(ctx.pixi, polygon, palette.tone1, index === 0 ? 0.4 : 0.85, ctx.gradient),
      { delay: index * 0.08, span: 0.55, enterDX: (index === 0 ? -1 : 1) * width * 0.14 },
    );
    ctx.add(drawPolygonOutline(ctx.pixi, polygon, palette.ink, 1.6, 0.5), {
  ...
  ctx.add(
    drawHatchFill(
      ctx.pixi,
      rectPolygon(right.x, right.y, right.width, right.height),
...
export const TEMPERA_CINEMA_COMPOSITIONS: Partial<
