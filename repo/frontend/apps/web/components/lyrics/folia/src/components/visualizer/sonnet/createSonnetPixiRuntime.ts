...
import { destroySonnetContainerChildren, unloadSonnetDisplayTree } from "./sonnetPixiResources";
import {
...
export class SonnetPixiRuntime {
  ...
  private clearOutroBlur() {
    if (this.outroBlurFilter && this.outroBlurScene) {
      this.outroBlurScene.container.filters = (this.outroBlurScene.container.filters ?? []).filter(
        (filter) => filter !== this.outroBlurFilter,
      );
  ...
  private updateShot(
    ...
    const motion =
      this.options.tuning.typographyMotion * resolveSonnetAnimationScale(this.options.theme);
    const camera =
      this.options.tuning.cameraIntensity * resolveSonnetAnimationScale(this.options.theme);
    ...
    const revealDoneTime =
      trackSegments.length > 0
        ? Math.max(
            ...trackSegments.map(
              (segment) => segment.trackingGlyphs.at(-1)?.startTime ?? view.shot.endTime,
            ),
          )
        : view.shot.endTime;
    ...
    view.container.scale.set(view.shot.camera.zoom * (1 + (cameraFrame.scale - 1) * camera));
    view.container.rotation =
      (view.shot.camera.rotation + cameraFrame.rotation + shake.rotation) * camera;
    ...
    view.segments.forEach((segmentView) => {
      ...
      guide.container.visible =
        guideActive && this.options.tuning.showGuide && !this.options.tuning.showOnlyText;
      if (guideActive) {
      ...
      segmentView.glyphs.forEach((glyph) => {
        const glyphProgress = resolveSegmentProgress(glyph.startTime, glyph.settleTime, time);
        ...
        const scale =
          isSonnetEmphasisRole(segmentView.role) && view.shot.kind === "type-impact"
  ...
  private renderFrame = () => {
    ...
    this.sceneCache.forEach((scene, index) => {
      ...
      const previousTransition =
        index > 0 ? this.options.program.paragraphs[index - 1]?.transitionOut : null;
    ...
    this.creditsContainer.position.set(width / 2, height / 2 + creditsFrame.posterOffsetY * height);
