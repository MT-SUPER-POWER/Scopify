...
const quietLine: TemperaCompositionDrawer = (ctx) => {
  ...
  [0.38, 0.5, 0.62].forEach((ratio, index) => {
    ctx.add(
      drawPolygonFill(
        ctx.pixi,
        rectPolygon(gridX, height * ratio, gridWidth, 1),
        palette.line,
        1,
        ctx.gradient,
      ),
...
const starfieldDots: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(drawSquareMarks(ctx.pixi, dense, palette.tone4, 0.45), {
    span: 0.6,
    enterDY: height * 0.2,
  });
  ctx.add(drawSquareMarks(ctx.pixi, sparse, palette.tone4, 0.25), {
    delay: 0.08,
    span: 0.6,
    enterDY: -height * 0.15,
  });
  ...
  ctx.add(
    drawLines(
      ctx.pixi,
      [{ x1: -bleed, y1: height * 0.45, x2: width + bleed, y2: height * 0.45 }],
      palette.tone4,
      1.2,
      0.5,
    ),
...
const hairGrid: TemperaCompositionDrawer = (ctx) => {
  ...
  const vertical = Array.from({ length: columns + 1 }, (_, index) => ({
    x1: (width / columns) * index,
    y1: -bleed,
    x2: (width / columns) * index,
    y2: height + bleed,
  }));
  const horizontal = Array.from({ length: rows + 1 }, (_, index) => ({
    x1: -bleed,
    y1: (height / rows) * index,
    x2: width + bleed,
    y2: (height / rows) * index,
  }));
  ctx.add(drawLines(ctx.pixi, vertical, palette.line, 1, 0.8), {
    span: 0.6,
    enterDY: -height * 0.1,
  });
  ctx.add(drawLines(ctx.pixi, horizontal, palette.line, 1, 0.8), {
    delay: 0.08,
    span: 0.6,
    enterDX: width * 0.1,
  });
...
const marginRule: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(x - 3, -bleed, 6, height + bleed * 2),
      palette.tone4,
      0.9,
      ctx.gradient,
    ),
...
const dotDrift: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(drawSquareMarks(ctx.pixi, near, palette.tone4, 0.5), {
    span: 0.6,
    enterDX: width * 0.12,
  });
  ctx.add(drawSquareMarks(ctx.pixi, far, palette.tone4, 0.24), {
    delay: 0.08,
    span: 0.6,
    enterDX: -width * 0.12,
  });
...
export const TEMPERA_SPARSE_COMPOSITIONS: Partial<
