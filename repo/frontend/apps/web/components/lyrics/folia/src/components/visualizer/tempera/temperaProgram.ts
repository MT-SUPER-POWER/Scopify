...
const metadataChanged = (previous: Line, next: Line) =>
  ...
  (previous.songPart !== undefined &&
    next.songPart !== undefined &&
...
const splitOversizedDraft = (draft: ParagraphDraft): ParagraphDraft[] => {
  ...
  while (
    remaining.length > 6 ||
    (remaining.length > 1 && remaining.at(-1)!.renderEndTime - remaining[0].line.startTime > 18)
  ) {
    if (loopGuard++ > 1000) {
    ...
    const rawSplitIndex =
      validCandidates.sort((a, b) => b.gap - a.gap)[0]?.splitIndex ??
...
const classifyParagraph = (
  ...
  if (lines.some((item) => item.line.isChorus || /chorus|副歌/i.test(item.line.songPart ?? "")))
    return "chorus";
  if (lines.some((item) => /bridge|break|間奏|ブリッジ/i.test(item.line.songPart ?? "")))
    return "break";
  if (index === total - 1) return "outro";
  const duration = lines.at(-1)!.renderEndTime - lines[0].line.startTime;
  const segmentCount = lines.reduce(
    (sum, line) => sum + line.segments.filter((segment) => segment.isWordLike).length,
    0,
  );
  const punctuationCount = lines.reduce(
    (sum, line) => sum + (line.line.fullText.match(/[!?！？…]/g)?.length ?? 0),
    0,
  );
...
const buildShotChunks = (
  ...
  lines.forEach((line) => {
    ...
    usable.forEach((entry, order) => {
      words += 1;
      const chunkSeed = hashTemperaSeed(
        `${seed}:${paragraphIndex}:${line.sourceIndex}:${chunks.length}`,
      );
...
const buildDecorFragments = (pool: string, count: number, seed: number): TemperaDecorFragment[] => {
  const chars = Array.from(pool).filter(
    (char) => char.trim().length > 0 && !PUNCTUATION_ONLY.test(char),
  );
...
const buildDecorSpec = (
  ...
  const sparse =
    resolveTemperaShotProfile(shotKind).mood === "quiet" ||
    paragraphKind === "break" ||
    paragraphKind === "outro";
  return {
    motif,
    ...
    fragments: sparse
      ...
      : [],
    watermark: buildDecorWatermark(
      watermarkPool,
      seed,
      resolveTemperaShotProfile(shotKind).mood !== "loud",
    ),
  };
};
...
const buildShots = (
  ...
  return chunks.map((chunk, shotIndex) => {
    ...
    const outsideSlice = lines.flatMap((item) =>
      item.sourceIndex === chunk.lineIndex
        ? item.segments.filter(
            (_, index) => index < chunk.segmentStart || index >= chunk.segmentEnd,
          )
        : item.segments,
    );
    ...
    return {
      ...
      endTime:
        shotIndex === chunks.length - 1 ? Math.max(chunk.endTime, paragraphEnd) : chunk.endTime,
...
export const compileTemperaProgram = (
  ...
  const compiled = lines.map((line, sourceIndex) => ({
    ...
    renderEndTime: Math.max(
      ...
      Math.min(
        ...
        lines[sourceIndex + 1]?.startTime ?? Number.POSITIVE_INFINITY,
      ),
    ),
  ...
  compiled.forEach((line, index) => {
    ...
    const boundary =
      previous && metadataChanged(previous.line, line.line)
        ? "metadata"
