...
export const resolveWashEndpoints = (background: RgbChannels, primary: RgbChannels) => {
...
export const getMonetBackgroundCacheKey = ({
  ...
  theme,
  tuning,
}: BuildMonetBackgroundOptions) =>
  JSON.stringify({
    sourceUrl: resolveSourceUrl({ coverUrl, monetBackgroundImage, tuning }),
    uploadedId:
      tuning.backgroundSource === "uploaded-global" ? (monetBackgroundImage?.id ?? null) : null,
    theme: {
