...
const SectionLabel: React.FC<{ children: React.ReactNode; theme: Theme }> = ({
  children,
  theme,
}) => (
  <div
          ...
          <button
            ...
            style={{
              color: option.disabled
                ? colorWithAlpha(theme.secondaryColor, 0.72)
                : theme.primaryColor,
...
export const MonetBackgroundSettingsCard: React.FC<MonetBackgroundSettingsCardProps> = ({
  ...
  const resolvedTuning = {
    ...
    backgroundBlurPx: clampValue(
      tuning.backgroundBlurPx ?? DEFAULT_MONET_BACKGROUND_TUNING.backgroundBlurPx,
      0,
      60,
      DEFAULT_MONET_BACKGROUND_TUNING.backgroundBlurPx,
    ),
    backgroundOverlayOpacity: clampValue(
      tuning.backgroundOverlayOpacity ?? DEFAULT_MONET_BACKGROUND_TUNING.backgroundOverlayOpacity,
      0,
      1,
      DEFAULT_MONET_BACKGROUND_TUNING.backgroundOverlayOpacity,
    ),
    backgroundGrayscale: clampValue(
      tuning.backgroundGrayscale ?? DEFAULT_MONET_BACKGROUND_TUNING.backgroundGrayscale,
      0,
      1,
      DEFAULT_MONET_BACKGROUND_TUNING.backgroundGrayscale,
    ),
    backgroundSaturation: clampValue(
      tuning.backgroundSaturation ?? DEFAULT_MONET_BACKGROUND_TUNING.backgroundSaturation,
      0,
      2,
      DEFAULT_MONET_BACKGROUND_TUNING.backgroundSaturation,
    ),
    backgroundWash: clampValue(
      tuning.backgroundWash ?? DEFAULT_MONET_BACKGROUND_TUNING.backgroundWash,
      0,
      1,
      DEFAULT_MONET_BACKGROUND_TUNING.backgroundWash,
    ),
    backgroundHalfPaneOffsetX: clampValue(
      tuning.backgroundHalfPaneOffsetX ?? DEFAULT_MONET_BACKGROUND_TUNING.backgroundHalfPaneOffsetX,
      -40,
      40,
      DEFAULT_MONET_BACKGROUND_TUNING.backgroundHalfPaneOffsetX,
    ),
    backgroundWashColorMode:
      tuning.backgroundWashColorMode ?? DEFAULT_MONET_BACKGROUND_TUNING.backgroundWashColorMode,
    backgroundWashCustomColor:
      normalizeHexInput(tuning.backgroundWashCustomColor) ??
      DEFAULT_MONET_BACKGROUND_TUNING.backgroundWashCustomColor,
    backgroundDriftEnabled:
      tuning.backgroundDriftEnabled ?? DEFAULT_MONET_BACKGROUND_TUNING.backgroundDriftEnabled,
    backgroundDriftStrength: clampValue(
      tuning.backgroundDriftStrength ?? DEFAULT_MONET_BACKGROUND_TUNING.backgroundDriftStrength,
      0,
      1,
      DEFAULT_MONET_BACKGROUND_TUNING.backgroundDriftStrength,
    ),
    backgroundStreaksEnabled:
      tuning.backgroundStreaksEnabled ?? DEFAULT_MONET_BACKGROUND_TUNING.backgroundStreaksEnabled,
  };

  const sourceOptions = useMemo<PresetOption<MonetBackgroundSource>[]>(
    () => [
      { value: "cover-derived", label: t("folia.options.monetBackgroundSourceCover") },
      {
        value: "uploaded-global",
        label: t("folia.options.monetBackgroundSourceUploaded"),
  ...
  const layoutOptions = useMemo<PresetOption<MonetBackgroundLayout>[]>(
    () => [
      { value: "half-pane-gradient", label: t("folia.options.monetLayoutHalfPane") },
      { value: "full-overlay", label: t("folia.options.monetLayoutFullOverlay") },
    ],
    [t],
  );
  const washModeOptions = useMemo<PresetOption<MonetBackgroundWashColorMode>[]>(
    () => [
      { value: "theme", label: t("folia.options.monetBackgroundWashTheme") },
      { value: "custom", label: t("folia.options.monetBackgroundWashCustom") },
    ],
    [t],
  );
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (result.ok) {
      ...
      setFeedback(result.error || t("folia.options.monetUploadBackground"));
    }
  };
    ...
    >
      <div className="space-y-1">
        <div className="text-sm font-medium" style={{ color: theme.primaryColor }}>
          {t("folia.options.monetBackgroundSettings")}
        </div>
        <div className="text-xs opacity-70" style={{ color: theme.secondaryColor }}>
          {t("folia.options.monetBackgroundSettingsDesc")}
        </div>
      </div>

      {/* Section 1: Background Source */}
      <div className="space-y-3.5 rounded-[18px] border border-white/5 bg-black/5 p-3.5 dark:bg-white/5">
        <div
          className="text-xs font-semibold tracking-wider uppercase opacity-65"
          style={{ color: theme.accentColor }}
        >
          {t("folia.options.monetGroupBackgroundSource")}
        </div>
        <PresetGroup
          label={t("folia.options.monetBackgroundSource")}
        ...
        <div className="space-y-2.5">
          <SectionLabel theme={theme}>{t("folia.options.monetUploadBackground")}</SectionLabel>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <ImagePlus size={15} />
              <span>{t("folia.options.monetUploadBackground")}</span>
            </button>
            <button
              type="button"
            ...
            >
              <Trash2 size={15} />
              <span>{t("folia.options.monetClearBackground")}</span>
            </button>
          </div>
      ...
      <div className="space-y-3.5 rounded-[18px] border border-white/5 bg-black/5 p-3.5 dark:bg-white/5">
        <div
          ...
          style={{ color: theme.accentColor }}
        >
          {t("folia.options.monetGroupLayoutPosition")}
        </div>
        <PresetGroup
          label={t("folia.options.monetBackgroundLayout")}
        ...
        {resolvedTuning.backgroundLayout === "half-pane-gradient" && (
          <SliderControl
            label={t("folia.options.monetHalfPaneOffsetX")}
        ...
        <BackgroundToggleRow
          label={t("folia.options.monetBackgroundDriftEnabled")}
          description={t("folia.options.monetBackgroundDriftEnabledDesc")}
        ...
        {resolvedTuning.backgroundDriftEnabled && (
          <SliderControl
            label={t("folia.options.monetBackgroundDriftStrength")}
      ...
      <div className="space-y-3.5 rounded-[18px] border border-white/5 bg-black/5 p-3.5 dark:bg-white/5">
        <div
          ...
          style={{ color: theme.accentColor }}
        >
          {t("folia.options.monetGroupFiltersPostProcessing")}
        </div>
        <SliderControl
          label={t("folia.options.monetBackgroundBlur")}
        ...
        />
        <SliderControl
          label={t("folia.options.monetBackgroundSaturation")}
        ...
        />
        <SliderControl
          label={t("folia.options.monetBackgroundGrayscale")}
      ...
      <div className="space-y-3.5 rounded-[18px] border border-white/5 bg-black/5 p-3.5 dark:bg-white/5">
        <div
          ...
          style={{ color: theme.accentColor }}
        >
          {t("folia.options.monetGroupColorTintWash")}
        </div>
        <SliderControl
          label={t("folia.options.monetBackgroundOverlayOpacity")}
        ...
        />
        <SliderControl
          label={t("folia.options.monetBackgroundWash")}
        ...
        <BackgroundToggleRow
          label={t("folia.options.monetBackgroundStreaksEnabled")}
          description={t("folia.options.monetBackgroundStreaksEnabledDesc")}
        ...
        <PresetGroup
          label={t("folia.options.monetBackgroundWashColorMode")}
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <div className="text-sm" style={{ color: theme.primaryColor }}>
                {t("folia.options.monetBackgroundWashCustomColor")}
              </div>
              <div
                className="rounded-full px-3 py-1 font-mono text-xs"
                style={{
                  color: theme.backgroundColor,
                  backgroundColor: resolvedTuning.backgroundWashCustomColor,
                }}
              >
