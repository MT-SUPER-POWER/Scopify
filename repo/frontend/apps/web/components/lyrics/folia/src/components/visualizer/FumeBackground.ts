...
export const buildFumeBackgroundScene = ({
  ...
  const shapeKinds: FumeBackgroundShapeKind[] = [
    "ring",
    "square",
    "cross",
    "ring",
  ...
  const baseShapes = Array.from({ length: shapeCount }).map((_, index) => {
    ...
    return {
      ...
      color: (seeded(`${localSeed}:color`) > 0.5
        ? "accent"
        : "secondary") as FumeBackgroundShapeColor,
      depth: seeded(`${localSeed}:depth`),
      ringGapStart:
        kind === "ring" ? mix(-Math.PI, Math.PI, seeded(`${localSeed}:gap-start`)) : undefined,
      ringGapSize:
        kind === "ring"
  ...
  const sparkShapes = Array.from({ length: sparkCount }).map((_, index) => {
    ...
    const x = clamp(
      sparkAreaLeft + (column + 0.5) * sparkCellWidth + jitterX,
      sparkAreaLeft,
      sparkAreaRight,
    );
    const y = clamp(
      sparkAreaTop + (row + 0.5) * sparkCellHeight + jitterY,
      sparkAreaTop,
      sparkAreaBottom,
    );
...
export const drawFumeBackground = ({
  ...
  const createLineGradient = (shape: FumeBackgroundShape, opacity: number) => {
    ...
    gradient.addColorStop(
      0.28,
      colorWithAlpha(mixColors(theme.secondaryColor, theme.accentColor, 0.24), opacity * 0.58),
    );
    gradient.addColorStop(
      0.54,
      colorWithAlpha(mixColors(theme.secondaryColor, theme.accentColor, 0.62), opacity * 0.92),
    );
  ...
  for (const shape of scene.shapes) {
    ...
    const audioScale =
      bandValue === undefined ? 1 : mix(0.95, 1.45, clamp((bandValue - 10) / 190, 0, 1));
    const audioOpacityBoost =
      bandValue === undefined ? 1 : mix(0.85, 1.55, clamp((bandValue - 10) / 190, 0, 1));
