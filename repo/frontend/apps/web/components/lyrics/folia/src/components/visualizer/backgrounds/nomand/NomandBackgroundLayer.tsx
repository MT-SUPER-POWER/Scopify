...
import { resolveNomandInverted } from "./nomandColorMode";
...
const NomandBackgroundLayer: React.FC<NomandBackgroundLayerProps> = ({
  ...
  const sourceUrl =
    tuning.imageSource === "uploaded-global"
      ? (monetBackgroundImage?.url ?? coverUrl)
      : (coverUrl ?? monetBackgroundImage?.url);

  if (!sourceUrl) {
    return (
      <div className="absolute inset-0 z-0" style={{ backgroundColor: theme.backgroundColor }} />
    );
  }
  ...
  return (
    ...
    >
      <ImageDithering
        ...
        inverted={resolveNomandInverted(tuning.inverted, isDaylight, tuning.originalColors)}
