...
const resolveWordScales = (segments: TemperaSegment[], seed: number, lineIndex: number) => {
  ...
  const heroIndex =
    heroCandidates.length > 0
      ? heroCandidates[
          Math.floor(temperaHash01(seed, lineIndex, 101) * heroCandidates.length) %
...
const packRows = (words: TemperaWordUnit[], maxWidth: number, seed: number): LayoutRow[] => {
  ...
  let currentWidth = 0;
  const budgetFor = (rowIndex: number) =>
    maxWidth * (0.72 + temperaHash01(seed, rowIndex, 109) * 0.28);
  const flush = () => {
...
const buildWordUnits = (
  ...
  lines.flatMap((segments, lineIndex) => {
    ...
    return segments.flatMap((segment, segmentIndex) => {
      const unit = buildTemperaWordUnit(
        ctx,
        segment,
        lineIndex,
        segmentIndex,
...
const applyGlyphTiming = (placements: TemperaGlyphPlacement[], settleStretch: number) => {
  ...
  const stretch = Number.isFinite(settleStretch)
