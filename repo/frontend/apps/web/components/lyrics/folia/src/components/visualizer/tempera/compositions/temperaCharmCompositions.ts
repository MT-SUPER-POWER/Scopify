import type {
  ...
  TemperaCompositionDrawer,
} from "../temperaCompositionContext";
...
const addField = (ctx: TemperaCompositionContext, color: string, alpha = 0.92) => {
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(-bleed, -bleed, width + bleed * 2, height + bleed * 2),
      color,
      alpha,
      ctx.gradient,
    ),
...
const bubbleDrift: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(drawDiscs(ctx.pixi, big, palette.tone3, 0.88, ctx.gradient), {
    delay: 0.05,
    span: 0.6,
    enterDY: height * 0.16,
    drift: true,
  });
  ctx.add(drawRings(ctx.pixi, big, palette.ink, 2, 0.55), {
    delay: 0.12,
    span: 0.55,
    enterDY: height * 0.12,
  });
  ctx.add(drawDiscs(ctx.pixi, small, palette.tone4, 0.7), {
    delay: 0.18,
    span: 0.6,
    enterDY: height * 0.22,
    drift: true,
...
const cloudWindow: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      lobedPolygon(cx, cy + height * 0.025, rx * 1.05, ry * 1.05, 9, 0.1),
      palette.tone4,
      0.5,
    ),
    { delay: 0.04, span: 0.6 },
  );
  ctx.add(drawPolygonFill(ctx.pixi, cloud, palette.paper, 0.96, ctx.gradient), {
    delay: 0.08,
    span: 0.55,
  });
...
const heartBurst: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rotatePolygon(
        heartPolygon(cx + lean * width * 0.022, cy + height * 0.025, rx, ry),
        cx,
        cy,
        lean * 0.06,
      ),
  ...
  ctx.add(drawPolygonFill(ctx.pixi, main, palette.paper, 0.95, ctx.gradient), {
    delay: 0.08,
    span: 0.55,
    enterDY: height * 0.1,
  });
  ctx.add(drawPolygonOutline(ctx.pixi, main, palette.ink, 3, 0.9), {
    delay: 0.14,
    span: 0.5,
    enterDY: height * 0.08,
  });
  [0.4, 0.24].forEach((scale, index) => {
    ...
    ctx.add(
      ...
      { delay: 0.2 + index * 0.06, span: 0.5, enterDX: lean * width * 0.12, drift: true },
      group,
    );
  });
};
...
const sparkleField: TemperaCompositionDrawer = (ctx) => {
  ...
  seeds.slice(0, 5).forEach((disc, index) => {
    ...
    const star = starPolygon(
      ...
      disc.radius * 0.16,
      4,
      temperaHash01(ctx.seed, index, 29) * 0.9,
    );
    const group = ctx.createGroup(0, disc.x, disc.y);
    ctx.add(
      drawPolygonFill(ctx.pixi, star, palette.tone4, 0.9, ctx.gradient),
      { delay: 0.06 + index * 0.05, span: 0.5, drift: true },
      group,
    );
  });
  ctx.add(
  ...
  ctx.add(
    drawLines(
      ctx.pixi,
      [{ x1: width * 0.08, y1: height * 0.74, x2: width * 0.92, y2: height * 0.72 }],
      palette.tone4,
      1.2,
      0.5,
    ),
...
const petalArc: TemperaCompositionDrawer = (ctx) => {
  ...
  for (let index = 0; index < 5; index += 1) {
    ...
    ctx.add(
      drawPolygonFill(
        ctx.pixi,
        petal,
        index % 2 === 0 ? palette.tone3 : palette.tone1,
        0.9,
        ctx.gradient,
      ),
      { delay: index * 0.05, span: 0.55, enterDY: -height * 0.1 },
    );
    ctx.add(drawPolygonOutline(ctx.pixi, petal, palette.ink, 1.6, 0.5), {
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      lobedPolygon(0, 0, width * 0.11, width * 0.11, 6, 0.34),
      palette.tone4,
      0.9,
      ctx.gradient,
    ),
    { delay: 0.28, span: 0.5, drift: true },
    hub,
  );
  if (!ctx.showDecor) return;
  ...
  ctx.add(
    ...
    { delay: 0.32, span: 0.5, drift: true },
  );
};
...
const scallopBand: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(drawPolygonFill(ctx.pixi, top, palette.tone3, 0.92, ctx.gradient), {
    delay: 0.04,
    span: 0.55,
    enterDY: -height * 0.18,
  });
  ctx.add(drawPolygonFill(ctx.pixi, bottom, palette.tone4, 0.92, ctx.gradient), {
    delay: 0.1,
    span: 0.55,
    enterDY: height * 0.18,
  });
  ctx.add(drawPolygonOutline(ctx.pixi, top, palette.ink, 2, 0.75), {
    delay: 0.14,
    span: 0.5,
    enterDY: -height * 0.12,
  });
  ctx.add(drawPolygonOutline(ctx.pixi, bottom, palette.ink, 2, 0.75), {
    delay: 0.18,
    span: 0.5,
    enterDY: height * 0.12,
  });
...
const ribbonLoop: TemperaCompositionDrawer = (ctx) => {
  ...
  const sag =
    height *
    (0.06 + temperaHash01(ctx.seed, 7, 97) * 0.05) *
    (temperaHash01(ctx.seed, 9, 97) > 0.5 ? 1 : -1);
  addField(ctx, palette.tone2);
  ...
  ctx.add(drawPolygonFill(ctx.pixi, echo, palette.tone4, 0.7, ctx.gradient), {
    delay: 0.04,
    span: 0.6,
    enterDX: -width * 0.16,
  });
  ctx.add(drawPolygonFill(ctx.pixi, band, palette.paper, 0.95, ctx.gradient), {
    delay: 0.08,
    span: 0.55,
    enterDX: width * 0.18,
  });
  ctx.add(drawPolygonOutline(ctx.pixi, band, palette.ink, 2.6, 0.85), {
    delay: 0.14,
    span: 0.5,
    enterDX: width * 0.14,
  });
  ...
  ([-1, 1] as const).forEach((side, index) => {
    ...
    ctx.add(
      drawPolygonFill(ctx.pixi, loop, palette.tone4, 0.92, ctx.gradient),
      { delay: 0.2 + index * 0.04, span: 0.5, enterDX: side * width * 0.06 },
      bow,
    );
    ctx.add(
      drawPolygonOutline(ctx.pixi, loop, palette.ink, 2, 0.8),
      { delay: 0.24 + index * 0.04, span: 0.5 },
      bow,
    );
  });
  const knot = roundedRectPolygon(
    -width * 0.018,
    -height * 0.035,
    width * 0.036,
    height * 0.07,
  ...
  ctx.add(
    drawPolygonFill(ctx.pixi, knot, palette.tone4, 0.95, ctx.gradient),
    { delay: 0.28, span: 0.5 },
    bow,
  );
...
const roundPlate: TemperaCompositionDrawer = (ctx) => {
  ...
  const unit = Math.min(width, height);
  addField(ctx, palette.tone3);
  const plate = roundedRectPolygon(
    width * 0.16,
    height * 0.24,
    width * 0.68,
    height * 0.52,
  ...
  ctx.add(drawPolygonFill(ctx.pixi, plate, palette.paper, 0.95, ctx.gradient), {
    delay: 0.05,
    span: 0.55,
    enterDY: height * 0.06,
  });
  ...
  ctx.add(drawHatchFill(ctx.pixi, inner, buildHatchSpec(ctx.seed, 101, 1.3), palette.tone2, 0.35), {
    delay: 0.1,
    span: 0.6,
    grow: true,
  });
  ctx.add(drawPolygonOutline(ctx.pixi, plate, palette.ink, 3, 0.9), { delay: 0.12, span: 0.5 });
  ctx.add(drawPolygonOutline(ctx.pixi, inner, palette.tone4, 1.2, 0.55), {
    delay: 0.16,
    span: 0.5,
  });
  if (!ctx.showDecor) return;
...
const haloBurst: TemperaCompositionDrawer = (ctx) => {
    ...
    ctx.add(
      drawPolygonFill(
        ...
        [
          cx,
          cy,
          cx + Math.cos(angle - spread) * reach,
          cy + Math.sin(angle - spread) * reach,
          cx + Math.cos(angle + spread) * reach,
          cy + Math.sin(angle + spread) * reach,
        ],
        palette.tone3,
      ...
      { delay: 0.04 + index * 0.012, span: 0.6 },
    );
  }
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      ellipsePolygon(cx, cy, unit * 0.34, unit * 0.34, 48),
      palette.paper,
      0.94,
      ctx.gradient,
    ),
  ...
  ).forEach(([scale, stroke], index) => {
    ctx.add(
      drawRings(ctx.pixi, [{ x: cx, y: cy, radius: unit * scale }], palette.ink, stroke, 0.75),
      { delay: 0.2 + index * 0.06, span: 0.5, drift: true },
    );
  });
};

export const TEMPERA_CHARM_COMPOSITIONS: Partial<
  Record<TemperaShotKind, TemperaCompositionDrawer>
> = {
