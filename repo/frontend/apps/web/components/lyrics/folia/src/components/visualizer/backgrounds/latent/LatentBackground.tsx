...
) => Math.max(previousPulse * 0.84, clampAudioAmount((currentEnergy - previousEnergy) * 7));
export const resolveLatentBeatSpeedTarget = (broadbandEnergy: number, onsetPulse: number) =>
...
const LatentBackground: React.FC<LatentBackgroundProps> = ({
  ...
  useEffect(() => {
    ...
    const updateAudioResponse = () => {
      ...
      currentDitheringMount?.setSpeed(
