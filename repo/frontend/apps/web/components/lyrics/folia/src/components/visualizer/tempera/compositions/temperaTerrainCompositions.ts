...
const chasm: TemperaCompositionDrawer = (ctx) => {
  ...
  addMass(
    ctx,
    rectPolygon(-bleed, height * 0.3, width * 0.46 + bleed, height * 0.7 + bleed),
    palette.tone3,
    {
      enterDX: -width * 0.1,
      edgeWidth: 2,
  ...
  addMass(
    ctx,
    rectPolygon(width * 0.56, height * 0.2, width * 0.44 + bleed, height * 0.8 + bleed),
    palette.tone4,
    {
      delay: 0.08,
      enterDX: width * 0.1,
...
const overhang: TemperaCompositionDrawer = (ctx) => {
  ...
  addMass(
    ctx,
    rectPolygon(-bleed, -bleed, width + bleed * 2, height * 0.34 + bleed),
    palette.tone4,
    {
      enterDY: -height * 0.1,
      edgeWidth: 3,
  ...
  addMass(
    ctx,
    rectPolygon(width * 0.72, height * 0.34, width * 0.28 + bleed, height * 0.66 + bleed),
    palette.tone3,
    {
      delay: 0.08,
      enterDX: width * 0.1,
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(-bleed, height * 0.34, width + bleed * 2, height * 0.14),
      palette.tone2,
      0.5,
      ctx.gradient,
    ),
...
const stepWell: TemperaCompositionDrawer = (ctx) => {
  ...
  (
    [
      [0.14, 0.3, 0.72, palette.tone3],
      [0.24, 0.44, 0.52, palette.tone4],
    ] as const
  ).forEach(([x, y, w, tone], index) => {
    ctx.add(
...
const pierRow: TemperaCompositionDrawer = (ctx) => {
  ...
  [0.04, 0.28, 0.52, 0.76].forEach((x, index) => {
    addMass(
      ctx,
      rectPolygon(width * x, height * 0.46, width * 0.14, height * 0.54 + bleed),
      palette.tone4,
      {
  ...
  addMass(
    ctx,
    rectPolygon(-bleed, height * 0.32, width + bleed * 2, height * 0.16),
    palette.tone3,
    {
      delay: 0.02,
      enterDX: -width * 0.12,
...
const revetment: TemperaCompositionDrawer = (ctx) => {
  ...
  const ribs = Array.from({ length: 9 }, (_, index) => {
    ...
    return {
      x1: x,
      y1: Math.max(top, height * 0.3) + height * 0.03,
      x2: x - width * 0.03,
      y2: height + bleed,
    };
  });
...
const towerCrop: TemperaCompositionDrawer = (ctx) => {
  ...
  addMass(
    ctx,
    rectPolygon(width * 0.58, height * 0.18, width * 0.42 + bleed, height * 0.82 + bleed),
    palette.tone4,
    {
      enterDX: width * 0.08,
      enterDY: height * 0.06,
  ...
  addFaceRuling(
    ctx,
    rectPolygon(width * 0.62, height * 0.26, width * 0.12, height * 0.6),
    257,
    palette.paper,
    0.3,
  );
...
const rubbleFan: TemperaCompositionDrawer = (ctx) => {
  ...
  for (let index = 0; index < 5; index += 1) {
    ...
    addMass(
      ctx,
      rotatePolygon(slab, hubX, hubY, angle),
...
const gnomon: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    ...
    { delay: 0.1, span: 0.65, enterDX: width * 0.1 },
  );
  addMass(
    ctx,
    [
    ...
    palette.tone4,
    { delay: 0.04, enterDY: height * 0.08, edgeWidth: 2 },
  );
  addSurveyLines(ctx, 3);
  if (!ctx.showDecor) return;
  addCornerTicks(ctx);
...
export const TEMPERA_TERRAIN_COMPOSITIONS: Partial<
