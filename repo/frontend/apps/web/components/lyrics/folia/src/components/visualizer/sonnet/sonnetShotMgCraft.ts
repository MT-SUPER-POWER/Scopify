...
const drawOrigamiCrane = ({
  ...
  target
    .moveTo(0, -s * 0.3)
    .lineTo(-direction * s * 0.15, -s * 0.95)
    .lineTo(direction * s * 0.28, -s * 0.1)
    .stroke({ color: primary, width: 1.5, alpha: 0.5 });
  target
    .moveTo(0, -s * 0.3)
    .lineTo(-direction * s * 0.15, -s * 0.95)
    .lineTo(-direction * s * 0.42, s * 0.02)
    .fill({ color: primary, alpha: 0.07 });
  // Neck + head.
  target
    .moveTo(direction * s * 0.5, 0)
...
const drawPaperPlaneTrail = ({
  ...
  target
    .moveTo(px + direction * s, py)
    .lineTo(px - direction * s * 0.8, py - s * 0.55)
    .lineTo(px - direction * s * 0.35, py)
    .lineTo(px + direction * s, py)
    .stroke({ color: primary, width: 2, alpha: 0.65 });
  target
    .moveTo(px + direction * s, py)
    .lineTo(px - direction * s * 0.35, py)
    .lineTo(px - direction * s * 0.8, py + s * 0.4)
  ...
  for (let seg = 0; seg < 3; seg += 1) {
    ...
    target
      .arc(
        px - direction * radius * 0.35,
        py + radius * 0.3,
        radius * (0.34 + seg * 0.06),
        start,
        start + Math.PI * 0.4,
      )
...
const drawWeaveBand = ({ target, radius, seed, primary, secondary }: AdditionalSonnetMgOptions) => {
  ...
  bandY.forEach((y, row) => {
    for (let i = 0; i < strips; i += 1) {
      ...
      if ((i + row) % 2 === 0) {
        target
          .moveTo(x0 + radius * 0.02, y)
          .lineTo(x0 + radius * 0.16, y)
          .stroke({ color: row === 0 ? primary : secondary, width: 5, alpha: 0.4 });
      } else {
        target
          .moveTo(x0 - radius * 0.05, y)
          .lineTo(x0 + radius * 0.02, y)
          .stroke({ color: row === 0 ? primary : secondary, width: 5, alpha: 0.4 });
        target
          .moveTo(x0 + radius * 0.16, y)
          .lineTo(x0 + radius * 0.23, y)
          .stroke({ color: row === 0 ? primary : secondary, width: 5, alpha: 0.4 });
      }
    }
  });
  ...
  for (let i = 0; i < strips; i += 1) {
    ...
    target
      .moveTo(x, y - radius * 0.05)
      .lineTo(x, y + radius * 0.05)
      .stroke({ color: primary, width: 6, alpha: 0.6 });
    target
      .moveTo(x, bandY[1 - overRow] - radius * 0.03)
      .lineTo(x, bandY[1 - overRow] + radius * 0.03)
...
const drawFoldedFan = ({ target, radius, seed, primary, secondary }: AdditionalSonnetMgOptions) => {
  ...
  target
    .arc(0, pivotY, radius * 0.5, -Math.PI / 2 - spread / 2, -Math.PI / 2 + spread / 2)
...
const drawRibbonCurl = ({
  ...
  for (const echo of [0, 1]) {
    ...
    target.moveTo(
      cx + Math.cos(start) * radius * 0.06,
      -radius * 0.1 + Math.sin(start) * radius * 0.06 + offset,
    );
    const steps = 64;
    ...
    target.stroke({
      color: echo === 0 ? primary : secondary,
      width: echo === 0 ? 3 : 1,
      alpha: echo === 0 ? 0.55 : 0.3,
    });
  }
...
const drawDreamcatcher = ({
  ...
  for (let i = -1; i <= 1; i += 1) {
    ...
    target
      .moveTo(sx, topY)
      .lineTo(sx, topY + len)
      .stroke({ color: primary, width: 1, alpha: 0.45 });
    const featherY = topY + len;
    target
      .moveTo(sx, featherY)
      .lineTo(sx, featherY + radius * 0.12)
...
const drawTasselDrop = ({
  ...
  target
    .moveTo(-radius * 0.3, barY)
    .lineTo(radius * 0.3, barY)
    .stroke({ color: primary, width: 2, alpha: 0.4 });
  const threads = 7;
  for (let i = 0; i < threads; i += 1) {
    ...
    target
      .circle(x + sway * 0.6, barY + len + 3, 2.5)
