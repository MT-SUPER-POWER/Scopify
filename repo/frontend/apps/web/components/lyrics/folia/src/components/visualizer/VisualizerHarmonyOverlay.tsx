...
const HarmonyGlowText: React.FC<{
  ...
  const graphemeTimings = useMemo(
    () => buildLineGraphemeTimeline(createHarmonyLine(vocal)),
    [vocal],
  );
  ...
  const fillWidth = useTransform(currentTime, (latest) => {
    ...
    for (let index = 0; index < timingCount; index += 1) {
      ...
      if (latest <= end) {
        return (
          startWidth + (endWidth - startWidth) * ((latest - start) / Math.max(0.001, end - start))
        );
      }
    }
  ...
  const renderParts = (active: boolean) =>
    parts.map((part) => {
      const color = part.word
        ? resolveWordColor(part.word.text, theme.wordColors, theme.accentColor, {
            cjkMatchMode: "exact",
          })
    ...
    <AnimatePresence>
      {snapshot.lines.length > 0 && (
        <motion.div
          ...
          className="pointer-events-none absolute inset-x-0 z-30 flex flex-col items-center gap-1.5 px-5 text-center"
