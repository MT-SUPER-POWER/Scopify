...
export class TemperaPixiRuntime {
  ...
  private drawOverlay(width: number, height: number) {
    ...
    g.moveTo(paddingX, paddingY + 14)
      .lineTo(paddingX, paddingY)
      .lineTo(paddingX + 14, paddingY)
      .stroke({ color: primary, width: 1.5, alpha: 0.5 });
    g.moveTo(width - paddingX - 14, height - paddingY)
      .lineTo(width - paddingX, height - paddingY)
      .lineTo(width - paddingX, height - paddingY - 14)
      .stroke({ color: primary, width: 1.5, alpha: 0.5 });
  ...
  private updateShot(view: TemperaShotView, time: number, width: number, height: number) {
    ...
    view.glyphs.forEach((glyph) => {
      ...
      glyph.echoes.forEach((echo, index) => {
        ...
        echo.position.set(glyph.baseX + frame.echoX * depth, glyph.baseY + frame.echoY * depth);
  ...
  private drawWipe(travel: number, angle: number, width: number, height: number, color: string) {
    ...
    wipe.clear();
    wipe
      .poly([start, -half, end, -half, end + notch, 0, end, half, start, half, start + notch, 0])
  ...
  private renderFrame = () => {
    ...
    const outgoingTransition =
      this.options.program.paragraphs[paragraphIndex]?.transitionOut ?? null;
    /**
    ...
    this.sceneCache.forEach((scene, index) => {
      ...
      const paragraphTransitionFrame =
        isIncoming && outgoingTransition
          ? resolveTemperaEnterTransitionFrame(
          ...
          : entering && previousTransition
            ...
            : resolveTemperaExitTransitionFrame(scene.paragraph, time, transitionsEnabled);
      ...
      scene.shots.forEach((shot, shotIndex) => {
        ...
        const isHandingOff = shotIndex < activeShotIndex && this.resolveShotExit(shot, time) < 1;
