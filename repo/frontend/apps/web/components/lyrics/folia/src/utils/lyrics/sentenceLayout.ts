...
class SentenceLayout implements LyricLayoutUnit {
  ...
  private static splitByWesternWords(text: string): string[] {
    ...
    const westernBlockRegex =
      /[a-zA-Z0-9]+(?:[a-zA-Z0-9'\-]*[a-zA-Z0-9]+)?(?:\s+[a-zA-Z0-9]+(?:[a-zA-Z0-9'\-]*[a-zA-Z0-9]+)?)+[.,;:!?。，；：！？]?\s*/g;
  ...
  private static shouldSkipBoundary(
    fullText: string,
    beforeBlock: string,
    _boundaryPos: number,
  ): boolean {
  ...
  private static secondarySplit(
    ...
    while (sentences.length < targetCount) {
      ...
      const textHash = sentences.reduce(
        (acc, s) => acc + s.text.split("").reduce((sum, c) => sum + c.charCodeAt(0), 0),
        0,
      );
