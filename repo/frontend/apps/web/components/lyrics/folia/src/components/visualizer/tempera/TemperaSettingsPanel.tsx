...
const TemperaSettingsPanel: React.FC<VisualizerSettingsPanelProps> = ({
  ...
  const booleanOptions: VisualizerPresetOption<boolean>[] = useMemo(
    () => [
      { value: true, label: t("folia.options.temperaToggleOn") || "开启" },
      { value: false, label: t("folia.options.temperaToggleOff") || "关闭" },
    ],
    [t],
  );
  ...
  const colorModeOptions: VisualizerPresetOption<TemperaColorMode>[] = useMemo(
    () => [
      { value: "duo", label: t("folia.options.temperaColorModeDuo") || "主题双色" },
      { value: "mono", label: t("folia.options.temperaColorModeMono") || "黑白灰" },
      { value: "gradient", label: t("folia.options.temperaColorModeGradient") || "封面渐变" },
    ],
    [t],
  );
  ...
  const visibilityControls: Array<{
    key: Extract<
      keyof TemperaTuning,
      "showBlocks" | "showDecor" | "textInversion" | "enableTransitions"
    >;
    label: string;
  }> = [
    { key: "showBlocks", label: t("folia.options.temperaShowBlocks") || "色块场景" },
    { key: "showDecor", label: t("folia.options.temperaShowDecor") || "装饰元素" },
    { key: "textInversion", label: t("folia.options.temperaTextInversion") || "文字动态反色" },
    { key: "enableTransitions", label: t("folia.options.temperaEnableTransitions") || "场景转场" },
  ];
  ...
  const motionControls: Array<{
    ...
    { key: "cameraIntensity", label: t("folia.options.temperaCameraIntensity") },
    { key: "glyphMotion", label: t("folia.options.temperaGlyphMotion") },
  ];
    ...
    >
      <div className="space-y-1">
        <div className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>
          {t("folia.options.temperaSettings")}
        </div>
        <div className="text-xs opacity-50" style={{ color: "var(--text-secondary)" }}>
          {t("folia.options.temperaSettingsDesc")}
        </div>
      </div>

      <TemperaSettingsSection title={t("folia.options.temperaImageSection") || "画布图片"}>
      ...
      <TemperaSettingsSection title={t("folia.options.temperaQualitySection")}>
        <div
        ...
        >
          <TemperaRangeControl
            label={t("folia.options.temperaTextureResolution")}
          ...
          >
            {t("folia.options.temperaTexturePerformanceWarning")}
          </p>
        </div>
      </TemperaSettingsSection>

      <TemperaSettingsSection title={t("folia.options.temperaMotionSection")}>
        ...
        <TemperaRangeControl
          label={t("folia.options.temperaGlyphSettleStretch") || "逐字入场时序"}
        ...
        <p
          ...
          style={{ color: "var(--text-secondary)" }}
        >
          {t("folia.options.temperaGlyphSettleStretchHint") ||
            "0 = 每字用同样的短窗口快速落位，切镜前画面已静止，适合快歌；1 = 整个分镜的字精确落在歌词结束那一刻，全程都在动，适合慢歌。"}
        </p>
        <VisualizerPresetGroup
          label={t("folia.options.temperaColorMode")}
      ...
      <TemperaSettingsSection title={t("folia.options.temperaPostProcessSection") || "后处理"}>
        <VisualizerPresetGroup
          label={t("folia.options.temperaPostProcessEnabled") || "整体后处理滤镜"}
        ...
        {temperaTuning.postProcessEnabled && (
          <>
            <VisualizerPresetGroup
              label={t("folia.options.temperaPostProcessTextureCompression") || "后处理纹理压缩"}
              value={temperaTuning.postProcessTextureCompression}
              options={booleanOptions}
              onChange={(postProcessTextureCompression) =>
                onTemperaTuningChange?.({ postProcessTextureCompression })
              }
              isDaylight={isDaylight}
              theme={theme}
            />
            <p
              ...
              style={{ color: "var(--text-secondary)" }}
            >
              {t("folia.options.temperaPostProcessTextureCompressionHint") ||
                "开启后后处理在 1x 纹理上运算再拉伸到画布，降低GPU和显存压力"}
            </p>
            <TemperaRangeControl
              label={t("folia.options.temperaPostProcessGrain") || "胶片颗粒"}
            ...
            />
            <TemperaRangeControl
              label={t("folia.options.temperaPostProcessContrast") || "对比度增强"}
            ...
            />
            <TemperaRangeControl
              label={t("folia.options.temperaPostProcessRgbShift") || "RGB 色差"}
            ...
            />
            <TemperaRangeControl
              label={t("folia.options.temperaPostProcessVignette") || "暗角"}
            ...
            />
            <TemperaRangeControl
              label={t("folia.options.temperaPostProcessLensDistortion") || "透镜扭曲"}
              ...
              onChange={(postProcessLensDistortion) =>
                onTemperaTuningChange?.({ postProcessLensDistortion })
              }
              onPointerDown={onSliderPointerDown}
              onPointerUp={onSliderCommit}
            />
      ...
      <TemperaSettingsSection title={t("folia.options.temperaDisplaySection")}>
