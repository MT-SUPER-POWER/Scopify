...
export const resolveClampFontPx = (minRem: number, preferredVw: number, maxRem: number): number => {
  ...
  return Math.min(
    maxRem * ROOT_FONT_PX,
    Math.max(minRem * ROOT_FONT_PX, viewportWidth * (preferredVw / 100)),
  );
};
...
export const measureMonetGraphemeOffsets = (
  text: string,
  fontPx: number,
  fontSpec: string,
): number[] => {
...
export const resolveMonetLineStatus = (
  ...
  if (
    index === activeIndex ||
    (activeIndex < 0 && currentTime >= line.startTime && currentTime <= line.endTime)
  ) {
    return "active";
  }
...
const findLineIndex = (lines: Line[], target: Line | null): number => {
  ...
  return lines.findIndex(
    (line) => line.startTime === target.startTime && line.fullText === target.fullText,
  );
};
...
export const measureMonetLineLayout = ({
  ...
  const hasActiveTranslation =
    showSubtitleTranslation && status === "active" && Boolean(line.translation?.trim());
