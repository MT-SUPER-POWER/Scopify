...
const chainPorts: TemperaCompositionDrawer = (ctx) => {
  ...
  addCutField(
    ctx,
    palette.tone4,
    ports.map((port) => circlePolygon(port.x, port.y, unit * 0.11, 40)),
  );
  ctx.add(
    drawRings(
      ctx.pixi,
      ports.map((port) => ({ x: port.x, y: port.y, radius: unit * 0.11 })),
      palette.ink,
      2.4,
      0.85,
    ),
...
const bridgeSpan: TemperaCompositionDrawer = (ctx) => {
  ...
  const band = axisRect(
    ...
    flowSpan(ctx),
    height * 0.46,
    channelAxis(ctx) + Math.PI / 2,
  );
  ctx.add(drawPolygonFill(ctx.pixi, band, palette.tone3, 0.95, ctx.gradient), {
    delay: 0.12,
    span: 0.6,
    enterDX: -width * 0.16,
  });
  ctx.add(drawPolygonOutline(ctx.pixi, band, palette.ink, 2, 0.7), {
    delay: 0.2,
    span: 0.5,
    enterDX: width * 0.1,
  });
...
export const TEMPERA_CORRIDOR_COMPOSITIONS: Partial<
