...
const VisualizerDiorama: React.FC<VisualizerDioramaProps> = (props) => {
  ...
  const [committedSong, setCommittedSong] = useState<{
  ...
  const committedSig =
    committedSong.lines.length === 0
      ? ""
      : `${committedSong.lines.length}|${committedSong.lines[0]?.fullText ?? ""}`;
  useEffect(() => {
  ...
  const activeOutgoingIndex = startingTransition
    ...
    : transition
      ...
      : null;
  const pruneFrom =
    activeOutgoingIndex != null ? Math.min(globalIndex, activeOutgoingIndex) : globalIndex;
  ...
  return (
    ...
    >
      <div className="absolute inset-0 z-0">
        ...
        >
          ...
          <DioramaScene
            ...
            showLyrics={showText}
            geometryVisibility={
              dioramaTuning?.geometryVisibility ?? DEFAULT_DIORAMA_TUNING.geometryVisibility
            }
            particleDensity={
            ...
            backgroundParticleRadial={
              dioramaTuning?.backgroundParticleRadial ??
              DEFAULT_DIORAMA_TUNING.backgroundParticleRadial
            }
            ...
            glowIntensity={
              (dioramaTuning?.glowEnabled ?? DEFAULT_DIORAMA_TUNING.glowEnabled)
                ? (dioramaTuning?.glowIntensity ?? DEFAULT_DIORAMA_TUNING.glowIntensity)
                : 0
            }
            soulIntensity={
              (dioramaTuning?.soulEnabled ?? DEFAULT_DIORAMA_TUNING.soulEnabled)
                ? (dioramaTuning?.soulIntensity ?? DEFAULT_DIORAMA_TUNING.soulIntensity)
                : 0
            }
            soulActiveEnabled={
              (dioramaTuning?.soulEnabled ?? DEFAULT_DIORAMA_TUNING.soulEnabled) &&
            ...
            gradientIntensity={
              (dioramaTuning?.gradientEnabled ?? DEFAULT_DIORAMA_TUNING.gradientEnabled)
                ? (dioramaTuning?.gradientIntensity ?? DEFAULT_DIORAMA_TUNING.gradientIntensity)
                : 0
            }
      ...
      <div className="pointer-events-none relative z-10 flex h-[70vh] w-full items-end justify-center p-8">
