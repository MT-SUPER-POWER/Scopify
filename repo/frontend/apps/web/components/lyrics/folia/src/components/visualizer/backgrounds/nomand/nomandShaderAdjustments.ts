...
const applyOverscanMargin = (scale: number) =>
  scale <= 1 ? 1 : clampOverscan(scale + OVERSCAN_MARGIN);

/** 图像 UV 每侧被推出去 `margin` 时，遮住裁切边所需的最小放大倍率。 */
const overscanForMargin = (margin: number) =>
  applyOverscanMargin(1 / (1 - 2 * Math.min(Math.max(margin, 0), 0.45)));
...
export const getPaperTextureOverscan = (roughness: number, fiber: number) => {
  ...
  const maxNormal =
    2 * folds + 1.5 * crumples + 0.2 * drops + 0.75 * clamp01(roughness) + 0.1 * clamp01(fiber);
...
export const getLensDistortionOverscan = (bulge: number, spread: number) => {
  ...
  if (low >= MAX_OVERSCAN || cornerPush(MAX_OVERSCAN) > MAX_OVERSCAN * headroom)
    return MAX_OVERSCAN;
