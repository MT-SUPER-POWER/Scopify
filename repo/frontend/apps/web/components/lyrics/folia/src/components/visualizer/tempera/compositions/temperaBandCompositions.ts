...
const bandStrip: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(-bleed, bandY, width + bleed * 2, bandHeight),
      palette.tone3,
      0.96,
      ctx.gradient,
    ),
  ...
  ctx.add(
    drawCrossMarks(
      ctx.pixi,
      buildCrossRow(ctx.seed, 17, width * 0.06, bandY - height * 0.16, 4, width * 0.055, 9),
      palette.ink,
      2,
      0.8,
    ),
...
const horizonBand: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(-bleed, -bleed, width + bleed * 2, waterline + bleed),
      palette.tone1,
      0.94,
      ctx.gradient,
    ),
  ...
  ctx.add(drawPolygonFill(ctx.pixi, water, palette.tone3, 0.95, ctx.gradient), {
    delay: 0.05,
    span: 0.55,
    enterDY: height * 0.3,
  });
  ctx.add(
    drawHatchFill(
      ctx.pixi,
      water,
      { ...buildHatchSpec(ctx.seed, 29), angle: 0 },
      palette.tone4,
      0.55,
    ),
  ...
  ctx.add(
    drawPolyline(
      ctx.pixi,
      buildWavyPath(ctx.seed, 31, -bleed, width + bleed, waterline, height * 0.012, 30),
      palette.ink,
      2.2,
      0.85,
    ),
...
const deepDive: TemperaCompositionDrawer = (ctx) => {
  ...
  tones.forEach((tone, index) => {
    ...
    ctx.add(
      drawPolygonFill(
        ctx.pixi,
        rectPolygon(-bleed, top, width + bleed * 2, bandHeight + 1),
        tone,
        0.95,
        ctx.gradient,
      ),
    ...
    ctx.add(
      drawPolyline(
        ctx.pixi,
        buildWavyPath(ctx.seed, 37 + index, -bleed, width + bleed, top, height * 0.008, 24),
        palette.paper,
        1.6,
        0.5,
      ),
...
const toneRamp: TemperaCompositionDrawer = (ctx) => {
  ...
  for (let index = 0; index < steps; index += 1) {
    ...
    const column = rectPolygon(
      -bleed + columnWidth * index,
      -bleed,
      columnWidth,
      height + bleed * 2,
    );
    ctx.add(
      drawHatchFill(
        ctx.pixi,
        column,
        { ...spec, spacing: spec.spacing * (1.6 - index * 0.32) },
        palette.tone4,
        0.6,
      ),
...
const doubleBand: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(-bleed, top, width + bleed * 2, rail),
      palette.tone3,
      0.95,
      ctx.gradient,
    ),
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(-bleed, top + rail + slot, width + bleed * 2, rail),
      palette.tone4,
      0.95,
      ctx.gradient,
    ),
...
const tiltBand: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(drawPolygonFill(ctx.pixi, band, palette.tone4, 0.95, ctx.gradient), {
    delay: 0.06,
    span: 0.55,
    enterDX: -width * 0.35,
  });
  ctx.add(
    drawHatchFill(
      ctx.pixi,
      band,
      { ...buildHatchSpec(ctx.seed, 59), angle: Math.PI / 2 },
      palette.paper,
      0.3,
    ),
...
const edgeRails: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(-bleed, -bleed, width + bleed * 2, rail + bleed),
      palette.tone3,
      0.94,
      ctx.gradient,
    ),
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(-bleed, height - rail, width + bleed * 2, rail + bleed),
      palette.tone3,
      0.94,
      ctx.gradient,
    ),
  ...
  ctx.add(
    ...
    { delay: 0.16, span: 0.5, enterDX: width * 0.2 },
  );
};
...
const terrace: TemperaCompositionDrawer = (ctx) => {
  ...
  tones.forEach((tone, index) => {
    ...
    ctx.add(
      drawPolygonFill(
        ctx.pixi,
        rectPolygon(left, top, width + bleed * 2 - left - bleed, bottom - top),
        tone,
        0.94,
        ctx.gradient,
      ),
