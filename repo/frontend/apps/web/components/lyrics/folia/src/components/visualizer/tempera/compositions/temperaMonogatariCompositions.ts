...
const monogatariCard: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    ...
    { delay: 0.22, span: 0.6, enterDX: ctx.width * 0.15 },
  );
};
...
const monogatariRule: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      rectPolygon(-bleed, y, width + bleed * 2, height * 0.012),
      palette.paper,
      0.85,
      ctx.gradient,
    ),
  ...
  ctx.add(
    ...
    { delay: 0.2, span: 0.6, enterDX: width * 0.2 },
  );
};
...
const monogatariEdge: TemperaCompositionDrawer = (ctx) => {
  ...
  const spine = rectPolygon(
    fromLeft ? -bleed : width - band,
    -bleed,
    band + bleed,
    height + bleed * 2,
  );
  ctx.add(
    drawPolygonFill(
      ctx.pixi,
      spine,
      field === palette.tone4 ? palette.tone1 : palette.tone4,
      0.95,
      ctx.gradient,
    ),
...
const monogatariStack: TemperaCompositionDrawer = (ctx) => {
  ...
  ctx.add(
    ...
    { delay: 0.14, span: 0.6, enterDY: height * 0.12 },
  );
};
...
export const TEMPERA_MONOGATARI_COMPOSITIONS: Partial<
