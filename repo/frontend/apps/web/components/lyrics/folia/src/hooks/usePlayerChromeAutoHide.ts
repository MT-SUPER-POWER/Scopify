...
const getNextPlayerChromeVisibilityMode = (
  mode: PlayerChromeVisibilityMode,
...
const getInitialPlayerChromeVisibilityMode = (
  autoHidePlayerChrome: boolean,
  initialPlayerChromeHidden: boolean,
): PlayerChromeVisibilityMode => {
  if (typeof window === "undefined") {
    return autoHidePlayerChrome
      ? "auto-hide"
...
export const usePlayerChromeAutoHide = ({
  ...
  const [playerChromeVisibilityMode, setPlayerChromeVisibilityMode] =
    useState<PlayerChromeVisibilityMode>(() =>
  ...
  const applyPlayerChromeVisibilityMode = useCallback(
    (nextMode: PlayerChromeVisibilityMode) => {
  ...
  const cyclePlayerChromeVisibilityMode = useCallback(() => {
    applyPlayerChromeVisibilityMode(
      getNextPlayerChromeVisibilityMode(playerChromeVisibilityModeRef.current),
    );
  }, [applyPlayerChromeVisibilityMode]);

  useEffect(() => {
    window.localStorage.setItem(
      PLAYER_CHROME_VISIBILITY_MODE_STORAGE_KEY,
      playerChromeVisibilityMode,
    );
  }, [playerChromeVisibilityMode]);
  ...
  return {
    ...
    setPlayerChromeVisibilityMode: applyPlayerChromeVisibilityMode,
  };
};
