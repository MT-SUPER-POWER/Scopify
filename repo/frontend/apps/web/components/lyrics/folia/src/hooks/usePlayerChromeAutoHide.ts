...
const getInitialPlayerChromeVisibilityMode = (autoHidePlayerChrome: boolean, initialPlayerChromeHidden: boolean): PlayerChromeVisibilityMode => {
    if (typeof window === 'undefined') {
        return autoHidePlayerChrome ? 'auto-hide' : initialPlayerChromeHidden ? 'always-hidden' : 'always-visible';
    }
...
export const usePlayerChromeAutoHide = ({
    ...
    const applyPlayerChromeVisibilityMode = useCallback((nextMode: PlayerChromeVisibilityMode) => {
    ...
    useEffect(() => {
        window.localStorage.setItem(PLAYER_CHROME_VISIBILITY_MODE_STORAGE_KEY, playerChromeVisibilityMode);
