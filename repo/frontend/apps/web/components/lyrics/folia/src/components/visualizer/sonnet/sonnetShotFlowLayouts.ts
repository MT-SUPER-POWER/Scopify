...
export const layoutEditorialColumn = <T extends SonnetFlowLayoutBox>(
  ...
  if (variant === 0) {
    ...
    placeWithGlobalFit(ctx, () => {
      ...
      const totalHeight =
        boxes.reduce((sum, box) => sum + box.measuredHeight, 0) + stackGap * (boxes.length - 1);
      // Prefer the single centered rail as long as it could fit at the
      // ladder's minimum scale; only genuinely long shots wrap.
      const fitsSingleRail =
        boxes.reduce((sum, box) => sum + box.measuredHeight, 0) * 0.52 +
    ...
    placeWithGlobalFit(ctx, () => {
      ...
      if (before.length > 0) {
        ...
        const kickerWidth =
          before.reduce((sum, box) => sum + box.measuredWidth, 0) + flowGap * (before.length - 1);
    ...
    placeWithGlobalFit(ctx, () => {
      ...
      const layLine = (line: T[], lineY: number, enterX: number) => {
        const lineWidth =
          line.reduce((sum, box) => sum + box.measuredWidth, 0) + flowGap * (line.length - 1);
    ...
    placeWithGlobalFit(ctx, () => {
      ...
      const blockLeft = -width * 0.4;
      const blockRight = width * 0.4;
...
export const layoutFragmentCollage = <T extends SonnetFlowLayoutBox>(
  ...
  const rectSeparation = (a: PlacedRect, b: PlacedRect) =>
    Math.max(
  ...
  placeWithGlobalFit(ctx, (globalScale) => {
    ...
    for (let i = 0; i < boxes.length; i++) {
      ...
      box.layoutDirection =
        Math.abs(Math.cos(candidate)) >= Math.abs(Math.sin(candidate)) ? "vertical" : "horizontal";
