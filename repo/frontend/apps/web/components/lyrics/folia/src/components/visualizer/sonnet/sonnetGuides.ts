...
export const createSonnetGuide = (
  ...
  const color = colorNumber(pixi, isHero ? theme.accentColor : theme.secondaryColor);
  ...
  const getBezier = (
    p_0: { x: number; y: number },
    p_1: { x: number; y: number },
    p_2: { x: number; y: number },
    p_3: { x: number; y: number },
    t: number,
  ) => {
    const mt = 1 - t;
    return {
      x:
        mt * mt * mt * p_0.x + 3 * mt * mt * t * p_1.x + 3 * mt * t * t * p_2.x + t * t * t * p_3.x,
      y:
        mt * mt * mt * p_0.y + 3 * mt * mt * t * p_1.y + 3 * mt * t * t * p_2.y + t * t * t * p_3.y,
    };
  };
  ...
  for (let i = 0; i < numShapes; i++) {
    ...
    if (type === 0) {
      ...
      shape
        ...
        .moveTo(0, -size)
        .lineTo(0, size)
        .stroke({ color, width: 2, alpha: sAlpha });
    } else {
      shape
        .moveTo(0, -size)
        .lineTo(size, 0)
  ...
  const update = (progress: number) => {
    ...
    if (drawProgress > 0 && fadeOut > 0) {
      ...
      graphics
        .circle(head.x, head.y, isHero ? 4.5 : 3)
        .fill({ color: 0xffffff, alpha: 1 * fadeOut });
    }
      ...
      if (localProg > -0.15 && fadeOut > 0) {
        ...
        if (localProg > 0 && localProg < 1.3) {
          ...
          if (headT > 0 && headT < 1) {
            ...
            graphics
              .circle(headPos.x, headPos.y, isHero ? 7 : 4)
              .fill({ color, alpha: 0.9 * fadeOut });
            graphics
              .circle(headPos.x, headPos.y, isHero ? 2.5 : 1.5)
              .fill({ color: 0xffffff, alpha: 1 * fadeOut });

            // Outer "follow circle" glow for rhythm game vibe
            graphics
              .circle(headPos.x, headPos.y, isHero ? 20 : 12)
              .stroke({ color, width: isHero ? 2 : 1, alpha: 0.4 * fadeOut });
          }
        }
      }
    });
    ...
    if (rectSpline) {
      ...
      if (localProg > 0 && localProg < 1.3 && fadeOut > 0) {
        ...
        if (headT > tailT) {
          ...
          graphics.stroke({
            color,
            width: rectSpline.thickness,
            alpha: strokeProps.alpha * fadeOut * 0.7,
          });
          ...
          graphics.stroke({
            color: 0xffffff,
            width: rectSpline.thickness * 0.3,
            alpha: strokeProps.alpha * fadeOut * 0.9,
          });
        }
      }
    }
