...
const getOrMeasureMonetLineLayout = (
  ...
  const cacheKey = buildMonetLayoutCacheKey(
    entry,
    fontPx,
    translationFontPx,
    fontStack,
...
const buildPositionedEntries = (
  ...
  const anchorIndex = Math.max(
    0,
    measuredEntries.findIndex((entry) => entry.offset === 0),
  );
...
const MonetTimedTokenSpan: React.FC<{
  ...
  const resolvedAccentColor =
    isChorus && chorusAccentColor ? mixColors(accentColor, chorusAccentColor, 0.48) : accentColor;

  return (
    <span className="block w-full max-w-full min-w-0 wrap-break-word whitespace-pre-wrap">
      {tokens.map((token) =>
        renderStaticPassed ? (
          <span
            key={token.key}
...
const MonetWordSweep: React.FC<{
  ...
  const fillWidth = useTransform(currentTime, (latest) => {
    ...
    const endWidth =
      graphemeOffsets[Math.min(wholeIndex + 1, graphemeOffsets.length - 1)] ?? startWidth;
  ...
  const glowShadow = useTransform(currentTime, (latest) => {
    ...
    const radiusTwo = Math.round(fontPx * (isChorus ? 0.9 : 0.65));
  ...
  return (
    ...
    >
      ...
      {showSubtitleTranslation && entry.status === "active" && entry.line.translation ? (
        <motion.div
          className="min-w-0 overflow-hidden wrap-break-word whitespace-pre-wrap"
  ...
  const handleRailWheel = useCallback(
    (event: WheelEvent) => {
      ...
      const steps = clampScrollSteps(
        Math.trunc(wheelAccumulatorRef.current / MONET_SCROLL_STEP_PX),
      );
      if (steps !== 0) {
        wheelAccumulatorRef.current = 0;
  ...
  return (
    ...
    >
      {positionedEntries.length > 0 ? (
        ...
        <div
          className="absolute top-1/2 left-0 -translate-y-1/2 font-semibold"
          style={{
