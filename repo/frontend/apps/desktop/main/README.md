...
```text
main/
├─ main.ts          极薄的 Electron 入口，只调用 initializeApplication
├─ core/            应用生命周期与各能力的 composition root
...
├─ prototypes/      尚未成为正式能力的验证代码
├─ constants.ts     资源句柄与进程级常量
└─ preload.ts       DesktopBridge 的安全实现
```

### `core/`

`core/index.ts` 是进程级 composition root。它记录并执行固定启动顺序：
