...
const shutdown = createApplicationShutdown({
  ...
  onError: (error) => coreLog.error("[app] shutdown cleanup failed", error),
  onStarted: () => {
    quitting = true;
  },
  ...
  void backend
    ...
    .then((status) => {
      if (status.state === "error") {
        backendLog.error("[backend] startup failed; continuing into the app", status.error);
      }
    })
    .catch((error) => backendLog.error("[backend] startup reconciliation failed", error));
...
async function releaseMainWindow(window: BrowserWindow) {
  await capabilities.waitUntilReady().catch((error) => {
    wallpaperLog.error("[desktop-playback-wallpaper] initialization failed", error);
  });
  ...
  stopProcessMemoryMonitor ??= startProcessMemoryMonitor(() => mainWindow);
}
...
async function prepareApplication() {
  ...
  if (verification && !verification.ok) {
    rendererLog.error(`[renderer] ${verification.message}`);
    dialog.showErrorBox("Renderer verification failed", verification.message);
    app.quit();
    return;
  }
  if (verification?.ok) rendererLog.info("[renderer] verified artifact", verification.manifest);
  ...
  if (process.platform === "darwin") {
    try {
      ...
      coreLog.error("[app] failed to set the macOS Dock icon", error);
    }
  }
