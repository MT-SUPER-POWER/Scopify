...
// Scopify Windows desktop-wallpaper helper: parents the Electron main window into the WorkerW
// layer below the desktop icons and keeps the session alive across explorer restarts. Protocol: JSONL events on
...
fn main() {
    ...
    match cli::parse(&args) {
        ...
        Err(message) => {
            eprintln!("usage: scopify-wallpaper-helper attach --hwnd <n> [--zguard]");
            eprintln!("       scopify-wallpaper-helper move --hwnd <n>");
            eprintln!("       scopify-wallpaper-helper detach --hwnd <n>");
...
#[cfg(windows)]
pub(crate) static DETACH_REQUESTED: std::sync::atomic::AtomicBool =
...
fn run_windows(command: Command) {
    unsafe {
        match command {
            ...
            Command::Attach { hwnd, zguard } => {
                attach_resident(hwnd, zguard);
            }
        }
    }
}
...
/// Resident attach: mount into WorkerW, install monitoring,
/// pump messages until a `detach` line arrives on stdin.
#[cfg(windows)]
unsafe fn attach_resident(hwnd: isize, zguard: bool) {
    ...
    if let Err(err) = monitor::start(hwnd, worker_w, zguard) {
        let _ = attach::detach_window(hwnd);
        events::emit(&Event::Error {
            message: err,
            kind: None,
        });
...
pub fn handle_detach_request() {
    ...
    unsafe {
        if let Some(hwnd) = monitor::resident_folia_hwnd() {
            match attach::detach_window(HWND(hwnd as _)) {
                Ok(()) => events::emit(&Event::Detached { hwnd }),
                Err(message) => events::emit(&Event::Error {
                    message,
                    kind: None,
                }),
            }
