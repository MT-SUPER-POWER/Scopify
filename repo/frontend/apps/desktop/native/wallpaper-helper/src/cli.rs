...
// live in attach.rs / monitor.rs and are cfg-gated to Windows.
...
mod tests {
    ...
    fn parses_attach_with_flags() {
        let command = parse(&args(&["attach", "--hwnd", "12345", "--zguard"])).unwrap();
        assert_eq!(
            command,
            Command::Attach {
                hwnd: 12345,
