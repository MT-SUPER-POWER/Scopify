...
use windows::Win32::UI::WindowsAndMessaging::{
    EnumWindows, FindWindowA, FindWindowExA, GetAncestor, GetWindow, GetWindowLongPtrW,
    GetWindowRect, IsWindow, SendMessageTimeoutW, SetParent, SetWindowLongPtrW, SetWindowPos,
    GA_PARENT, GWL_EXSTYLE, GWL_STYLE, GW_CHILD, HWND_TOP, SMTO_NORMAL, SWP_ASYNCWINDOWPOS,
    SWP_FRAMECHANGED, SWP_NOACTIVATE, SWP_NOMOVE, SWP_NOSIZE, SWP_NOZORDER, SWP_SHOWWINDOW,
    WINDOW_EX_STYLE, WINDOW_STYLE, WS_CAPTION, WS_CHILDWINDOW, WS_CLIPSIBLINGS, WS_EX_ACCEPTFILES,
...
static ORIGINAL_WINDOW_STATE: Mutex<Option<OriginalWindowState>> = Mutex::new(None);
...
unsafe fn probe_classic_worker_w() -> Option<HWND> {
    ...
    let _ = EnumWindows(
        Some(enum_window),
        LPARAM(&mut worker_w as *mut Option<HWND> as isize),
    );
    worker_w
}
...
unsafe fn normalize_styles(hwnd: HWND) {
    let style = normalize_style_bits(GetWindowLongPtrW(hwnd, GWL_STYLE) as u32);
    SetWindowLongPtrW(hwnd, GWL_STYLE, style.0 as isize);

    let ex_style = normalize_extended_style_bits(GetWindowLongPtrW(hwnd, GWL_EXSTYLE) as u32);
    SetWindowLongPtrW(hwnd, GWL_EXSTYLE, ex_style.0 as isize);
}

fn normalize_style_bits(style: u32) -> WINDOW_STYLE {
    let normal_window_chrome =
        WS_CAPTION | WS_THICKFRAME | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_SYSMENU | WS_POPUP;
    (WINDOW_STYLE(style) | WS_CHILDWINDOW) & !WS_CLIPSIBLINGS & !normal_window_chrome
}

fn normalize_extended_style_bits(style: u32) -> WINDOW_EX_STYLE {
    let normal_window_chrome = WS_EX_ACCEPTFILES
        | WS_EX_APPWINDOW
        | WS_EX_CLIENTEDGE
        | WS_EX_DLGMODALFRAME
        | WS_EX_STATICEDGE
        | WS_EX_WINDOWEDGE;
    WINDOW_EX_STYLE(style) & !normal_window_chrome
}
...
mod tests {
    ...
    #[test]
    fn wallpaper_style_removes_the_normal_window_non_client_frame() {
        let normal_frame =
            WS_CAPTION | WS_THICKFRAME | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_SYSMENU | WS_POPUP;
        let normalized = normalize_style_bits(normal_frame.0);

        assert_ne!(normalized & WS_CHILDWINDOW, WINDOW_STYLE(0));
        assert_eq!(normalized & WS_CAPTION, WINDOW_STYLE(0));
        assert_eq!(normalized & WS_THICKFRAME, WINDOW_STYLE(0));
        assert_eq!(normalized & WS_MINIMIZEBOX, WINDOW_STYLE(0));
        assert_eq!(normalized & WS_MAXIMIZEBOX, WINDOW_STYLE(0));
        assert_eq!(normalized & WS_SYSMENU, WINDOW_STYLE(0));
        assert_eq!(normalized & WS_POPUP, WINDOW_STYLE(0));

        let extended = normalize_extended_style_bits(
            (WS_EX_APPWINDOW | WS_EX_CLIENTEDGE | WS_EX_WINDOWEDGE).0,
        );
        assert_eq!(extended, WINDOW_EX_STYLE(0));
    }
}
...
pub unsafe fn attach_window(hwnd: HWND) -> Result<(HWND, AttachMode), String> {
    if !IsWindow(Some(hwnd)).as_bool() {
        return Err("Scopify main window no longer exists".to_string());
    }
    ...
    let should_capture_original = ORIGINAL_WINDOW_STATE
        .lock()
        .map_err(|_| "original window state lock poisoned".to_string())?
        .is_none();
    let mut original = if should_capture_original {
        let mut bounds = RECT::default();
        GetWindowRect(hwnd, &mut bounds)
            .map_err(|err| format!("GetWindowRect before attach failed: {err}"))?;
        Some(OriginalWindowState {
            parent: 0,
            style: GetWindowLongPtrW(hwnd, GWL_STYLE),
            extended_style: GetWindowLongPtrW(hwnd, GWL_EXSTYLE),
            bounds,
        })
    } else {
        None
    ...
    let previous_parent = match SetParent(hwnd, Some(worker_w)) {
        Ok(parent) => parent,
        Err(err) => {
            if let Some(state) = original {
                SetWindowLongPtrW(hwnd, GWL_STYLE, state.style);
                SetWindowLongPtrW(hwnd, GWL_EXSTYLE, state.extended_style);
            }
            return Err(format!("SetParent failed: {err}"));
        }
    };
    if let Some(state) = original.as_mut() {
        state.parent = previous_parent.0 as isize;
        *ORIGINAL_WINDOW_STATE
            .lock()
            .map_err(|_| "original window state lock poisoned".to_string())? = Some(*state);
    }
...
pub unsafe fn detach_window(hwnd: HWND) -> Result<(), String> {
    ...
    let original = ORIGINAL_WINDOW_STATE
        .lock()
        .map_err(|_| "original window state lock poisoned".to_string())?
        .take();
    let original_parent = original.as_ref().and_then(|state| {
        if state.parent == 0 {
            None
        } else {
            Some(HWND(state.parent as _))
        }
    });
    SetParent(hwnd, original_parent).map_err(|err| format!("SetParent restore failed: {err}"))?;
    if let Some(state) = original {
        SetWindowLongPtrW(hwnd, GWL_STYLE, state.style);
        SetWindowLongPtrW(hwnd, GWL_EXSTYLE, state.extended_style);
        SetWindowPos(
        ...
        .map_err(|err| format!("SetWindowPos restore failed: {err}"))?;
    } else {
        restore_styles(hwnd);
    }
    Ok(())
