...
describe("desktop playback wallpaper PowerShell scripts", () => {
  const developmentRuntime = {
    appPath: "D:/workspace/Scopify/repo/frontend/apps/desktop",
    isPackaged: false,
    resourcesPath: "C:/Program Files/Scopify/resources",
  };

  test("resolves both scripts from the development prototype directory", () => {
    expect(resolveDesktopPlaybackWallpaperScriptPath("host.ps1", developmentRuntime)).toBe(
      join(developmentRuntime.appPath, "prototypes", "desktop-wallpaper-host-spike", "host.ps1"),
    );
    expect(
      resolveDesktopPlaybackWallpaperScriptPath("system-wallpaper.ps1", developmentRuntime),
    ).toBe(
      join(
        developmentRuntime.appPath,
        "prototypes",
  ...
  test("resolves both scripts from packaged resources", () => {
    const packagedRuntime = { ...developmentRuntime, isPackaged: true };

    expect(resolveDesktopPlaybackWallpaperScriptPath("host.ps1", packagedRuntime)).toBe(
      join(packagedRuntime.resourcesPath, "desktop-wallpaper-host-spike", "host.ps1"),
    );
    expect(resolveDesktopPlaybackWallpaperScriptPath("system-wallpaper.ps1", packagedRuntime)).toBe(
      join(packagedRuntime.resourcesPath, "desktop-wallpaper-host-spike", "system-wallpaper.ps1"),
    );
  });
});
...
test("desktop wallpaper driver reuses the main window instead of creating a Replica", () => {
  const driverSource = readFileSync(
    fileURLToPath(
      new URL(
        "../electron/main/capabilities/desktopPlaybackWallpaper/electronDriver.ts",
        import.meta.url,
      ),
    ),
    "utf8",
...
test("production wallpaper hosting uses the resident native helper instead of PowerShell", () => {
  const hostSource = readFileSync(
    fileURLToPath(
      new URL(
        "../electron/main/capabilities/desktopPlaybackWallpaper/nativeWallpaperHost.ts",
        import.meta.url,
      ),
    ),
    "utf8",
