...
const splitBucket = (bucket: ColorBucket): [ColorBucket, ColorBucket] | null => {
  ...
  const channel = (Object.keys(ranges) as Array<keyof typeof ranges>).reduce(
    (widest, candidate) => (ranges[candidate] > ranges[widest] ? candidate : widest),
    "r",
  );
...
const extractRepresentativeColorsFromPixels = (
...
export const extractRepresentativeColors = (imageUrl: string, count = 6): Promise<string[]> =>
  new Promise((resolve) => {
    ...
    image.onload = () => {
      ...
      resolve(
        extractRepresentativeColorsFromPixels(context.getImageData(0, 0, 50, 50).data, count),
      );
    };
