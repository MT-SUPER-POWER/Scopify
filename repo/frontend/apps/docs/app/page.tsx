import type { Metadata } from "next";

import desktopScreenshot from "@/assets/features/desktopMusic.png";
import foliaStageScreenshot from "@/assets/features/lyrics-folia-stage.png";
import { FoliaLatentBackground } from "@/components/marketing/folia-latent-background";
import { LandingClosing } from "@/components/marketing/landing-closing";
import { LandingHeader } from "@/components/marketing/landing-header";
import { LandingHero } from "@/components/marketing/landing-hero";
import { LandingStory } from "@/components/marketing/landing-story";

export const metadata: Metadata = {
  title: "Scopify",
  description: "音乐发现、Folia 动态歌词与桌面体验，都在 Scopify。",
};

const LANDING_THEME = {
  accentColor: "#b8c7ef",
  backgroundColor: "#070912",
  primaryColor: "#d9deea",
  secondaryColor: "#7186b3",
};

export default function HomePage() {
  return (
    <main className="relative isolate min-h-screen overflow-x-clip bg-[#06070b] text-white">
      <div className="pointer-events-none fixed inset-0 z-0">
        <FoliaLatentBackground coverUrl={foliaStageScreenshot.src} theme={LANDING_THEME} />
      </div>
      <div className="pointer-events-none fixed inset-0 z-0 bg-[linear-gradient(180deg,rgba(4,6,12,0.36)_0%,rgba(5,7,13,0.05)_42%,rgba(4,5,9,0.5)_100%)]" />
      <div className="landing-grain pointer-events-none fixed inset-0 z-0" />

      <div className="relative z-10">
        <LandingHeader />
        <LandingHero />
        <LandingStory
          align="left"
          eyebrow="FOLIA"
          title="有些歌，值得看着听。"
          description="逐字歌词跟着演唱推进，排版、颜色和背景也会随歌曲变化。"
          image={foliaStageScreenshot}
          imageAlt="Scopify Folia 动态歌词舞台"
        />
        <LandingStory
          align="right"
          eyebrow="DESKTOP"
          title="关掉窗口，音乐还在。"
          description="桌面歌词、音乐壁纸和播放控制，让正在播放自然留在桌面上。"
          image={desktopScreenshot}
          imageAlt="Scopify 桌面音乐壁纸与 Folia 控制器"
        />
        <LandingClosing />
      </div>
    </main>
  );
}
