...
<ComponentPreview name="shadcn-kbd" />

以内联样式展示快捷键或键盘输入提示。上方示例直接渲染 `@scopify/ui` 中的真实组件，可以操作并观察其状态变化。
...
当前模块公开 `Kbd`、`KbdGroup`。实际 props 继承自对应的 Radix primitive 或原生 HTML 元素，并叠加 shadcn 的样式与 slot 约定。

完整的交互模式、可访问性说明和底层 props 请查看 [shadcn/ui Kbd 键盘按键 文档](https://ui.shadcn.com/docs/components/radix/kbd)；实现差异以本仓库当前 CLI 源码为准。
