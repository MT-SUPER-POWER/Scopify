...
<ComponentPreview name="shadcn-separator" />

在内容区域之间提供视觉或语义分隔。上方示例直接渲染 `@scopify/ui` 中的真实组件，可以操作并观察其状态变化。
...
当前模块公开 `Separator`。实际 props 继承自对应的 Radix primitive 或原生 HTML 元素，并叠加 shadcn 的样式与 slot 约定。

完整的交互模式、可访问性说明和底层 props 请查看 [shadcn/ui Separator 分隔线 文档](https://ui.shadcn.com/docs/components/radix/separator)；实现差异以本仓库当前 CLI 源码为准。
