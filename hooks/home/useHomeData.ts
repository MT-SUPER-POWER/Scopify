...
export interface TimeTheme {
  ...
  greetingKey: "home.greeting.night" | "home.greeting.morning" | "home.greeting.afternoon" | "home.greeting.evening";
  gradient: string;
}

export const TIME_THEMES: TimeTheme[] = [
  { start: 0, end: 5, greetingKey: "home.greeting.night", gradient: "from-indigo-950/90 via-[#121212]/80 to-[#121212] h-80" },
  { start: 5, end: 7, greetingKey: "home.greeting.morning", gradient: "from-rose-300/45 via-orange-200/30 to-[#121212] h-30" },
  { start: 7, end: 10, greetingKey: "home.greeting.morning", gradient: "from-sky-300/60 via-[#121212]/80 to-[#121212] h-80" },
  { start: 10, end: 14, greetingKey: "home.greeting.afternoon", gradient: "from-sky-500/65 via-[#121212]/80 to-[#121212] h-80" },
  { start: 14, end: 17, greetingKey: "home.greeting.afternoon", gradient: "from-cyan-400/60 via-[#121212]/80 to-[#121212] h-80" },
  { start: 17, end: 19, greetingKey: "home.greeting.evening", gradient: "from-orange-400/45 via-purple-500/30 to-[#121212] h-40" },
  { start: 19, end: 22, greetingKey: "home.greeting.evening", gradient: "from-violet-900/80 via-[#121212]/85 to-[#121212] h-80" },
  { start: 22, end: 24, greetingKey: "home.greeting.night", gradient: "from-slate-900/90 via-[#121212]/85 to-[#121212] h-80" },
];
...
export function useHomeData() {
  ...
  const handlePlayPlaylist = useCallback(async (id: number | string, e: React.MouseEvent) => {
    ...
    try {
      ...
      if (!tracks.length) { toast.error(t("home.toast.playlistEmpty")); return; }
      setQueue(tracks, 0);
      await playQueueIndex(0);
    } catch { toast.error(t("home.toast.loadPlaylistFailed")); }
  ...
  const handlePlayAlbum = useCallback(async (id: number | string, e: React.MouseEvent) => {
    ...
    try {
      ...
      const tracks: SongDetail[] = (res.data?.songs || []).map((song: SongDetail) =>
        pruneSongDetail({ ...song, al: { ...song.al, picUrl: song.al?.picUrl || res.data?.album?.picUrl || res.data?.album?.blurPicUrl } }));
      if (!tracks.length) { toast.error(t("home.toast.albumEmpty")); return; }
      setQueue(tracks, 0);
      await playQueueIndex(0);
    } catch { toast.error(t("home.toast.loadAlbumFailed")); }
    finally { setLoadingPlayId(null); }
  }, [loadingPlayId, setQueue, playQueueIndex, t]);
  ...
  const fetchHomeData = useCallback(async () => {
    ...
    Promise.all(requests).then((results) => {
      ...
      const shuffled = [...(recommendRes.data?.recommend || [])]
        .map((item, index) => ({ item, index })).sort(() => Math.random() - 0.5).slice(0, 8)
        .sort((a, b) => a.index - b.index).map(({ item }) => item);
  ...
  return {
    playlists, bannerPlaylist, suggestedArtists, isLoading, loadingPlayId, hasError, dateInfo,
    userName, userId, setLoadingPlayId, handlePlayPlaylist, handlePlayAlbum, fetchHomeData, t,
  };
}
